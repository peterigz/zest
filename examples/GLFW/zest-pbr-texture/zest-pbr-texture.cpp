...
void InitPBRTextureExample(PBRTextureExample *app) {
	...
	zest_shader_handle pbr_vert = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/pbr.vert", "pbr_vert.spv", zest_vertex_shader, true);
	zest_shader_handle pbr_texture_frag = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/pbr_texture.frag", "pbr_texture_frag.spv", zest_fragment_shader, true);
	zest_shader_handle skybox_vert = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/sky_box.vert", "sky_box_vert.spv", zest_vertex_shader, true);
	zest_shader_handle skybox_frag = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/sky_box.frag", "sky_box_frag.spv", zest_fragment_shader, true);
	app->brd_shader = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/genbrdflut.comp", "genbrdflut_comp.spv", zest_compute_shader, true);
	app->irr_shader = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/irradiancecube.comp", "irradiancecube_comp.spv", zest_compute_shader, true);
	app->prefiltered_shader = zest_CreateShaderFromFile(app->device, "examples/GLFW/zest-pbr-texture/shaders/prefilterenvmap.comp", "prefilterenvmap_comp.spv", zest_compute_shader, true);
	...
	app->material_push.ao_index = zest_AcquireSampledImageIndex(app->device, zest_GetImage(app->ao_texture), zest_texture_2d_binding);
	app->material_push.metallic_index = zest_AcquireSampledImageIndex(app->device, zest_GetImage(app->metallic_texture), zest_texture_2d_binding);
	app->material_push.roughness_index = zest_AcquireSampledImageIndex(app->device, zest_GetImage(app->roughness_texture), zest_texture_2d_binding);
...
void UpdateLights(PBRTextureExample *app, float timer) {
	...
	buffer_data->exposure = app->exposure;
	buffer_data->gamma = app->gamma;
...
void UploadMeshData(const zest_command_list context, void *user_data) {
	PBRTextureExample *app = (PBRTextureExample *)user_data;
...
void UpdateImGui(PBRTextureExample *app) {
	...
	zest_StartTimerLoop(app->timer) {
		...
		ImGui::DragFloat("Gamma", &app->gamma, 0.02f);
...
int main(void) {
	...
	InitPBRTextureExample(&imgui_app);

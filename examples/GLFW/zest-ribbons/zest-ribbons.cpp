...
void UploadRibbonData(VkCommandBuffer command_buffer, const zest_frame_graph_context_t *context, void *user_data) {
	...
	if (segment_buffer->storage_buffer) {
		zest_cmd_CopyBuffer(command_buffer, app->ribbon_segment_staging_buffer[context->current_fif], segment_buffer->storage_buffer, app->ribbon_segment_staging_buffer[context->current_fif]->memory_in_use);
	}
	if (ribbon_instance_buffer->storage_buffer) {
		zest_cmd_CopyBuffer(command_buffer, app->ribbon_instance_staging_buffer[context->current_fif], ribbon_instance_buffer->storage_buffer, app->ribbon_instance_staging_buffer[context->current_fif]->memory_in_use);
	}
}
...
void UpdateCallback(zest_microsecs elapsed, void* user_data) {
	...
	app->index_count = 0;
	zest_StageData(app->ribbon_segments, app->ribbon_segment_staging_buffer[context->current_fif], SEGMENT_COUNT *RIBBON_COUNT * sizeof(ribbon_segment));
	app->index_count += (SEGMENT_COUNT * RIBBON_COUNT) * app->ribbon_buffer_info.indicesPerSegment;
	zest_StageData(app->ribbon_instances, app->ribbon_instance_staging_buffer[context->current_fif], app->ribbon_count * sizeof(ribbon_instance));

	zest_swapchain swapchain = zest_GetMainWindowSwapchain();

	app->segment_buffer_info.size = app->ribbon_segment_staging_buffer[context->current_fif]->memory_in_use;
	app->instance_buffer_info.size = app->ribbon_instance_staging_buffer[context->current_fif]->memory_in_use;

...
layout(location = 2) in vec2 in_pbr;

// Multiple Render Targets (G-buffer outputs)
// Position is reconstructed from depth buffer in lighting pass
layout(location = 0) out vec4 gNormal;      // World normal (packed to [0,1])
layout(location = 1) out vec4 gAlbedo;      // Albedo RGB
layout(location = 2) out vec4 gPBR;         // Roughness, Metallic, AO, unused

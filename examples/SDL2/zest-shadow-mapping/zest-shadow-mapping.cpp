...
/*
Basic Shadow Mapping Example
Recreated from Sascha Willems "Shadow mapping for directional light sources"
https://github.com/SaschaWillems/Vulkan/tree/master/examples/shadowmapping

This example demonstrates:
- Basic shadow mapping with a single depth pass for directional lights
- Depth bias to prevent shadow acne artifacts
- Optional PCF (Percentage Closer Filtering) for soft shadow edges
- Frame graph with transfer, shadow, scene, and ImGui passes
- Transient depth resources for shadow map and scene depth buffer
- Instanced mesh rendering with custom instance data
*/
...
void InitShadowMappingExample(ShadowMappingExample *app) {
	...
	//- view_buffer: camera matrices and light info for scene rendering
	//- offscreen_uniform_buffer: light's view-projection matrix for shadow pass
	...
	zest_AddVertexAttribute(app->mesh_pipeline, 0, 3, zest_format_r16g16_unorm, offsetof(zest_vertex_t, uv));
	zest_AddVertexAttribute(app->mesh_pipeline, 1, 4, zest_format_r32g32b32_sfloat, 0);
	zest_AddVertexAttribute(app->mesh_pipeline, 1, 5, zest_format_r8g8b8a8_unorm, offsetof(mesh_instance_t, color));
	zest_AddVertexAttribute(app->mesh_pipeline, 1, 6, zest_format_r32g32b32_sfloat, offsetof(mesh_instance_t, rotation));
	zest_AddVertexAttribute(app->mesh_pipeline, 1, 7, zest_format_r32g32b32_sfloat, offsetof(mesh_instance_t, scale));
	...
	//Shadow pass pipeline - depth-only rendering from light's perspective.
	//Uses depth bias to prevent shadow acne (self-shadowing artifacts).
	//No fragment shader needed since we only write depth.
	...
	zest_AddVertexAttribute(app->shadow_pipeline, 1, 1, zest_format_r32g32b32_sfloat, 0);
	zest_AddVertexAttribute(app->shadow_pipeline, 1, 2, zest_format_r8g8b8a8_unorm, offsetof(mesh_instance_t, color));
	zest_AddVertexAttribute(app->shadow_pipeline, 1, 3, zest_format_r32g32b32_sfloat, offsetof(mesh_instance_t, rotation));
	zest_AddVertexAttribute(app->shadow_pipeline, 1, 4, zest_format_r32g32b32_sfloat, offsetof(mesh_instance_t, scale));
	...
	zest_SetPipelineDepthBias(app->shadow_pipeline, true);  //Enable dynamic depth bias
	zest_SetPipelineDepthTest(app->shadow_pipeline, true, true);

	//Load GLTF scene and create mesh layer
	...
	app->scene_push.enable_pcf = 1;  //PCF = Percentage Closer Filtering for soft shadows

	zest_FreeMesh(vulkan_scene);
}

void UpdateLight(ShadowMappingExample *app) {
	app->light_position.x = cosf(zest_Radians(app->frame_timer * 360.0f)) * 40.0f;
	app->light_position.y = 50.0f + sinf(zest_Radians(app->frame_timer * 360.0f)) * 20.0f;
	app->light_position.z = 25.0f + sinf(zest_Radians(app->frame_timer * 360.0f)) * 5.0f;
}
...
void UpdateUniform3d(ShadowMappingExample *app) {
	...
	//Build the light's view-projection matrix for the shadow pass.
	//This transforms world-space positions into the light's clip space for depth comparison.
	...
	//Scene rendering uniforms - includes the light_space matrix for shadow sampling
	...
	view_ptr->projection.v[1].y *= -1.f;  //Vulkan Y-flip
	view_ptr->model = zest_M4(1.f);
	view_ptr->light_pos = { app->light_position.x, app->light_position.y, app->light_position.z, 1.f };
	view_ptr->light_space = offscreen_ptr->depth_mvp;  //For transforming to light space in fragment shader
	view_ptr->z_near = app->z_near;
	view_ptr->z_far = app->z_far;
}

//Transfer pass - upload instance data to GPU
...
//Scene pass - render with shadow sampling
...
void MainLoop(ShadowMappingExample *app) {
	...
	while (running) {
		...
		if (zest_BeginFrame(app->context)) {
			...
			//Frame graph caching - cache key changes when ImGui presence changes
			...
			if (!frame_graph) {
				...
				//Shadow map: 16-bit depth is sufficient and uses less memory
				...
				if (zest_BeginFrameGraph(app->context, "ImGui", &cache_key)) {
					...
					//Group swapchain and depth buffer for combined output
					...
					zest_BeginRenderPass("Mesh Layer Pass"); {
						zest_ConnectInput(mesh_layer_resource);
						zest_ConnectInput(shadow_buffer);  //Sample shadow map

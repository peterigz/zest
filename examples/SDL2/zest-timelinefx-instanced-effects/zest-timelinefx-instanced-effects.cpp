...
void RecordComputeSprites(zest_command_list command_buffer, void *user_data) {
	...
	zest_cmd_BindPipelineShaderResource(command_buffer, example.tfx_rendering.pipeline, example.tfx_rendering.shader_resource, context->current_fif);
	...
	zest_EndRecording(draw_routine->recorder, context->current_fif);
}
...
void UploadBuffers(ComputeExample *example) {
	...
	tfx_animation_manager_t *animation_manager = example->animation_manager_3d;
...
void PrepareComputeForEffectPlayback(ComputeExample *example) {
	...
	example->playback_shader = zest_CreateShaderFromBinaryFile("examples/assets/spv/sprite_data_playback.comp.spv", shaderc_compute_shader);
...
void PrepareComputeForBoundingBoxCalculation(ComputeExample *example) {
	//Register a new compute shader
	example->bounding_box_compute = zest__new_compute("Sprite data bb compute");
	example->bb_shader = zest_CreateShaderFromBinaryFile("examples/assets/spv/sprite_data_bounding_box.comp.spv", shaderc_compute_shader);
	...
	//Specify the shader that we want to use for the compute shader
	example->bb_compute_pipeline_3d = zest_AddComputeShader(&builder, example->bb_shader);
	//Specify the size of the push constant struct if we're using it
	zest_SetComputePushConstantSize(&builder, sizeof(zest_compute_push_constant_t));
	//Set the user data that can be passed to the callbacks
	zest_SetComputeUserData(&builder, example);
...
void SpriteComputeFunction(zest_compute compute) {
	...
	VkCommandBuffer command_buffer = zest_BeginComputeRecording(compute->recorder, context->current_fif);
	...
	zest_CopyBufferCB(zest_CurrentCommandBuffer(), example->offsets_staging_buffer[context->current_fif], example->offsets_buffer->buffer[context->current_fif], tfx_GetOffsetsSizeInBytes(example->animation_manager_3d), 1);
	zest_CopyBufferCB(zest_CurrentCommandBuffer(), example->animation_instances_staging_buffer[context->current_fif], example->animation_instances_buffer->buffer[context->current_fif], tfx_GetAnimationInstancesSizeInBytes(example->animation_manager_3d), 1);
	...
	zest_EndRecording(compute->recorder, context->current_fif);
}
...
void InitTimelineFXRenderResources(tfx_render_resources_t &render_resources, const char *library_path) {
	...
	render_resources.particle_texture = zest_CreateTexture("Particle Texture", zest_texture_storage_type_packed, zest_image_flag_use_filtering, zest_format_r8g8b8a8_unorm, shape_count);
	...
	render_resources.fragment_shader = zest_CreateShaderFromFile("examples/assets/shaders/timelinefxstatic.frag", "tfx_frag.spv", shaderc_fragment_shader, false, compiler, NULL, 0);
	render_resources.vertex_shader = zest_CreateShaderFromFile("examples/assets/shaders/timelinefx3dstatic.vert", "tfx_vertex3d.spv", shaderc_vertex_shader, false, compiler, NULL, 0);
	...
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(tfx_3d_instance_t, position)));	            // Location 0: Postion and stretch in w
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(tfx_3d_instance_t, rotations)));	            // Location 1: Rotations
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 2, VK_FORMAT_R8G8B8_SNORM, offsetof(tfx_3d_instance_t, alignment)));					// Location 2: Alignment
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 3, VK_FORMAT_R16G16B16A16_SSCALED, offsetof(tfx_3d_instance_t, size_handle)));		    // Location 3: Size and handle of the sprite
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 4, VK_FORMAT_R16G16_SSCALED, offsetof(tfx_3d_instance_t, intensity_gradient_map)));    // Location 4: 2 intensities for each color
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 5, VK_FORMAT_R8G8B8_UNORM, offsetof(tfx_3d_instance_t, curved_alpha_life)));          	// Location 5: Sharpness and mix lerp value
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 6, VK_FORMAT_R32_UINT, offsetof(tfx_3d_instance_t, indexes)));							// Location 6: texture indexes to sample the correct image and color ramp
	zest_AddVertexAttribute(&instance_create_info.attributeDescriptions, zest_CreateVertexInputDescription(0, 7, VK_FORMAT_R32_UINT, offsetof(tfx_3d_instance_t, captured_index)));   				// Location 7: index of the sprite in the previous buffer when double buffering
...
void UpdateTimelineFXImageData(tfx_render_resources_t &tfx_rendering, tfx_library library) {
	...
	zest_buffer staging_buffer = zest_CreateStagingBuffer(tfx_GetGPUShapesSizeInBytes(library->gpu_shapes), tfx_GetGPUShapesArray(library->gpu_shapes));
	zest_cmd_CopyBuffer(staging_buffer, zest_GetBufferFromDescriptorBuffer(tfx_rendering.image_data), tfx_GetGPUShapesSizeInBytes(library->gpu_shapes));
	zest_FreeBuffer(staging_buffer);
}
...
void InitExample(ComputeExample *example) {
	...
	example->floor_texture = zest_CreateTexture("Floor texture", zest_texture_storage_type_bank, zest_image_flag_use_filtering, zest_format_r8g8b8a8_unorm, 10);
	...
	example->library = tfx_LoadEffectLibrary("examples/assets/instanced_effects.tfx", ShapeLoader, GetUV, example);
	assert(example->library->error_flags == tfxErrorCode_success);		//Unable to load the effects library!
	...
	tfx_effect_emitter_t *effect1 = tfx_GetLibraryEffectPath(example->library, "Big Explosion");
	tfx_effect_emitter_t *effect2 = tfx_GetLibraryEffectPath(example->library, "Star Burst Flash");
	tfx_effect_emitter_t *effect3 = tfx_GetLibraryEffectPath(example->library, "EmissionSingleShot");
	tfx_effect_emitter_t *effect4 = tfx_GetLibraryEffectPath(example->library, "Firework");
	...
	tfx_particle_manager_info_t pm_info = tfx_CreateParticleManagerInfo(tfxParticleManagerSetup_3d_unordered);
	example->pm = tfx_CreateParticleManager(pm_info);
	//Set up the camera in the particle manager
	tfx_SetPMCamera(example->pm, &example->camera.front.x, &example->camera.position.x);
	...
	tfx_BuildLibraryGPUShapeData(example->library, example->gpu_image_data, GetUV);
	...
	tfx_AddSpriteData(example->animation_manager_3d, effect1, example->pm, camera_position_for_recording);
	tfx_AddSpriteData(example->animation_manager_3d, effect3, example->pm, camera_position_for_recording);
	tfx_AddSpriteData(example->animation_manager_3d, effect4, example->pm, camera_position_for_recording);
	...
	*/
	...
	3)	Create the 2 buffers that need writing to from the CPU, this is the offset's buffer containing the index offset for each instance into the sprite data and
		the actual effect instances data containing position, scale and so on.
	*/
...
void CalculateBoundingBoxes(ComputeExample *example, tfx_animation_manager animation_manager, tfx_effect_emitter_t *effect) {
	...
	tfx_sprite_data_settings_t &anim = *tfx_GetEffectSpriteDataSettings(example->library, effect);
	...
	for (int i = 0; i != anim.frames_after_compression; ++i) {
		...
		zest_UpdateComputeDescriptors(example->bounding_box_compute, context->current_fif);
		//Update the offset and instance buffers either.
		memcpy(example->offsets_staging_buffer[context->current_fif]->data, animation_manager->offsets.data, tfx_GetOffsetsSizeInBytes(animation_manager));
		memcpy(example->animation_instances_staging_buffer[context->current_fif]->data, animation_manager->render_queue.data, tfx_GetAnimationInstancesSizeInBytes(animation_manager));
		...
		zest_cmd_CopyBuffer(example->bounding_boxes->buffer[0], bounding_boxes, sizeof(tfx_bounding_box_t) * frame_group_count);
		...
		metrics.frame_meta[i].radius = tfx__length_vec3(&half_extents);
...
void Update(zest_microsecs elapsed, void *data) {
	...
	int r = tfx_RandomRangeFromToInt(&example->pm->random, 0, 3);
	if (!example->left_mouse_clicked && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
		...
		if (anim_id != tfxINVALID) {
			...
			tfx_vec3_t position = tfx_vec3_t(tfx_RandomRangeFromTo(&example->pm->random, -10.f, 10.f), tfx_RandomRangeFromTo(&example->pm->random, 8.f, 15.f), tfx_RandomRangeFromTo(&example->pm->random, -10.f, 10.f));
			tfx_SetAnimationPosition3d(example->animation_manager_3d, anim_id, &position.x);
			tfx_SetAnimationScale(example->animation_manager_3d, anim_id, tfx_RandomRangeFromTo(&example->pm->random, 0.5f, 1.5f));
		}
	...
	if (example->trigger_effect >= ZEST_MILLISECONDS_IN_MICROSECONDS(25)) {
		...
		if (anim_id != tfxINVALID) {
			tfx_vec3_t position = tfx_vec3_t(tfx_RandomRangeFromTo(&example->pm->random, -10.f, 10.f), tfx_RandomRangeFromTo(&example->pm->random, 8.f, 15.f), tfx_RandomRangeFromTo(&example->pm->random, -10.f, 10.f));
			tfx_SetAnimationPosition3d(example->animation_manager_3d, anim_id, &position.x);
			tfx_SetAnimationScale(example->animation_manager_3d, anim_id, tfx_RandomRangeFromTo(&example->pm->random, 0.75f, 1.5f));
		}

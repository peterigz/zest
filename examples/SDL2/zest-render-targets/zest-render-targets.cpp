...
void InitExample(render_target_app_t *example) {
	...
	//Load or generate an MSDF font for rendering text
	...
	//Create compute pipelines for the bloom down/up sampling passes
...
//Final composite pass - draws a fullscreen triangle that samples both the base (downsampled)
//and bloom (upsampled) images, blending them together with additive blending.
void zest_DrawRenderTarget(zest_command_list command_list, void *user_data) {
	...
	//Get bindless descriptor indices for both render targets to pass to the shader
...
//Progressively downsample the image through mip levels.
//Each dispatch reads from the previous mip and writes to the next smaller mip,
//with barriers between each level to ensure the write completes before the next read.
void zest_DownsampleCompute(zest_command_list command_list, void* user_data) {
	...
	//Each mip level gets its own bindless index for reading (sampled) and writing (storage)
...
//Progressively upsample from the smallest mip back to full resolution.
//Starts by copying the smallest downsampled mip, then blends each level back up,
//combining with the corresponding downsample mip for detail preservation.
...
void Mainloop(render_target_app_t *example) {
	...
	while (running) {
		...
		if (zest_BeginFrame(example->context)) {
			...
			//Mouse X controls bloom threshold, mouse Y controls knee (soft transition width)
			...
			//HDR render target with 7 mip levels for the bloom down/up sample chain
			...
			if (!frame_graph) {
				if (zest_BeginFrameGraph(example->context, "Bloom Example Render Graph", &cache_key)) {
					...
					//Transient resources - created and destroyed within the frame
					...
					//---------------------------------Draw Base Pass---------------------------------------------------
					//Render font text into the HDR downsampler target
					...
					//---------------------------------Render Pass------------------------------------------------------
					//Composite base + bloom onto the swapchain with additive blending
		...
		//Resize layers and update font projection when the window is resized

...
void main()
{
	...
	// Calculate direct lighting from point lights
	vec3 Lo = vec3(0.0);
	for(int i = 0; i < ubo_lights[material.lights_index].lights.length(); i++) {
		vec3 L = normalize(ubo_lights[material.lights_index].lights[i].xyz - worldPos);
		Lo += specularContribution(L, V, N, F0, albedo, metallic, roughness);
	}

	// Flip Y for cubemap sampling to account for different coordinate system conventions

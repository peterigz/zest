...
ZEST_API zest_draw_batch_handle zest_CreateFontLayer(zest_context context, const char *name, zest_uint max_characters);
...
ZEST_API void zest_SetMSDFFontDrawing(zest_draw_batch layer, zest_msdf_font_t *font, zest_font_resources_t *font_resources);
ZEST_API float zest_DrawMSDFText(zest_draw_batch layer, float x, float y, float handle_x, float handle_y, float size, float letter_spacing, const char* format, ...);
...
ZEST_API zest_image_collection_t zest_CreateImageCollection(zest_context context, zest_format format, zest_uint max_images, zest_uint array_count, zest_image_collection_flags flags);
...
zest_image_collection_t zest__load_ktx(zest_context context, const char *file_path) {
...
zest_image_handle zest_LoadCubemap(zest_context context, const char *name, const char *file_name) {
	...
	zest_StageData(image_collection.bitmap_array.data, staging_buffer, bitmap_array->total_mem_size);
	...
	zest_imm_TransitionImage(context, image, zest_image_layout_transfer_dst_optimal, 0, mip_levels, 0, 6);
	zest_imm_CopyBufferRegionsToImage(context, image_collection.buffer_copy_regions, bitmap_array->size_of_array, staging_buffer, image);
    zest_imm_TransitionImage(context, image, zest_image_layout_shader_read_only_optimal, 0, mip_levels, 0, 6);
...
zest_draw_batch_handle zest_CreateFontLayer(zest_context context, const char *name, zest_uint max_characters) {
	ZEST_ASSERT(name, "Specify a name for the font layer");
	zest_draw_batch_handle layer = zest_CreateInstanceLayer(context, name, sizeof(zest_font_instance_t), max_characters);
	return layer;
}
...
void zest_SetMSDFFontDrawing(zest_draw_batch layer, zest_msdf_font_t *font, zest_font_resources_t *font_resources) {
...
float zest__draw_msdf_text(zest_draw_batch layer, const char* text, float x, float y, float handle_x, float handle_y, float size, float letter_spacing) {
	ZEST_ASSERT_HANDLE(layer);	//Not a valid layer handle
...
float zest_DrawMSDFText(zest_draw_batch layer, float x, float y, float handle_x, float handle_y, float size, float letter_spacing, const char* format, ...) {
	ZEST_ASSERT_HANDLE(layer);	//Not a valid layer handle
    char buffer[1024];
    va_list args;
...
zest_image_collection_t zest_CreateImageCollection(zest_context context, zest_format format, zest_uint max_images, zest_uint bitmap_array_count, zest_image_collection_flags flags) {
...
zest_image_handle zest_CreateImageAtlas(zest_context context, zest_image_collection_t *atlas, zest_uint layer_width, zest_uint layer_height, zest_image_flags flags) {
	...
	zest_BeginImmediateCommandBuffer(context);
    zest_imm_TransitionImage(context, image, zest_image_layout_transfer_dst_optimal, 0, mip_levels, 0, layer_count);
	zest_imm_CopyBufferRegionsToImage(context, atlas->buffer_copy_regions, atlas->bitmap_array.size_of_array, staging_buffer, image);
	if (mip_levels > 1) {
		zest_imm_GenerateMipMaps(context, image);
	} else {
		zest_imm_TransitionImage(context, image, zest_image_layout_shader_read_only_optimal, 0, mip_levels, 0, layer_count);
	}
	zest_EndImmediateCommandBuffer(context);

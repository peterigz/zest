...
void zest_implsdl2_AddPlatformExtensionsCallback(void) {
	zest_uint count;
	SDL_Vulkan_GetInstanceExtensions((SDL_Window*)ZestApp->window->window_handle, &count, nullptr);
	...
	SDL_Vulkan_GetInstanceExtensions((SDL_Window*)ZestApp->window->window_handle, &count, sdl_extensions);
...
void zest_implsdl2_GetWindowSizeCallback(void *user_data, int *fb_width, int *fb_height, int *window_width, int *window_height) {
	SDL_GL_GetDrawableSize((SDL_Window*)ZestApp->window->window_handle, fb_width, fb_height);
	SDL_GetWindowSize((SDL_Window*)ZestApp->window->window_handle, window_width, window_height);
}

void zest_implsdl2_DestroyWindowCallback(void *user_data) {
	SDL_DestroyWindow((SDL_Window*)ZestApp->window->window_handle);
}

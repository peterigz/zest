...
zest_bool zest__vk_initialise_swapchain(zest_swapchain swapchain, zest_window window) {
    ...
    zest_swapchain_support_details_t swapchain_support_details = zest__vk_query_swapchain_support(swapchain->context, ZestDevice->backend->physical_device);
...
static VKAPI_ATTR VkBool32 VKAPI_CALL zest__vk_debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData, void *pUserData) {
    ...
    if (ZEST__FLAGGED(ZestApp->create_info.flags, zest_init_flag_log_validation_errors_to_memory)) {
        if (!zest_map_valid_key(ZestDevice->validation_errors, (zest_key)pCallbackData->messageIdNumber)) {
            ...
            zest_map_insert_key(context, ZestDevice->validation_errors, (zest_key)pCallbackData->messageIdNumber, error_message);
        }
    }
...
zest_bool zest__vk_create_logical_device(zest_context context) {
    ...
    zest_vec_resize(context, ZestDevice->backend->queue_families, queue_family_count);
    ...
    // Graphics queue
    {
		...
		zest_map_insert_key(context, ZestDevice->queue_names, indices.graphics_family_index, "Graphics Queue");
    }
    ...
    // Dedicated compute queue
    {
        ...
        if (indices.compute_family_index != indices.graphics_family_index)
        {
            ...
            queue_create_count++;
			zest_map_insert_key(context, ZestDevice->queue_names, indices.compute_family_index, "Compute Queue");
        }
    ...
    //Dedicated transfer queue
    {
        ...
        if (indices.transfer_family_index != indices.graphics_family_index && indices.transfer_family_index != indices.compute_family_index)
        {
            ...
            queue_create_count++;
			zest_map_insert_key(context, ZestDevice->queue_names, indices.transfer_family_index, "Transfer Queue");
        }
        ZEST_APPEND_LOG(ZestDevice->log_path.str, "Transfer queue index set to: %i", indices.transfer_family_index);
    }
    
	zest_map_insert_key(context, ZestDevice->queue_names, VK_QUEUE_FAMILY_IGNORED, "Ignored");
    ...
    ZestDevice->graphics_queue.backend = zest__vk_new_queue_backend(context);
    ZestDevice->compute_queue.backend = zest__vk_new_queue_backend(context);
    ZestDevice->transfer_queue.backend = zest__vk_new_queue_backend(context);
    ...
    zest_vec_push(context, ZestDevice->queues, &ZestDevice->graphics_queue);
    ...
    if (ZestDevice->graphics_queue.backend->vk_queue != ZestDevice->compute_queue.backend->vk_queue) {
		zest_vec_push(context, ZestDevice->queues, &ZestDevice->compute_queue);
    ...
    if (ZestDevice->graphics_queue.backend->vk_queue != ZestDevice->transfer_queue.backend->vk_queue) {
		zest_vec_push(context, ZestDevice->queues, &ZestDevice->transfer_queue);
...
void *zest__vk_new_submission_batch_backend(zest_context context) {
    zest_submission_batch_backend backend = zloc_LinearAllocation(context->renderer->frame_graph_allocator[ZEST_FIF], sizeof(zest_submission_batch_backend_t));
...
void *zest__vk_new_frame_graph_context_backend(void) {
    zest_command_list_backend_t *backend_context = zloc_LinearAllocation( ZestRenderer->frame_graph_allocator[ZEST_FIF], sizeof(zest_command_list_backend_t) );
...
zest_bool zest__vk_finish_compute(zest_compute_builder_t *builder, zest_compute compute) {
    ...
    zest_vec_foreach(i, compute->shaders) {
        ...
        context->renderer->backend->last_result = vkCreateComputePipelines(ZestDevice->backend->logical_device, VK_NULL_HANDLE, 1, &compute_pipeline_create_info, &ZestDevice->backend->allocation_callbacks, &pipeline);
...
void zest__vk_cleanup_device_backend(zest_context context) {
    ...
    zest_vec_free(context, ZestDevice->backend->queue_families);
    ZEST__FREE(context, ZestDevice->backend);
}
...
zest_bool zest__vk_set_next_command_buffer(zest_command_list command_list, zest_queue queue) {
    if (zest_vec_size(queue->backend->command_buffers[ZEST_FIF]) > queue->next_buffer) {
        ...
        zest_vec_push(command_list->context, queue->backend->command_buffers[ZEST_FIF], new_command_buffer);
        queue->next_buffer++;
...
zest_bool zest__vk_begin_render_pass(const zest_command_list command_list, zest_execution_details_t *exe_details) {
	...
	ZestDevice->backend->pfn_vkCmdBeginRendering(command_list->backend->command_buffer, &rendering_info);	
	return ZEST_TRUE;
}

void zest__vk_end_render_pass(const zest_command_list command_list) {
	//vkCmdEndRenderPass(command_list->backend->command_buffer);
	ZestDevice->backend->pfn_vkCmdEndRendering(command_list->backend->command_buffer);
}

...
ZEST_PRIVATE zest_u64 zest__vk_get_semaphore_counter_value(zest_execution_timeline timeline);
...
ZEST_PRIVATE void *zest__vk_new_frame_graph_context_backend(zloc_linear_allocator_t *allocator);
...
ZEST_PRIVATE void *zest__vk_new_submission_batch_backend(zloc_linear_allocator_t *allocator);
...
ZEST_PRIVATE zest_bool zest__vk_end_single_time_commands(zest_queue queue);
...
void zest__vk_initialise_platform_callbacks(zest_platform_t *platform) {
	...
	platform->get_semaphore_counter_value				    = zest__vk_get_semaphore_counter_value;
...
ZEST_PRIVATE inline VkResult zest__vk_create_temporary_image(zest_device device, zest_uint width, zest_uint height, zest_uint mipLevels, VkSampleCountFlagBits sample_count, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage* image, VkDeviceMemory* memory) {
	...
	ZEST_SET_MEMORY_CONTEXT(device, zest_platform_context, zest_command_allocate_memory_image);
    ZEST_VK_ASSERT_RESULT(device, vkAllocateMemory(device->backend->logical_device, &alloc_info, &device->backend->allocation_callbacks, memory));
...
ZEST_PRIVATE inline VkFence zest__vk_create_fence(zest_device device) {
    VkFenceCreateInfo fence_info = ZEST__ZERO_INIT(VkFenceCreateInfo);
    fence_info.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
    fence_info.flags = 0;
    VkFence fence;
	ZEST_SET_MEMORY_CONTEXT(device, zest_platform_device, zest_command_fence);
    vkCreateFence(device->backend->logical_device, &fence_info, &device->backend->allocation_callbacks, &fence);
    return fence;
}
...
void *zest__vk_new_queue_backend(zest_device device, zest_uint family_index) {
    ...
    *backend = ZEST__ZERO_INIT(zest_queue_backend_t);
	...
	ZEST_SET_MEMORY_CONTEXT(device, zest_platform_device, zest_command_command_pool);
...
void *zest__vk_new_submission_batch_backend(zloc_linear_allocator_t *allocator) {
    zest_submission_batch_backend backend = (zest_submission_batch_backend)zloc_LinearAllocation(allocator, sizeof(zest_submission_batch_backend_t));
...
void *zest__vk_new_frame_graph_context_backend(zloc_linear_allocator_t *allocator) {
    zest_command_list_backend_t *backend_context = (zest_command_list_backend_t*)zloc_LinearAllocation(allocator, sizeof(zest_command_list_backend_t) );
...
zest_u64 zest__vk_get_semaphore_counter_value(zest_execution_timeline timeline) {
	zest_context context = timeline->context;
	zest_u64 value = 0;
	vkGetSemaphoreCounterValue(context->device->backend->logical_device, timeline->backend->semaphore, &value);
	return value;
}
// -- End Fences
...
zest_bool zest__vk_create_image_memory_pool(zest_device device, zest_context context, zest_size size_in_bytes, zest_buffer_info_t *buffer_info, zest_device_memory_pool pool) {
    ...
    alloc_info.memoryTypeIndex = zest__vk_find_memory_type(device, buffer_info->memory_type_bits, buffer_info->property_flags);
    ZEST_ASSERT(alloc_info.memoryTypeIndex != ZEST_INVALID);
...
zest_bool zest__vk_create_device_memory(zest_device device, zest_size size_in_bytes, zest_buffer_info_t *buffer_info, zest_device_memory memory) {
    VkMemoryAllocateInfo alloc_info = ZEST__ZERO_INIT(VkMemoryAllocateInfo);
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.allocationSize = size_in_bytes;
    alloc_info.memoryTypeIndex = zest__vk_find_memory_type(device, buffer_info->memory_type_bits, buffer_info->property_flags);
	...
	ZEST_SET_MEMORY_CONTEXT(device, zest_platform_context, zest_command_allocate_memory_pool);
    ZEST_RETURN_FALSE_ON_FAIL(device, vkAllocateMemory(device->backend->logical_device, &alloc_info, &device->backend->allocation_callbacks, &memory->backend->memory));
...
void zest__vk_cleanup_memory_backend(zest_device_memory memory) {
    ...
    if (memory->backend->memory) {
        vkFreeMemory(device->backend->logical_device, memory->backend->memory, &device->backend->allocation_callbacks);
    }
...
zest_bool zest__vk_create_image(zest_context context, zest_image image, zest_uint layer_count, zest_sample_count_flags num_samples, zest_image_flags flags) {
	...
	ZEST_PRINT("%p", image->backend->vk_image);
    VkMemoryRequirements memory_requirements;
	...
	if (ZEST__FLAGGED(flags, zest_image_flag_transient)) {
		...
		zest_device_memory memory = zest__create_device_memory(device, memory_requirements.size, &buffer_info);
...
zest_queue zest__vk_begin_single_time_commands(zest_device device, zest_device_queue_type target_queue) {
	zest_queue queue = 0;
	switch (target_queue) {
...
zest_bool zest__vk_end_single_time_commands(zest_queue queue) {
    ...
    submit_info.pCommandBuffers = &queue->backend->command_buffer;

    VkFence fence = zest__vk_create_fence(device);
	ZEST_CLEANUP_ON_FAIL(device, vkQueueSubmit(queue->backend->vk_queue, 1, &submit_info, fence));
    vkWaitForFences(device->backend->logical_device, 1, &fence, VK_TRUE, UINT64_MAX);
    vkDestroyFence(device->backend->logical_device, fence, &device->backend->allocation_callbacks);
...
void zest__vk_carry_over_semaphores(zest_frame_graph frame_graph, zest_wave_submission_t *wave_submission, zest_execution_backend backend) {
    ...
    zloc_linear_allocator_t *allocator = &frame_graph->allocator_entry->allocator;
	for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
...
zest_bool zest__vk_submit_frame_graph_batch(zest_frame_graph frame_graph, zest_execution_backend backend, zest_submission_batch_t *batch, zest_map_queue_value *queues) {
    ...
    if (submission_index == zest_vec_size(frame_graph->submissions) - 1) {
		...
		//Signal the internal allocator timeline for safe memory reuse
		if (frame_graph->allocator_entry) {
			zest_frame_graph_allocator_t *entry = frame_graph->allocator_entry;
			zest_execution_timeline timeline = zest_GetExecutionTimeline(entry->timeline);
			entry->signal_value = timeline->current_value + 1;
			timeline->current_value = entry->signal_value;
			entry->in_use = ZEST_FALSE;  //Mark as no longer in use
			zest_vec_linear_push(allocator, signal_semaphores, timeline->backend->semaphore);
			zest_vec_linear_push(allocator, signal_values, entry->signal_value);
			zest_vec_linear_push(allocator, signal_stages, VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT);
		}
    } else {
...
void zest__vk_add_image_barrier(zest_resource_node resource, zest_execution_barriers_t *barriers, zest_bool acquire, 
	...
	zloc_linear_allocator_t *allocator = &resource->frame_graph->allocator_entry->allocator;
    if (acquire) {
        zest_vec_linear_push(allocator, barriers->backend->acquire_image_barriers, image_barrier);
        zest_vec_linear_push(allocator, barriers->acquire_image_barrier_nodes, resource);
    } else {
        zest_vec_linear_push(allocator, barriers->backend->release_image_barriers, image_barrier);
...
void zest__vk_add_memory_buffer_barrier(zest_resource_node resource, zest_execution_barriers_t *barriers, zest_bool acquire, zest_access_flags src_access, zest_access_flags dst_access,
	...
	zloc_linear_allocator_t *allocator = &resource->frame_graph->allocator_entry->allocator;
    if (acquire) {
        zest_vec_linear_push(allocator, barriers->backend->acquire_buffer_barriers, buffer_barrier);
        zest_vec_linear_push(allocator, barriers->acquire_buffer_barrier_nodes, resource);
    } else {
        zest_vec_linear_push(allocator, barriers->backend->release_buffer_barriers, buffer_barrier);
...
zest_bool zest__vk_upload_buffer(const zest_command_list command_list, zest_buffer_uploader_t *uploader) {
    ...
    zest_vec_foreach(i, uploader->buffer_copies) {
        ...
        zest_vec_linear_push(&command_list->frame_graph->allocator_entry->allocator, buffer_copies, buffer_copy);
    }
...
void zest__vk_bind_descriptor_sets(const zest_command_list command_list, zest_pipeline_bind_point bind_point, zest_pipeline_layout layout, zest_descriptor_set *descriptor_sets, zest_uint set_count, zest_uint first_set) {
	zest_context context = command_list->context;
    zloc_linear_allocator_t *allocator = &command_list->frame_graph->allocator_entry->allocator;
...
zest_bool zest_CopyBitmapToImage(zest_context context, void *bitmap, zest_size image_size, zest_image dst_image, zest_uint width, zest_uint height) {
	...
	zest_device device = context->device;
	zest_queue queue = zest__vk_begin_single_time_commands(device, zest_queue_graphics);
	if (!queue) {
		return ZEST_FALSE;
	}
    ...
    ZEST_CLEANUP_ON_FALSE(zest__vk_end_single_time_commands(queue));

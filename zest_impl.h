...
zest_buffer_linear_allocator zest__create_linear_buffer_allocator(zest_context context, zest_buffer_info_t *buffer_info, zest_size size) {
	...
	allocator->backend = (zest_buffer_linear_allocator_backend)context->device->platform->create_buffer_linear_allocator_backend(context, size, buffer_info);
	allocator->buffer_size = size;
	allocator->current_offset = 0;
	return allocator;
}

zest_bool zest__add_gpu_memory_pool(zest_context context, zest_buffer_allocator allocator, zest_size minimum_size, zest_device_memory_pool *memory_pool) {
...
zest_buffer zest_CreateBuffer(zest_context context, zest_size size, zest_buffer_info_t* buffer_info) {
    ...
    if (!zest_map_valid_key(device->buffer_allocators, key)) {
        ...
        if (zest__add_gpu_memory_pool(context, buffer_allocator, size, &buffer_pool) != ZEST_TRUE) {
			return 0;
        }
    }
...
zest_bool zest_imm_TransitionImage(zest_context context, zest_image_handle image_handle, zest_image_layout new_layout, zest_uint base_mip_index, zest_uint mip_levels, zest_uint base_array_index, zest_uint layer_count) {
...
zest_bool zest_imm_CopyBufferRegionsToImage(zest_context context, zest_buffer_image_copy_t *regions, zest_uint regions_count, zest_buffer staging_buffer, zest_image_handle image_handle) {
...
zest_bool zest_imm_GenerateMipMaps(zest_context context, zest_image_handle image_handle) {
    zest_image image = (zest_image)zest__get_store_resource_checked(image_handle.store, image_handle.value);
...
zest_bool zest_GrowBuffer(zest_buffer* buffer, zest_size unit_size, zest_size minimum_bytes) {
    ...
    if (new_buffer) {
        *buffer = new_buffer;
        ...
        zest_device_memory_pool buffer_pool = 0;
        if (zest__add_gpu_memory_pool(context, buffer_allocator, new_size, &buffer_pool) != ZEST_TRUE) {
            return ZEST_FALSE;
        } else {
...
void zest_AddCopyCommand(zest_buffer_uploader_t *uploader, zest_buffer source_buffer, zest_buffer target_buffer, zest_size size) {
	...
	zest_context context = source_buffer->memory_pool->context;
...
zest_set_layout zest__new_descriptor_set_layout(zest_context context, const char *name) {
    zest_set_layout descriptor_layout = (zest_set_layout)ZEST__NEW(context->allocator, zest_set_layout);
    *descriptor_layout = ZEST__ZERO_INIT(zest_set_layout_t);
    descriptor_layout->backend = (zest_set_layout_backend)context->device->platform->new_set_layout_backend(context->allocator);
    ...
    zest_SetText(context->allocator, &descriptor_layout->name, name);
...
void zest__release_bindless_index(zest_set_layout layout, zest_uint binding_number, zest_uint index_to_release) {
    ...
    zest_vec_push(layout->context->allocator, manager->free_indices, index_to_release);
}

void zest__cleanup_set_layout(zest_set_layout layout) {
	zest_context context = layout->context;
    zest_FreeText(context->allocator, &layout->name);
	zest_vec_foreach(i, layout->descriptor_indexes) {
		zest_vec_free(context->allocator, layout->descriptor_indexes[i].free_indices);
	}
...
zest_descriptor_pool zest__create_descriptor_pool(zest_context context, zest_uint max_sets) {
    ...
    zest_descriptor_pool pool = (zest_descriptor_pool)ZEST__NEW(context->allocator, zest_descriptor_pool);
    *pool = blank;
...
void zest_WaitForIdleDevice(zest_device device) { device->platform->wait_for_idle_device(device); }
...
void zest__initialise_store(zloc_allocator *allocator, void *origin, zest_resource_store_t *store, zest_uint struct_size) {
	...
	zest__initialise_bucket_array(allocator, &store->data, struct_size, 8);
    store->alignment = 16;
	store->origin = origin;
}
...
zest_frame_graph zest__compile_frame_graph() {
    ...
    if (zloc__count_bits(last_execution_wave->queue_bits) > 1) {
		...
		ZEST_FRAME_GRAPH_OOM(frame_graph, pass_node);
...
zest_uint zest__acquire_bindless_storage_buffer_index(zest_buffer buffer, zest_set_layout layout, zest_descriptor_set set, zest_uint target_binding_number) {
    ...
    if (array_index == ZEST_INVALID) {
        //Ran out of space in the descriptor pool
        ZEST__REPORT(layout->context, zest_report_bindless_indexes, "Ran out of space in the descriptor pool when trying to acquire an index for image, binding number %i.", binding_number);
        return ZEST_INVALID;
    }
	zest_context context = buffer->memory_pool->context;
...
zest_uint *zest_AcquireImageMipIndexes(zest_context context, zest_image_handle image_handle, zest_image_view_array_handle view_array_handle, zest_binding_number_type binding_number, zest_descriptor_type descriptor_type) {
	ZEST_ASSERT(image_handle.store->data.allocator == view_array_handle.store->data.allocator);	//image and view arrays must have the same context!
    zest_image image = (zest_image)zest__get_store_resource_checked(image_handle.store, image_handle.value);
    ...
    for (int mip_index = 0; mip_index != view_array->count; ++mip_index) {
		...
		if (bindless_index == ZEST_INVALID) {
			//Ran out of space in the descriptor pool
			ZEST__REPORT(context, zest_report_bindless_indexes, "Ran out of space in the descriptor pool when trying to acquire an index for an image mip index, binding number %i.", binding_number);
			zest_vec_foreach(i, mip_collection->mip_indexes[binding_number]) {
				zest_ReleaseBindlessIndex(context, mip_collection->mip_indexes[binding_number][i], binding_number);
				zest_vec_free(context->allocator, mip_collection->mip_indexes[binding_number]);
			}
			return NULL;
		}
...
void zest_ReleaseStorageBufferIndex(zest_context context, zest_uint array_index) {
    zest__release_bindless_index(context->bindless_set_layout, zest_storage_buffer_binding, array_index);
}
...
zest_resource_node zest__add_transient_image_resource(const char *name, const zest_image_info_t *description, zest_bool assign_bindless, zest_bool image_view_binding_only) {
	...
	ZEST_FRAME_GRAPH_OOM(frame_graph, node);
...
zest_resource_node zest_AddTransientLayerResource(const char *name, const zest_layer layer, zest_bool prev_fif) {
    ...
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_manual_fif)) {
		...
		ZEST_FRAME_GRAPH_OOM(frame_graph, resource);
...
zest_resource_node zest_ImportSwapchainResource() {
	...
	ZEST_FRAME_GRAPH_OOM(frame_graph, frame_graph->swapchain_resource);
...
zest_resource_node zest__add_frame_graph_resource(zest_resource_node resource) {
    ...
    *node = *resource;
	...
	ZEST_FRAME_GRAPH_OOM(frame_graph, frame_graph->resource_versions.data);
    return node;
}
...
zest_pass_node zest_BeginGraphicBlankScreen(const char *name) {
	...
	ZEST_FRAME_GRAPH_OOM(zest__frame_graph_builder->frame_graph, pass);
...
zest_uint zest_GetTransientSampledImageBindlessIndex(const zest_command_list command_list, zest_resource_node resource, zest_binding_number_type binding_number) {
    ...
    if (bindless_index == ZEST_INVALID) {
        //Ran out of space in the descriptor pool
        ZEST__REPORT(context, zest_report_bindless_indexes, "Ran out of space in the descriptor pool when trying to acquire an index for a sampled image, binding number %i.", binding_number);
        return ZEST_INVALID;
    }
...
zest_image_handle zest_CreateImage(zest_context context, zest_image_info_t *create_info) {
    ...
    if (ZEST__FLAGGED(image->info.flags, zest_image_flag_storage)) {
        ...
        zest_imm_TransitionImage(context, handle, zest_image_layout_general, 0, ZEST__ALL_MIPS, 0, ZEST__ALL_LAYERS);
...
void zest_BindAtlasRegionToImage(zest_atlas_region_t *region, zest_uint sampler_index, zest_image_handle image_handle, zest_binding_number_type binding_number) {
	region->image_index = zest_ImageDescriptorIndex(image_handle, binding_number);
...
void zest_UploadInstanceLayerData(const zest_command_list command_list, void *user_data) {
    ...
    if (ZEST_VALID_HANDLE(layer)) {  //You must pass in the zest_layer in the user data
        ...
        if (memory_in_use && device_buffer) {
            zest_AddCopyCommand(&instance_upload, staging_buffer, device_buffer, memory_in_use);
        } else {
            return;
        }
...
void zest_UploadLayerStagingData(zest_layer layer, const zest_command_list command_list) {
    ...
    if (ZEST_VALID_HANDLE(layer)) {  //You must pass in the zest_layer in the user data
		...
		if (layer->memory_refs[layer->fif].vertex_memory_in_use) {
			zest_AddCopyCommand(&instance_upload, staging_buffer, device_buffer, layer->memory_refs[layer->fif].vertex_memory_in_use);
        } else {
            return;
        }

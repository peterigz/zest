...
void zest__make_standard_descriptor_layouts() {
    zest_AddDescriptorLayout("uniform buffer", zest_CreateDescriptorSetLayout(1, 0, 0));
    zest_AddDescriptorLayout("1 sampler", zest_CreateDescriptorSetLayout(0, 0, 1));
    zest_AddDescriptorLayout("Standard 1 uniform 1 sampler", zest_CreateDescriptorSetLayout(1, 0, 1));
    zest_AddDescriptorLayout("Render target layout", zest_CreateDescriptorSetLayout(0, 0, 1));
...
void zest_UpdateShaderResources(zest_shader_resources resources, zest_descriptor_set descriptor_set, zest_uint index) {
	ZEST_ASSERT(index < zest_vec_size(resources->sets[index]));    //Must be a valid index for the descriptor set in the resources that you want to update.
...
zest_shader_resources zest_CombineUniformAndTextureSampler(zest_descriptor_set descriptor_set, zest_texture texture) {
    ZEST_ASSERT(ZEST__FLAGGED(texture->flags, zest_texture_flag_textures_ready));   //Texture must have been processed an descriptor set created
...
void zest__add_pipeline_descriptor_write(zest_pipeline pipeline, zest_uint layout_index, VkWriteDescriptorSet set, zest_index fif) {
    zest_vec_push(pipeline->descriptor_writes[layout_index].writes[fif], set);
}

void zest__free_pipeline_descriptor_writes(zest_pipeline pipeline) {
    for (ZEST_EACH_FIF_i) {
        for (zest_foreach_j(pipeline->descriptor_writes)) {
            zest_vec_free(pipeline->descriptor_writes[j].writes[i]);
        }
    }
...
void zest_MakePipelineDescriptorWrites(zest_pipeline pipeline) {
    ...
    for (ZEST_EACH_FIF_i) {
        zest_vec_resize(pipeline->descriptor_set[i], zest_vec_size(pipeline->descriptor_layouts));
    }
    for (zest_vec_foreach(layout_index, pipeline->descriptor_layouts)) {
        zest_pipeline_descriptor_writes_t writes = { 0 };
        zest_vec_push(pipeline->descriptor_writes, writes);
        for (ZEST_EACH_FIF_i) {
            zest_AllocateDescriptorSets(ZestRenderer->descriptor_pool, &pipeline->descriptor_layouts[layout_index], 1, &pipeline->descriptor_set[i][layout_index]);
            if (pipeline->uniforms) {
                if (!pipeline->uniform_buffer) {
                    zest__add_pipeline_descriptor_write(pipeline, layout_index, zest_CreateBufferDescriptorWriteWithType(pipeline->descriptor_set[i][layout_index], zest_GetUniformBufferInfo("Standard 2d Uniform Buffer", i), 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER), i);
                }
                else {
                    zest__add_pipeline_descriptor_write(pipeline, layout_index, zest_CreateBufferDescriptorWriteWithType(pipeline->descriptor_set[i][layout_index], &pipeline->uniform_buffer->descriptor_info[i], 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER), i);
                }
                binding++;
            }
        }
        for (ZEST_EACH_FIF_i) {
            zest_UpdateDescriptorSet(pipeline->descriptor_writes[layout_index].writes[i]);
        }
    }
    if (!pipeline->shader_resources) {
        zest_AddDescriptorSetToResources(pipeline->shader_resources, pipeline->descriptor_set);
    }
}
...
void zest_BindPipeline(zest_pipeline pipeline, VkDescriptorSet *descriptor_set, zest_uint set_count) {
...
void zest_BindPipelineCB(VkCommandBuffer command_buffer, zest_pipeline_t* pipeline, VkDescriptorSet *descriptor_set, zest_uint set_count) {
...
void zest_BuildPipeline(zest_pipeline pipeline) {
    ...
    if (zest_map_valid_name(ZestRenderer->shaders, pipeline->pipeline_template.vertShaderFile.str)) {
        ...
        if (!vert_shader->spv) {
			zest_bool status = zest_CompileShader(vert_shader);
			ZEST_ASSERT(status);    //Failed to compile shader, check the log file for the error.
        }
    ...
    if (zest_map_valid_name(ZestRenderer->shaders, pipeline->pipeline_template.fragShaderFile.str)) {
        ...
        if (!frag_shader->spv) {
			zest_bool status = zest_CompileShader(frag_shader);
			ZEST_ASSERT(status);    //Failed to compile shader, check the log file for the error.
        }
...
void zest_MakePipelineTemplate(zest_pipeline pipeline, VkRenderPass render_pass, zest_pipeline_template_create_info_t* create_info) {
    ...
    for (zest_vec_foreach(i, create_info->descriptorSetLayouts)) {
...
shaderc_compilation_result_t zest_ValidateShader(const char *shader_code, shaderc_shader_kind type, const char *name) {
...
zest_bool zest_CompileShader(zest_shader shader) {
    shaderc_compilation_result_t result = shaderc_compile_into_spv(ZestRenderer->shader_compiler, shader->shader_code.str, zest_TextLength(&shader->shader_code), shader->type, shader->name.str, "main", NULL);
...
zest_shader zest_CreateShader(const char *shader_code, shaderc_shader_kind type, const char *name, zest_bool format_code, zest_bool disable_caching) {
...
void zest_GetDescriptorSetsForBinding(zest_shader_resources shader_resources, VkDescriptorSet **draw_sets, zest_uint static_index) {
    for (zest_vec_foreach(set_index, shader_resources->sets)) {
...
void zest__compile_builtin_shaders(void) {
    zest_CreateShader(zest_shader_imgui_vert, shaderc_vertex_shader, "imgui_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_imgui_frag, shaderc_fragment_shader, "imgui_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_billboard_vert, shaderc_vertex_shader, "billboard_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_sprite_frag, shaderc_fragment_shader, "image_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_sprite_alpha_frag, shaderc_fragment_shader, "sprite_alpha_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_sprite_vert, shaderc_vertex_shader, "sprite_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_shape_vert, shaderc_vertex_shader, "shape_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_shape_frag, shaderc_fragment_shader, "shape_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_3d_lines_vert, shaderc_vertex_shader, "3d_lines_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_3d_lines_frag, shaderc_fragment_shader, "3d_lines_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_font_frag, shaderc_fragment_shader, "font_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_mesh_vert, shaderc_vertex_shader, "mesh_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_mesh_instance_vert, shaderc_vertex_shader, "mesh_instance_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_mesh_instance_frag, shaderc_fragment_shader, "mesh_instance_frag.spv", 1, 0);
    zest_CreateShader(zest_shader_swap_vert, shaderc_vertex_shader, "swap_vert.spv", 1, 0);
    zest_CreateShader(zest_shader_swap_frag, shaderc_fragment_shader, "swap_frag.spv", 1, 0);
}
...
void zest__create_texture_image(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout image_layout, zest_bool copy_bitmap) {
    ...
    if (copy_bitmap) {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1);
        zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->memory_offset, texture->frame_buffer[texture->current_index].image, (zest_uint)zest_GetTextureSingleBitmap(texture)->width, (zest_uint)zest_GetTextureSingleBitmap(texture)->height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);
        zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, zest_GetTextureSingleBitmap(texture)->width, zest_GetTextureSingleBitmap(texture)->height, mip_levels, 1, image_layout);
    }
    else {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, image_layout, mip_levels, 1);
    }
}
...
void zest__create_texture_image_array(zest_texture texture, zest_uint mip_levels) {
    ...
    zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, texture->layer_count);
    zest__copy_buffer_regions_to_image(texture->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->memory_offset, texture->frame_buffer[texture->current_index].image);

    zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);

    zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, texture->bitmap_array.width, texture->bitmap_array.height, mip_levels, texture->layer_count, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}
...
void zest__create_texture_stream(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout image_layout, zest_bool copy_bitmap) {
    ...
    if (copy_bitmap) {
        ...
        zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, zest_GetTextureSingleBitmap(texture)->width, zest_GetTextureSingleBitmap(texture)->width, mip_levels, 1, image_layout);
    }
    else {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, image_layout, mip_levels, 1);
    }
...
void zest__refresh_render_target_sampler(zest_render_target render_target, zest_index fif) {
    if (render_target->sampler_textures[fif]->sampler != VK_NULL_HANDLE) {
        vkDestroySampler(ZestDevice->logical_device, render_target->sampler_textures[fif]->sampler[render_target->sampler_textures[fif]->current_index], &ZestDevice->allocation_callbacks);
    }
...
void zest_CleanUpRenderTarget(zest_render_target render_target, zest_index fif) {
    ...
    if (render_target->sampler_textures[fif]->sampler != VK_NULL_HANDLE) {
        vkDestroySampler(ZestDevice->logical_device, render_target->sampler_textures[fif]->sampler[render_target->sampler_textures[fif]->current_index], &ZestDevice->allocation_callbacks);
        render_target->sampler_textures[fif]->sampler[render_target->sampler_textures[fif]->current_index] = VK_NULL_HANDLE;
    }
}
...
void zest__reset_mesh_layer_drawing(zest_layer layer) {
    zest_vec_clear(layer->draw_instructions[layer->fif]);
    layer->memory_refs[layer->fif].write_to_buffer->memory_in_use = 0;
    layer->memory_refs[layer->fif].write_to_index_buffer->memory_in_use = 0;
    layer->current_instruction = zest__layer_instruction();
    layer->memory_refs[layer->fif].index_count = 0;
    layer->memory_refs[layer->fif].vertex_count = 0;
    layer->memory_refs[layer->fif].vertex_ptr = layer->memory_refs[layer->fif].write_to_buffer->data;
...
void zest__end_mesh_instructions(zest_layer layer) {
    if (layer->current_instruction.total_indexes) {
        ...
        layer->memory_refs[layer->fif].write_to_index_buffer->memory_in_use += layer->current_instruction.total_indexes * sizeof(zest_uint);
        layer->memory_refs[layer->fif].write_to_buffer->memory_in_use += layer->current_instruction.total_indexes * layer->vertex_struct_size;
...
void zest_DrawInstanceLayer(zest_layer layer, VkCommandBuffer command_buffer) {
    ...
    for (zest_foreach_i(layer->draw_instructions[layer->fif])) {
        ...
        for (zest_vec_foreach(set_index, current->shader_resources->sets)) {
...
zest_bool zest__grow_instance_buffer(zest_layer layer, zest_size type_size, zest_size minimum_size) {
    ...
    if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        grown = zest_GrowBuffer(&layer->memory_refs[layer->fif].device_instance_data, type_size, minimum_size);
        layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].device_instance_data;
    }
    else {
        grown = zest_GrowBuffer(&layer->memory_refs[layer->fif].staging_instance_data, type_size, minimum_size);
        zest_GrowBuffer(&layer->memory_refs[layer->fif].device_instance_data, type_size, minimum_size);
        layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].staging_instance_data;
    }
    return grown;
}
...
void zest_NextInstance(zest_layer layer) {
    ...
    ZEST_ASSERT(instance_ptr >= (zest_byte*)layer->memory_refs[layer->fif].write_to_buffer->data && instance_ptr <= (zest_byte*)layer->memory_refs[layer->fif].write_to_buffer->end);
    if (instance_ptr == layer->memory_refs[layer->fif].write_to_buffer->end) {
...
void zest_DrawInstanceBulk(zest_layer layer, void *src, zest_uint amount) {
    ...
    int fif = layer->fif;
    ptrdiff_t diff = (zest_byte *)layer->memory_refs[layer->fif].write_to_buffer->end - (instance_ptr + size_in_bytes_to_copy);
    if (diff <= 0) {
...
void zest_DrawLine(zest_layer layer, float start_point[2], float end_point[2], float width) {
    ...
    zest_shape_instance_t* line = (zest_shape_instance_t*)layer->memory_refs[layer->fif].instance_ptr;
...
zest_buffer zest_GetVertexWriteBuffer(zest_layer layer) {
    return layer->memory_refs[layer->fif].write_to_buffer;
}

zest_buffer zest_GetIndexWriteBuffer(zest_layer layer) {
    return layer->memory_refs[layer->fif].write_to_index_buffer;
}
...
zest_buffer zest_GetIndexDeviceBuffer(zest_layer layer) {
    return layer->memory_refs[layer->fif].device_index_data;
}

void zest_GrowMeshVertexBuffers(zest_layer layer) {
    zest_size memory_in_use = zest_GetVertexWriteBuffer(layer)->memory_in_use;
    zest_GrowBuffer(&layer->memory_refs[layer->fif].write_to_buffer, layer->vertex_struct_size, memory_in_use);
    zest_GrowBuffer(&layer->memory_refs[layer->fif].device_vertex_data, layer->vertex_struct_size, memory_in_use);
}

void zest_GrowMeshIndexBuffers(zest_layer layer) {
    zest_size memory_in_use = zest_GetVertexWriteBuffer(layer)->memory_in_use;
    zest_GrowBuffer(&layer->memory_refs[layer->fif].write_to_index_buffer, sizeof(zest_uint), memory_in_use);
    zest_GrowBuffer(&layer->memory_refs[layer->fif].device_index_data, sizeof(zest_uint), memory_in_use);
}
...
void zest_UploadMeshBuffersCallback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
    ...
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        zest_buffer staging_vertex = layer->memory_refs[layer->fif].write_to_buffer;
        zest_buffer staging_index = layer->memory_refs[layer->fif].write_to_index_buffer;
        zest_buffer device_vertex = layer->memory_refs[layer->fif].device_vertex_data;
        zest_buffer device_index = layer->memory_refs[layer->fif].device_index_data;
...
void zest_PushVertex(zest_layer layer, float pos_x, float pos_y, float pos_z, float intensity, float uv_x, float uv_y, zest_color color, zest_uint parameters) {
    ...
    *vertex_ptr = vertex;
    ...
    ZEST_ASSERT(vertex_ptr >= (zest_textured_vertex_t*)layer->memory_refs[layer->fif].write_to_buffer->data && vertex_ptr <= (zest_textured_vertex_t*)layer->memory_refs[layer->fif].write_to_buffer->end);
    if (vertex_ptr == layer->memory_refs[layer->fif].write_to_buffer->end) {
        zest_bool grown = 0;
        if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            grown = zest_GrowBuffer(&layer->memory_refs[layer->fif].device_vertex_data, sizeof(zest_textured_vertex_t), 0);
            layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].device_vertex_data;
        }
        else {
            grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].write_to_buffer, sizeof(zest_textured_vertex_t), 0);
            zest_GrowBuffer(&layer->memory_refs[layer->fif].device_vertex_data, sizeof(zest_textured_vertex_t), 0);
            layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].staging_vertex_data;
        }
        if (grown) {
            layer->memory_refs[layer->fif].vertex_count++;
            vertex_ptr = (zest_textured_vertex_t*)layer->memory_refs[layer->fif].write_to_buffer->data;
...
void zest_PushIndex(zest_layer layer, zest_uint offset) {
    ...
    *index_ptr = index;
    ...
    ZEST_ASSERT(index_ptr >= (zest_uint*)layer->memory_refs[layer->fif].write_to_index_buffer->data && index_ptr <= (zest_uint*)layer->memory_refs[layer->fif].write_to_index_buffer->end);
    if (index_ptr == layer->memory_refs[layer->fif].write_to_index_buffer->end) {
        zest_bool grown = 0;
        if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            grown = zest_GrowBuffer(&layer->memory_refs[layer->fif].device_index_data, sizeof(zest_uint), 0);
            layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].device_index_data;
        }
        else {
            grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].write_to_buffer, sizeof(zest_uint), 0);
            zest_GrowBuffer(&layer->memory_refs[layer->fif].device_index_data, sizeof(zest_uint), 0);
            layer->memory_refs[layer->fif].write_to_buffer = layer->memory_refs[layer->fif].staging_index_data;
        }
        if (grown) {
            layer->memory_refs[layer->fif].index_count++;
            layer->current_instruction.total_indexes++;
            index_ptr = (zest_uint*)layer->memory_refs[layer->fif].write_to_index_buffer->data;
...
void zest_MakeCompute(zest_compute_builder_t* builder, zest_compute compute) {
    ...
    for (zest_foreach_i(compute->shader_names)) {
        ...
        if (zest_map_valid_name(ZestRenderer->shaders, filename.str)) {
            ...
            if (!shader->spv) {
                zest_bool status = zest_CompileShader(shader);
                ZEST_ASSERT(status);    //Failed to compile shader, check the log file for the error.
            }

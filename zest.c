...
void zest__do_scheduled_tasks(void) {
    ...
    zest_map_foreach(i, ZestRenderer->render_graphs) {
        zest_render_graph render_graph = ZestRenderer->render_graphs.data[i];
        ZEST__UNFLAG(render_graph->flags, zest_render_graph_is_compiled);
    }
...
void zest__initialise_renderer(zest_create_info_t* create_info) {
    ...
    zest_ForEachFrameInFlight(fif) {
        ZestRenderer->standard_uniform_buffer[fif] = zest_CreateUniformBuffer(sizeof(zest_uniform_buffer_data_t));
    }
...
void zest__cleanup_renderer() {
    ...
    for (ZEST_EACH_FIF_i) {
        vkDestroySemaphore(ZestDevice->logical_device, ZestRenderer->frame_sync[i].image_available_semaphore, &ZestDevice->allocation_callbacks);
        vkDestroySemaphore(ZestDevice->logical_device, ZestRenderer->frame_sync[i].render_finished_semaphore, &ZestDevice->allocation_callbacks);
        ...
        for (zest_foreach_k(command_queue->draw_commands)) {
            ...
            if (draw_commands->composite_pipeline) {
				...
				zest_ForEachFrameInFlight(fif) {
					...
					draw_commands->composite_descriptor_set[fif].vk_descriptor_set = zest_FinishDescriptorSet(draw_commands->descriptor_pool, &set_builder, draw_commands->composite_descriptor_set[fif].vk_descriptor_set);
                }
                zest_ForEachFrameInFlight(fif) {
...
void zest__create_sync_objects() {
    ...
    for (ZEST_EACH_FIF_i) {
        ZEST_VK_CHECK_RESULT(vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &ZestRenderer->frame_sync[i].image_available_semaphore));
        ZEST_VK_CHECK_RESULT(vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &ZestRenderer->frame_sync[i].render_finished_semaphore));
    }
...
zest_uniform_buffer zest_CreateUniformBuffer(zest_size uniform_struct_size) {
    ...
    buffer_info.property_flags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
	uniform_buffer->buffer = zest_CreateBuffer(uniform_struct_size, &buffer_info, ZEST_NULL);
	uniform_buffer->descriptor_info.buffer = *zest_GetBufferDeviceBuffer(uniform_buffer->buffer);
	uniform_buffer->descriptor_info.offset = uniform_buffer->buffer->memory_offset;
	uniform_buffer->descriptor_info.range = uniform_struct_size;
    return zest__add_uniform_buffer(uniform_buffer);
}

void zest_Update2dUniformBuffer() {
    zest_uniform_buffer_data_t* ubo_ptr = (zest_uniform_buffer_data_t*)zest_GetUniformBufferData(ZestRenderer->standard_uniform_buffer[ZEST_FIF]);
...
void zest_AddLayoutBuilderBindingBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, VkDescriptorType descriptor_type, zest_uint descriptor_count, VkShaderStageFlags stage_flags, VkDescriptorBindingFlags binding_flags, const VkSampler *p_immutable_samplers) {
    bool binding_exists = zest__binding_exists_in_layout_builder(builder, binding_number);
    ZEST_ASSERT(binding_number < 64);   //Invalid binding number, values of 0 - 63 only
    ZEST_ASSERT(!binding_exists);       //That binding number already exists in the layout builder
    VkDescriptorSetLayoutBinding binding = { 0 };
...
void zest_AddLayoutBuilderSamplerBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, zest_uint max_texture_count) {
...
void zest_AddLayoutBuilderSampledImageBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, zest_uint max_texture_count) {
    VkDescriptorBindingFlags binding_flags = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;
...
void zest_AddLayoutBuilderCombinedImageSamplerBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, zest_uint max_texture_count) {
    VkDescriptorBindingFlags binding_flags = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;
...
void zest_AddLayoutBuilderUniformBufferBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, zest_uint max_buffer_count, VkShaderStageFlags shader_stages) {
    VkDescriptorBindingFlags binding_flags = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;
...
void zest_AddLayoutBuilderStorageBufferBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint binding_number, zest_uint max_buffer_count, VkShaderStageFlags shader_stages) {
    VkDescriptorBindingFlags binding_flags = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;
...
zest_descriptor_set_layout zest_FinishDescriptorSetLayout(zest_descriptor_set_layout_builder_t *builder, const char *name, ...) {
    ...
    zest_descriptor_set_layout set_layout = zest__add_descriptor_set_layout(layer_name.str, layout);
...
zest_descriptor_set_layout zest_FinishDescriptorSetLayoutForBindless(zest_descriptor_set_layout_builder_t *builder, zest_uint num_global_sets_this_pool_should_support, VkDescriptorPoolCreateFlags additional_pool_flags, const char *name, ...) {
    ZEST_ASSERT(name);  //Give the descriptor set a unique name
    zest_text_t layer_name = { 0 };
    va_list args;
    va_start(args, name);
...
ZEST_API zest_descriptor_set_t zest_CreateBindlessSet(zest_descriptor_set_layout layout) {
...
zest_descriptor_set_layout zest__add_descriptor_set_layout(const char *name, VkDescriptorSetLayout layout) {
    zest_descriptor_set_layout_t blank = { 0 };
...
zest_uint zest__acquire_bindless_index(zest_descriptor_set_layout layout_handle, zest_uint binding_number) {
...
void zest__release_bindless_index(zest_descriptor_set_layout layout_handle, zest_uint binding_number, zest_uint index_to_release) {
    ZEST_CHECK_HANDLE(layout_handle);   //Not a valid layout handle!
...
void zest_AddSetBuilderUniformBuffer( zest_descriptor_set_builder_t *builder, zest_uint dst_binding, zest_uint dst_array_element, zest_uniform_buffer uniform_buffer) {
    ZEST_CHECK_HANDLE(uniform_buffer);  //Not a valid uniform buffer
    VkDescriptorBufferInfo buffer_info = uniform_buffer->descriptor_info;
...
void zest_AddSetBuilderInstanceLayer(zest_descriptor_set_builder_t *builder, zest_layer layer, zest_uint dst_binding, zest_uint fif) {
    ZEST_CHECK_HANDLE(layer);   //Not a valid layer object
    VkDescriptorBufferInfo buffer_info = layer->memory_refs[fif].instance_descriptor;
    zest_AddSetBuilderWrite(builder, dst_binding, 0, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, NULL, &buffer_info, NULL);
}

void zest_AddSetBuilderInstanceLayerLerp(zest_descriptor_set_builder_t *builder, zest_layer layer, zest_uint dst_binding, zest_uint fif) {
	int index = (int)fif - 1 < 0 ? ZEST_MAX_FIF - 1 : (int)fif - 1;
    ZEST_CHECK_HANDLE(layer);   //Not a valid layer object
    VkDescriptorBufferInfo buffer_info = layer->memory_refs[index].instance_descriptor;
    zest_AddSetBuilderWrite(builder, dst_binding, 0, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, NULL, &buffer_info, NULL);
}

VkDescriptorSet zest_FinishDescriptorSet(zest_descriptor_pool pool, zest_descriptor_set_builder_t *builder, VkDescriptorSet new_set_to_populate_or_update ) {
    ZEST_CHECK_HANDLE(pool);    //Must be a valid desriptor pool
    ZEST_ASSERT(builder != NULL && builder->associated_layout != NULL);

    VkDescriptorSet target_set = new_set_to_populate_or_update;
...
void zest_AddDescriptorSetsToResources(zest_shader_resources resources, zest_descriptor_set *descriptor_sets) {
    zest_ForEachFrameInFlight(fif) {
        ZEST_CHECK_HANDLE(descriptor_sets[fif]);    //Not a valid descriptor set. Make sure the array you pass in is a valid array of descriptor sets that is the same length as the number of frames in flight
        zest_vec_push(resources->sets[fif], descriptor_sets[fif]);
    }
}
...
void zest_SetPipelineTemplateVertShader(zest_pipeline_template pipeline_template, const char* file, const char* prefix) {
...
void zest_SetPipelineTemplateFragShader(zest_pipeline_template pipeline_template, const char* file, const char* prefix) {
...
void zest_SetPipelineTemplateShader(zest_pipeline_template pipeline_template, const char* file, const char* prefix) {
    zest_SetText(&pipeline_template->fragShaderFile, file);
    zest_SetText(&pipeline_template->vertShaderFile, file);
    if (prefix) {
        zest_SetText(&pipeline_template->shader_path_prefix, prefix);
...
void zest_FinalisePipelineTemplate(zest_pipeline_template pipeline_template) {
...
zest_pipeline_template zest_CopyPipelineTemplate(const char *name, zest_pipeline_template pipeline_to_copy) {
    zest_pipeline_template copy = zest_CreatePipelineTemplate(name);
...
zest_pipeline_template zest_CreatePipelineTemplate(const char* name) {
...
void zest_AddVertexInputDescription(zest_pipeline_template pipeline_template,  VkVertexInputAttributeDescription description) {
    zest_vec_push(pipeline_template->attributeDescriptions, description);
}
...
zest_descriptor_set_layout zest_GetDescriptorSetLayout(const char* name) { 
    ZEST_ASSERT(zest_map_valid_name(ZestRenderer->descriptor_layouts, name));   //Must be a valid descriptor set layout, check the name is correct
...
void* zest_GetUniformBufferData(zest_uniform_buffer uniform_buffer) {
    return uniform_buffer->buffer->data;
}
...
void zest__prepare_standard_pipelines() {
    ...
    //2d sprite rendering
    zest_pipeline_template sprite_instance_pipeline = zest_CreatePipelineTemplate("pipeline_2d_sprites");
    ...
    sprite_instance_pipeline->scissor.extent = zest_GetSwapChainExtent();
    ...
    zest_FinalisePipelineTemplate(sprite_instance_pipeline_alpha);
    sprite_instance_pipeline_alpha->depthStencil.depthWriteEnable = VK_FALSE;
    ...
    zest_FinalisePipelineTemplate(line3d_instance_pipeline);
    ...
    zest_SetPipelineTemplatePushConstantRange(font_pipeline, sizeof(zest_push_constants_t), 0, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);
    zest_SetText(&font_pipeline->vertShaderFile, "sprite_vert.spv");
    zest_SetText(&font_pipeline->fragShaderFile, "font_frag.spv");
    zest_FinalisePipelineTemplate(font_pipeline);
    ...
    zest_pipeline_template mesh_pipeline = zest_CreatePipelineTemplate("pipeline_mesh");
    mesh_pipeline->scissor.offset.x = 0;
    mesh_pipeline->scissor.offset.y = 0;
    zest_SetPipelineTemplatePushConstantRange(mesh_pipeline, sizeof(zest_push_constants_t), 0, VK_SHADER_STAGE_VERTEX_BIT);
    ...
    zest_FinalisePipelineTemplate(mesh_pipeline);
    ...
    zest_pipeline_template imesh_pipeline = zest_CreatePipelineTemplate("pipeline_mesh_instance");
    imesh_pipeline->scissor.offset.x = 0;
    imesh_pipeline->scissor.offset.y = 0;
    zest_SetPipelineTemplatePushConstantRange(imesh_pipeline, sizeof(zest_push_constants_t), 0, VK_SHADER_STAGE_VERTEX_BIT);
    ...
    zest_FinalisePipelineTemplate(imesh_pipeline);
    ...
    zest_pipeline_template final_render = zest_CreatePipelineTemplate("pipeline_swap_chain");
    final_render->scissor.extent = zest_GetSwapChainExtent();
    zest_SetPipelineTemplatePushConstantRange(final_render, sizeof(zest_render_target_push_constants_t), 0, VK_SHADER_STAGE_VERTEX_BIT);
    ...
    zest_FinalisePipelineTemplate(final_render);
...
void zest__present_frame() {
    ...
    presentInfo.pWaitSemaphores = &ZestRenderer->frame_sync[ZEST_FIF].render_finished_semaphore;
...
void zest__dummy_submit_for_present_only(void) {
    ...
    image_barrier.image = ZestRenderer->swapchain_images[ZestRenderer->current_frame];
    ...
    submit_info.pWaitSemaphores = &ZestRenderer->frame_sync[ZEST_FIF].image_available_semaphore;
    submit_info.pWaitDstStageMask = wait_stage_array;

    submit_info.signalSemaphoreCount = 1;
    submit_info.pSignalSemaphores = &ZestRenderer->frame_sync[ZEST_FIF].render_finished_semaphore;
...
void zest__create_transient_buffer(zest_rg_resource_node node) {
    node->storage_buffer = zest_CreateBuffer(node->buffer_desc.size, &node->buffer_desc.buffer_info, 0);
}

void zest__create_transient_image(zest_rg_resource_node node) {
...
zest_render_graph zest_NewRenderGraph(const char *name, zest_descriptor_set_layout set_layout, bool force_on_graphics_queue) {
...
void zest_ResetRenderGraph(zest_render_graph render_graph) {
    ZEST_CHECK_HANDLE(render_graph);    //Not a valid render graph handle!
    render_graph->compiled_execution_order = NULL;
    render_graph->passes = NULL;
    render_graph->resources = NULL;
    render_graph->pass_exec_details_list = NULL;
    render_graph->submissions = NULL;

    render_graph->id_counter = 0;
}

bool zest_BeginRenderGraph(zest_render_graph render_graph) {
    ...
    zest_ResetRenderGraph(render_graph);
...
bool zest_BeginRenderToScreen(zest_render_graph render_graph) {
    if (zest_BeginRenderGraph(render_graph)) {
		ZEST__FLAG(render_graph->flags, zest_render_graph_expecting_swap_chain_usage);
    } else {
        return false;
    }
...
*/
void zest_EndRenderGraph(zest_render_graph render_graph) {
    ZEST_CHECK_HANDLE(render_graph);        //Not a valid render graph!
    ...
    zest_vec_foreach(i, render_graph->passes) { 
        ...
        zest_map_foreach(j, pass_node->inputs) {
            ...
            zest_rg_resource_node resource = input_usage->resource_node;
    ...
    zest_vec_foreach(consumer_idx, render_graph->passes) {
        zest_rg_pass_node consumer_pass = &render_graph->passes[consumer_idx];
    ...
    zest_rg_pass_node pass_node = &render_graph->passes[first_pass_index];
    ...
    zest_vec_foreach(execution_index, render_graph->compiled_execution_order) { 
        ...
        zest_map_foreach(input_idx, current_pass->inputs) {
            zest_rg_resource_node resource_node = current_pass->inputs.data[input_idx].resource_node;
            if (resource_node) { 
        ...
        zest_map_foreach(output_idx, current_pass->outputs) {
            zest_rg_resource_node resource_node = current_pass->outputs.data[output_idx].resource_node;
            if (resource_node) { 
                resource_node->first_usage_pass_idx = ZEST__MIN(resource_node->first_usage_pass_idx, (zest_uint)execution_index);
                resource_node->last_usage_pass_idx = ZEST__MAX(resource_node->last_usage_pass_idx, (zest_uint)execution_index);
            }
    ...
    zest_vec_foreach(r, render_graph->resources) {
        zest_rg_resource_node resource = &render_graph->resources[r];
    ...
    zloc_size execution_marker = zloc_GetMarker(allocator);
    ...
    zest_vec_foreach(batch_index, render_graph->submissions) {
        ...
        zest_vec_foreach(k, batch->pass_indices) {
            ...
            zest_rg_pass_node pass = &render_graph->passes[current_pass_index];
            zest_execution_details_t *exe_details = &render_graph->pass_exec_details_list[current_pass_index];
            exe_details->pre_pass_image_barriers = 0;
            exe_details->pre_pass_buffer_barriers = 0;
            exe_details->pre_pass_image_barrier_nodes = 0;
            exe_details->pre_pass_buffer_barrier_nodes = 0;
            exe_details->overall_dst_stage_mask_for_pre_pass_barriers = 0;
            exe_details->overall_src_stage_mask_for_pre_pass_barriers = 0;
            exe_details->clear_values = 0;
            exe_details->render_pass = VK_NULL_HANDLE;
            exe_details->frame_buffer = VK_NULL_HANDLE;

            zest_bool requires_dynamic_render_pass = ZEST_FALSE;

            //Create_memory_barriers_for_inputs
            zest_map_foreach(i, pass->inputs) {
                zest_resource_usage_t *input_usage = &pass->inputs.data[i];
                zest_rg_resource_node input_resource = pass->inputs.data[i].resource_node;
                VkImageLayout required_layout = input_usage->image_layout;
                VkAccessFlags required_access = input_usage->access_mask;
                VkPipelineStageFlags required_stage = input_usage->stage_mask;
                //Write a function to check for access conflicts here for more robust check
                bool needs_barrier = false;
                if (input_resource->current_layout != required_layout ||
                    (input_resource->current_access_mask & zest_access_write_bits_general) &&
                    (required_access & zest_access_read_bits_general)) {
                    /* add more robust hazard checks here, e.g., if current_access involves write and required_access is read */
                    needs_barrier = ZEST_TRUE;
                }
                if (needs_barrier) {
                    ...
                    case zest_resource_type_swap_chain_image: 
                        VkImageMemoryBarrier image_barrier = zest__create_image_memory_barrier(
                            input_resource->image_buffer.image,
                            input_resource->current_access_mask,
                            required_access,
                            input_resource->current_layout,
                            required_layout,
                            0, input_resource->image_desc.mip_levels);

                        if (input_resource->current_queue_family_index != batch_target_qfi &&
                            input_resource->current_queue_family_index != VK_QUEUE_FAMILY_IGNORED) {
                            // Queue family ownership acquire
                            image_barrier.srcQueueFamilyIndex = input_resource->current_queue_family_index;
                            image_barrier.dstQueueFamilyIndex = batch_target_qfi;
                            // For acquire, srcAccessMask is often 0 as per spec
                            image_barrier.srcAccessMask = 0;
                        }

						zest_vec_linear_push(allocator, exe_details->pre_pass_image_barriers, image_barrier);
						zest_vec_linear_push(allocator, exe_details->pre_pass_image_barrier_nodes, input_resource);
                        break;
                    case zest_resource_type_buffer:
                        VkBufferMemoryBarrier buffer_barrier = zest__create_buffer_memory_barrier(
                            input_resource->storage_buffer->memory_pool->buffer,
                            input_resource->current_access_mask,
                            required_access,
                            input_resource->storage_buffer->memory_offset,
                            input_resource->storage_buffer->size);

                        if (input_resource->current_queue_family_index != batch_target_qfi &&
                            input_resource->current_queue_family_index != VK_QUEUE_FAMILY_IGNORED) {
                            // Queue family ownership acquire
                            buffer_barrier.srcQueueFamilyIndex = input_resource->current_queue_family_index;
                            buffer_barrier.dstQueueFamilyIndex = batch_target_qfi;
                            // For acquire, srcAccessMask is often 0 as per spec
                            buffer_barrier.srcAccessMask = 0;
                        }

						zest_vec_linear_push(allocator, exe_details->pre_pass_buffer_barriers, buffer_barrier);
						zest_vec_linear_push(allocator, exe_details->pre_pass_buffer_barrier_nodes, input_resource);
                        break;
                    }
                ...
                input_resource->last_stage_mask = required_stage;
                input_resource->current_queue_family_index = batch_target_qfi;
                if ((input_resource->type == zest_resource_type_image || input_resource->type == zest_resource_type_swap_chain_image) &&
                    input_usage->access_mask & zest_access_render_pass_bits) {
                    requires_dynamic_render_pass = ZEST_TRUE;
                }
            }

            //Create_memory_barriers_for_outputs
            //Now loop over outputs and create memory barriers for transitioning for whatever the final layout should be (for this pass)
            zest_map_foreach(o, pass->outputs) {
                zest_resource_usage_t *output_usage = &pass->outputs.data[o];
                zest_rg_resource_node output_resource = pass->outputs.data[o].resource_node;
                VkImageLayout required_layout = output_usage->image_layout;
                VkAccessFlags required_access = output_usage->access_mask;
                VkPipelineStageFlags required_stage = output_usage->stage_mask;
                bool needs_barrier = false;
                if (output_resource->current_layout != required_layout ||
                    (output_resource->current_access_mask & (zest_access_write_bits_general | zest_access_read_bits_general))) {
                    /* add more robust hazard checks, e.g. if current_access involves read and write_access is write (RAW hazard not for pre-barrier)
                       or if previous write (current_access) needs to finish before this write (WAW) */
                    needs_barrier = ZEST_TRUE;
                }
                if (needs_barrier) {
                    switch (output_resource->type) {
                    case zest_resource_type_image:
                    case zest_resource_type_swap_chain_image:
                        VkImageMemoryBarrier image_barrier = zest__create_image_memory_barrier(output_resource->image_buffer.image,
                            output_resource->current_access_mask,
                            required_access,
                            output_resource->current_layout,
                            required_layout,
                            0, output_resource->image_desc.mip_levels);

                        if (output_resource->current_queue_family_index != batch_target_qfi &&
                            output_resource->current_queue_family_index != VK_QUEUE_FAMILY_IGNORED) {
                            // Queue family ownership acquire
                            image_barrier.srcQueueFamilyIndex = output_resource->current_queue_family_index;
                            image_barrier.dstQueueFamilyIndex = batch_target_qfi;
                            // For acquire, srcAccessMask is often 0 as per spec
                            image_barrier.srcAccessMask = 0;
                        }
						...
						zest_vec_linear_push(allocator, exe_details->pre_pass_image_barriers, image_barrier);
						zest_vec_linear_push(allocator, exe_details->pre_pass_image_barrier_nodes, output_resource);
                        break;
                    case zest_resource_type_buffer:
                        VkBufferMemoryBarrier buffer_barrier = zest__create_buffer_memory_barrier(
                            output_resource->storage_buffer->memory_pool->buffer,
                            output_resource->current_access_mask,
                            required_access,
                            output_resource->storage_buffer->memory_offset,
                            output_resource->storage_buffer->size);
                        if (output_resource->current_queue_family_index != batch_target_qfi &&
                            output_resource->current_queue_family_index != VK_QUEUE_FAMILY_IGNORED) {
                            // Queue family ownership acquire
                            buffer_barrier.srcQueueFamilyIndex = output_resource->current_queue_family_index;
                            buffer_barrier.dstQueueFamilyIndex = batch_target_qfi;
                            // For acquire, srcAccessMask is often 0 as per spec
                            buffer_barrier.srcAccessMask = 0;
                        }
						...
						zest_vec_linear_push(allocator, exe_details->pre_pass_buffer_barriers, buffer_barrier);
						zest_vec_linear_push(allocator, exe_details->pre_pass_buffer_barrier_nodes, output_resource);
                        break;
                    }
					exe_details->overall_src_stage_mask_for_pre_pass_barriers |= output_resource->last_stage_mask;
					exe_details->overall_dst_stage_mask_for_pre_pass_barriers |= required_stage;
                }
                output_resource->current_layout = required_layout;
                output_resource->current_access_mask = required_access;
                output_resource->last_stage_mask = required_stage;
                output_resource->current_queue_family_index = batch_target_qfi;
                if ((output_resource->type == zest_resource_type_image || output_resource->type == zest_resource_type_swap_chain_image) &&
                    output_usage->access_mask & zest_access_render_pass_bits) {
                    requires_dynamic_render_pass = ZEST_TRUE;
                }
            }

            // Handle TOP_OF_PIPE if no actual prior stages generated by resource usage
            if (exe_details->overall_src_stage_mask_for_pre_pass_barriers == 0) {
                exe_details->overall_src_stage_mask_for_pre_pass_barriers = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
            }
            // Handle BOTTOM_OF_PIPE if no actual subsequent stages (though less common for pre-pass barriers)
            if (exe_details->overall_dst_stage_mask_for_pre_pass_barriers == 0) {
                exe_details->overall_dst_stage_mask_for_pre_pass_barriers = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
            }

            //If this pass is a render pass with an execution callback
            //Create_render_passes
            if (zest_vec_size(pass->execution_callbacks) && requires_dynamic_render_pass) {
                ...
                zest_rg_resource_node *attachment_resource_nodes = 0;
                ...
                zest_vec_foreach(c, color_attachment_info) {
                    zest_rg_resource_node node = color_attachment_info[c].resource_node;
                    ...
                    if (!zest_map_valid_key(attachment_indexes, (zest_key)node)) {
                        ...
                        attachment.finalLayout = zest__determine_final_layout(render_graph, k_global + 1, node, usage_info);
                ...
                if (depth_attachment_info.resource_node) {
                    zest_rg_resource_node node = depth_attachment_info.resource_node;
                ...
                zest_map_foreach(i, attachment_indexes) {
                    zest_rg_resource_node node = attachment_resource_nodes[i];
                ...
                //Cleanup the temp lists
                zloc_ResetToMarker(allocator, execution_marker);
            }
			k_global++;
...
VkImageLayout zest__determine_final_layout(zest_render_graph render_graph, int start_from_idx, zest_rg_resource_node node, zest_resource_usage_t *current_usage) {
...
void zest_ExecuteRenderGraph(zest_render_graph render_graph) {
    ZEST_CHECK_HANDLE(render_graph);    //Not a valid render graph handle!
    zest_vec_foreach(batch_index, render_graph->submissions) {
        zest_submission_batch_t *batch = &render_graph->submissions[batch_index];
        VkCommandBuffer command_buffer;
        ...
        zest_vec_foreach(i, batch->pass_indices) {
            ...
            if (zest_vec_size(exe_details->pre_pass_image_barriers) > 0 ||
                zest_vec_size(exe_details->pre_pass_buffer_barriers) > 0) {
                ...
                vkCmdPipelineBarrier(
                    ...
                    0,
                    0, NULL,
                    zest_vec_size(exe_details->pre_pass_buffer_barriers),
                    exe_details->pre_pass_buffer_barriers,
                    zest_vec_size(exe_details->pre_pass_image_barriers),
                    exe_details->pre_pass_image_barriers
                );
            }
        ...
        // 4. Prepare VkSubmitInfo for this batch
        ...
        if (batch_signal_sem != VK_NULL_HANDLE) {
            submit_info.signalSemaphoreCount = 1;
            submit_info.pSignalSemaphores = &batch_signal_sem;
        }

        ZEST_VK_CHECK_RESULT(vkQueueSubmit(batch->queue, 1, &submit_info, submit_fence));
		ZEST__FLAG(ZestRenderer->flags, zest_renderer_flag_work_was_submitted);
    }

	zest_vec_foreach(index, render_graph->resources) {
		zest_rg_resource_node resource = &render_graph->resources[index];
		if (ZEST__FLAGGED(resource->flags, zest_resource_node_flag_transient)) {
...
void zest_PrintCompiledRenderGraph(zest_render_graph render_graph) {
    ...
    zest_vec_foreach(batch_index, render_graph->submissions) {
        ...
        zest_vec_foreach(batch_pass_index, batch->pass_indices) {
            ...
            zest_rg_pass_node pass_node = &render_graph->passes[pass_index];
            zest_execution_details_t *exe_details = &render_graph->pass_exec_details_list[pass_index];
            ...
            if (zest_vec_size(exe_details->pre_pass_image_barriers) > 0 ||
                zest_vec_size(exe_details->pre_pass_buffer_barriers) > 0) {
				zest_text_t overal_src_pipeline_stages = zest_vulkan_pipeline_stage_flags_to_string(exe_details->overall_src_stage_mask_for_pre_pass_barriers);
				zest_text_t overal_dst_pipeline_stages = zest_vulkan_pipeline_stage_flags_to_string(exe_details->overall_dst_stage_mask_for_pre_pass_barriers);
                ZEST_PRINT("      Pre-Pass Barriers (Overall Pipeline Src Stages: %s, Dst Stages: %s):",
                ...
                ZEST_PRINT("        Images:");
                zest_vec_foreach(barrier_index, exe_details->pre_pass_image_barriers) {
                    VkImageMemoryBarrier *imb = &exe_details->pre_pass_image_barriers[barrier_index];
                    zest_rg_resource_node image_resource = exe_details->pre_pass_image_barrier_nodes[barrier_index];
                ...
                ZEST_PRINT("        Buffers:");
                zest_vec_foreach(barrier_index, exe_details->pre_pass_buffer_barriers) {
                    VkBufferMemoryBarrier *bmb = &exe_details->pre_pass_buffer_barriers[barrier_index];
                    zest_rg_resource_node buffer_resource = exe_details->pre_pass_buffer_barrier_nodes[barrier_index];
                    ...
                    ZEST_PRINT("            %s, Access: %s -> %s, QFI: %u -> %u",
...
void zest_SubmitRenderGraph(zest_render_graph render_graph, VkFence fence) {
}

// --Command Queue functions
zest_uint zest__get_image_binding_number(zest_render_graph render_graph, zest_rg_resource_node resource, bool image_view_only) {
...
zest_uint zest__get_buffer_binding_number(zest_render_graph render_graph, zest_rg_resource_node resource) {
    if (!ZEST_VALID_HANDLE(render_graph->bindless_layout)) {
        ZEST_PRINT("ERROR: render graph %s doesn't have a bindless layout assigned, but you're tring to add a resource that uses a binding number.", render_graph->name);
        return ZEST_INVALID;
    }
...
void zest_AssignBindlessTextureIndex(zest_texture texture, zest_descriptor_set_layout layout, zest_descriptor_set_t *set, zest_uint target_binding_number) {
    ...
    texture->descriptor_array_index = zest__acquire_bindless_index(layout, binding_number);
    ZEST_ASSERT(texture->descriptor_array_index != ZEST_INVALID);   //Ran out of space in the descriptor pool
    ...
    write.dstArrayElement = texture->descriptor_array_index;
	vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);
}

void zest_AssignBindlessStorageBufferIndex(zest_descriptor_buffer buffer, zest_descriptor_set_layout layout, zest_descriptor_set_t *set, zest_uint target_binding_number) {
    ZEST_CHECK_HANDLE(layout);  //Must be a valid handle to a descriptor set layout
    ...
    ZEST_ASSERT(binding_number != ZEST_INVALID);    //Could not find an appropriate descriptor type in the layout with that target binding number!
    buffer->descriptor_array_index = zest__acquire_bindless_index(layout, binding_number);
    ...
    write.dstArrayElement = buffer->descriptor_array_index;
    vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);
}

zest_rg_resource_node zest_AddTransientImageResource(zest_render_graph render_graph, const char *name, const zest_image_description_t *description, zest_bool assign_bindless, zest_bool image_view_binding_only) {
    zest_rg_resource_node_t node = { 0 };
    node.name = name;
    ...
    if (assign_bindless) {
        ...
        node.bindless_index = zest__get_image_binding_number(render_graph, &node, image_view_binding_only);
...
zest_rg_resource_node zest_AddTransientBufferResource(zest_render_graph render_graph, const char *name, const zest_buffer_description_t *description, zest_bool assign_bindless) {
...
zest_rg_resource_node_t zest__create_import_buffer_resource_node(zest_render_graph render_graph, const char *name, zest_descriptor_buffer buffer) {
    zest_rg_resource_node_t node = { 0 };
    node.name = name;
	node.id = render_graph->id_counter++;
...
zest_rg_resource_node_t zest__create_import_image_resource_node(zest_render_graph render_graph, const char *name, zest_texture texture) {
    zest_rg_resource_node_t node = { 0 };
    node.name = name;
	node.id = render_graph->id_counter++;
...
zest_rg_resource_node zest_ImportImageResource(zest_render_graph render_graph, const char *name, zest_texture texture, VkImageLayout initial_layout_at_graph_start, VkImageLayout desired_layout_after_graph_use) {
    zest_rg_resource_node_t node = zest__create_import_image_resource_node(render_graph, name, texture);
...
zest_rg_resource_node zest_ImportImageResourceReadOnly(zest_render_graph render_graph, const char *name, zest_texture texture) {
    zest_rg_resource_node_t node = zest__create_import_image_resource_node(render_graph, name, texture);
...
zest_rg_resource_node zest_ImportStorageBufferResource(zest_render_graph render_graph, const char *name, zest_descriptor_buffer buffer) {
    zest_rg_resource_node_t node = zest__create_import_buffer_resource_node(render_graph, name, buffer);
    zest_vec_linear_push(ZestRenderer->render_graph_allocator, render_graph->resources, node);
    return &zest_vec_back(render_graph->resources);
}

zest_rg_resource_node zest_ImportLayerResource(zest_render_graph render_graph, const char *name, zest_layer layer) {
    zest_rg_resource_node_t node = { 0 };
    node.name = name;
	node.id = render_graph->id_counter++;
	node.resource = (zest_resource_handle)layer;
    node.first_usage_pass_idx = UINT_MAX;
	node.type = zest_resource_type_image;
    node.render_graph = render_graph;
    node.current_queue_family_index = VK_QUEUE_FAMILY_IGNORED;
...
zest_bool zest_AcquireSwapChainImage() {
    VkResult result = vkAcquireNextImageKHR(ZestDevice->logical_device, ZestRenderer->swapchain, UINT64_MAX, ZestRenderer->frame_sync[ZEST_FIF].image_available_semaphore, ZEST_NULL, &ZestRenderer->current_frame);
...
zest_rg_resource_node zest_ImportSwapChainResource(zest_render_graph render_graph, const char *name) {
    zest_rg_resource_node_t node = { 0 };
    node.name = name;
	node.id = render_graph->id_counter++;
    node.first_usage_pass_idx = UINT_MAX;
    ...
    node.render_graph = render_graph;
    node.magic = zest_INIT_MAGIC;
    node.image_buffer.image = ZestRenderer->swapchain_images[ZestRenderer->current_frame];
    node.image_buffer.base_view = ZestRenderer->swapchain_image_views[ZestRenderer->current_frame];
...
zest_rg_pass_node zest_AddGraphicBlankScreen(zest_render_graph render_graph, const char *name) {
    zest_rg_pass_node pass = zest__add_pass_node(render_graph, name, zest_queue_graphics);
    zest_AddPassTask(pass, zest_EmptyRenderPass, 0);
    return pass;
}

zest_rg_pass_node zest_AddGraphicPassNode(zest_render_graph render_graph, const char *name) {
    return zest__add_pass_node(render_graph, name, zest_queue_graphics);
}

zest_rg_pass_node zest_AddComputePassNode(zest_render_graph render_graph, zest_compute compute, const char *name) {
    bool force_graphics_queue = (render_graph->flags & zest_render_graph_force_on_graphics_queue) > 0;
    zest_rg_pass_node node = zest__add_pass_node(render_graph, name, force_graphics_queue ? zest_queue_graphics : zest_queue_compute);
    node->compute = compute;
    return node;
}

zest_rg_pass_node zest_AddTransferPassNode(zest_render_graph render_graph, const char *name) {
    bool force_graphics_queue = (render_graph->flags & zest_render_graph_force_on_graphics_queue) > 0;
    return zest__add_pass_node(render_graph, name, force_graphics_queue ? zest_queue_graphics : zest_queue_transfer);
}

zest_rg_resource_node zest_GetPassInputResource(zest_rg_pass_node pass, const char *name) {
...
zest_rg_resource_node zest_GetPassOutputResource(zest_rg_pass_node pass, const char *name) {
...
void zest_add_pass_buffer_usage(zest_rg_pass_node pass_node, zest_rg_resource_node buffer_resource, zest_resource_access_purpose purpose, VkPipelineStageFlags relevant_pipeline_stages, zest_bool is_output) {
...
void zest_AddPassUniformBufferInput(zest_rg_pass_node pass, zest_rg_resource_node uniform_buffer, VkPipelineStageFlags stages) {
    zest_add_pass_buffer_usage(pass, uniform_buffer, zest_purpose_uniform_buffer, stages, ZEST_FALSE);
}

void zest_AddPassStorageBufferRead(zest_rg_pass_node pass, zest_rg_resource_node storage_buffer, VkPipelineStageFlags stages) {
    zest_add_pass_buffer_usage(pass, storage_buffer, zest_purpose_storage_buffer_read, stages, ZEST_FALSE);
}

void zest_AddPassStorageBufferWrite(zest_rg_pass_node pass, zest_rg_resource_node storage_buffer, VkPipelineStageFlags stages) {
...
zest_command_queue_draw_commands zest_NewDrawCommandSetupUpSampler(const char *name, zest_render_target render_target, zest_render_target downsampler, zest_pipeline_template pipeline) { ZEST_CHECK_HANDLE(render_target);  //render_target must be a valid handle to a render target
    ...
    for (int mip_level = 0; mip_level != render_target->mip_levels; ++mip_level) {
        zest_ForEachFrameInFlight(fif) {
			...
			render_target->mip_level_descriptor_sets[render_target->current_index][fif][mip_level].vk_descriptor_set = zest_FinishDescriptorSet(draw_commands->descriptor_pool,&builder, render_target->mip_level_descriptor_sets[render_target->current_index][fif][mip_level].vk_descriptor_set);
        }
    }
    if (zest_Vec2Length2(render_target->create_info.ratio_of_screen_size)) {
        draw_commands->viewport_scale = render_target->create_info.ratio_of_screen_size;
...
void zest_AddLayer(zest_layer layer) {
    ...
    ZEST_ASSERT(layer->draw_routine);   //Layer must have a draw routine. Use the layer builders to create a new layer
    ...
    layer->draw_routine->draw_commands = draw_commands;
...
void zest__setup_font_texture(zest_font font) {
    ...
    zest_ForEachFrameInFlight(fif) {
        zest_AddDescriptorSetToResources(font->shader_resources, &ZestRenderer->standard_uniform_buffer[fif]->descriptor_set, fif);
    }
...
void zest__create_render_target_sampler(zest_render_target render_target) {
    ...
    zest_ForEachFrameInFlight(fif) {
        ...
        set->vk_descriptor_set = zest_FinishDescriptorSet(render_target->descriptor_pool[index], &builder, set->vk_descriptor_set);
    }
}
    ...
    for (zest_index mip_level = 0; mip_level != render_target->mip_levels; ++mip_level) {
        ...
        if (ZEST__FLAGGED(render_target->flags, zest_render_target_flag_upsampler)) {
            zest_ForEachFrameInFlight(fif) {
				...
				render_target->mip_level_descriptor_sets[index][fif][mip_level].vk_descriptor_set = zest_FinishDescriptorSet(render_target->descriptor_pool[index], &builder, render_target->mip_level_descriptor_sets[index][fif][mip_level].vk_descriptor_set);
            }
        }
    }
}
...
void zest_ResetInstanceLayerDrawing(zest_layer layer) {
    ...
    ZEST_CHECK_HANDLE(layer->draw_routine);	//Not a valid handle!
    zest_vec_clear(layer->draw_instructions[layer->draw_routine->fif]);
    layer->memory_refs[layer->draw_routine->fif].write_to_buffer->memory_in_use = 0;
    layer->current_instruction = zest__layer_instruction();
    layer->memory_refs[layer->draw_routine->fif].instance_count = 0;
    layer->memory_refs[layer->draw_routine->fif].instance_ptr = layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data;
}

zest_uint zest_GetInstanceLayerCount(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    return layer->memory_refs[layer->draw_routine->fif].instance_count;
}
...
void zest__reset_mesh_layer_drawing(zest_layer layer) {
    ...
    zest_vec_clear(layer->draw_instructions[layer->draw_routine->fif]);
    layer->memory_refs[layer->draw_routine->fif].write_to_buffer->memory_in_use = 0;
    layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->memory_in_use = 0;
    layer->current_instruction = zest__layer_instruction();
    layer->memory_refs[layer->draw_routine->fif].index_count = 0;
    layer->memory_refs[layer->draw_routine->fif].vertex_count = 0;
    layer->memory_refs[layer->draw_routine->fif].vertex_ptr = layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data;
    layer->memory_refs[layer->draw_routine->fif].index_ptr = layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->data;
}
...
int zest_LayerHasInstructionsCallback(zest_draw_routine draw_routine) {
    ...
    zest_layer layer = (zest_layer)draw_routine->draw_data;
    return zest_vec_size(layer->draw_instructions[layer->draw_routine->fif]) > 0;
}
...
void zest_StartInstanceInstructions(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    layer->current_instruction.start_index = layer->memory_refs[layer->draw_routine->fif].instance_count ? layer->memory_refs[layer->draw_routine->fif].instance_count : 0;
    layer->current_instruction.push_constants = layer->push_constants;
}

void zest_ResetLayer(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    ZEST_CHECK_HANDLE(layer->draw_routine);   //Not a valid draw routine in layer. Did you set up the layer properly? It must have a draw routine!
    layer->draw_routine->last_fif = layer->draw_routine->fif;
    layer->draw_routine->fif = (layer->draw_routine->fif + 1) % ZEST_MAX_FIF;
    for (ZEST_EACH_FIF_i) {
        layer->draw_routine->recorder->outdated[i] = 1;
    }
}
...
void zest__start_mesh_instructions(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    layer->current_instruction.start_index = layer->memory_refs[layer->draw_routine->fif].index_count ? layer->memory_refs[layer->draw_routine->fif].index_count : 0;
    layer->current_instruction.push_constants = layer->push_constants;
}
...
void zest_EndInstanceInstructions(zest_layer layer) {
    ...
    if (layer->current_instruction.total_instances) {
        ...
        zest_vec_push(layer->draw_instructions[layer->draw_routine->fif], layer->current_instruction);
        layer->memory_refs[layer->draw_routine->fif].write_to_buffer->memory_in_use += layer->current_instruction.total_instances * layer->instance_struct_size;
...
zest_bool zest_MaybeEndInstanceInstructions(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    if (layer->draw_routine->last_fif == layer->draw_routine->fif) return 0;
    layer->draw_routine->last_fif = layer->draw_routine->fif;
    if (layer->current_instruction.total_instances) {
        layer->last_draw_mode = zest_draw_mode_none;
        zest_vec_push(layer->draw_instructions[layer->draw_routine->fif], layer->current_instruction);
...
void zest__end_mesh_instructions(zest_layer layer) {
    ...
    if (layer->current_instruction.total_indexes) {
        ...
        layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->memory_in_use += layer->current_instruction.total_indexes * sizeof(zest_uint);
        layer->memory_refs[layer->draw_routine->fif].write_to_buffer->memory_in_use += layer->current_instruction.total_indexes * layer->vertex_struct_size;
...
void zest__update_instance_layer_buffers_callback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
    ...
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        if (!zest_vec_empty(layer->draw_instructions[layer->draw_routine->fif])) {
            zest_AddCopyCommand(&layer->vertex_upload, layer->memory_refs[layer->draw_routine->fif].staging_instance_data, layer->memory_refs[layer->draw_routine->fif].device_instance_data, 0);
        }
...
zest_bool zest__grow_instance_buffer(zest_layer layer, zest_size type_size, zest_size minimum_size) {
    ...
    if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        grown = zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_instance_data, type_size, minimum_size);
        layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].device_instance_data;
    }
    else {
        grown = zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].staging_instance_data, type_size, minimum_size);
        zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_instance_data, type_size, minimum_size);
        layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].staging_instance_data;
    }
    return grown;
}
...
void zest_NextInstance(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_byte* instance_ptr = (zest_byte*)layer->memory_refs[layer->draw_routine->fif].instance_ptr + layer->instance_struct_size;
    //Make sure we're not trying to write outside of the buffer range
    ZEST_ASSERT(instance_ptr >= (zest_byte*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data && instance_ptr <= (zest_byte*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end);
    if (instance_ptr == layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end) {
        if (zest__grow_instance_buffer(layer, layer->instance_struct_size, 0)) {
            layer->memory_refs[layer->draw_routine->fif].instance_count++;
            instance_ptr = (zest_byte*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data;
            instance_ptr += layer->memory_refs[layer->draw_routine->fif].instance_count * layer->instance_struct_size;
...
zest_draw_buffer_result zest_DrawInstanceBuffer(zest_layer layer, void *src, zest_uint amount) {
    ...
    zest_byte *instance_ptr = (zest_byte *)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
    int fif = layer->draw_routine->fif;
    ptrdiff_t diff = (zest_byte *)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end - (instance_ptr + size_in_bytes_to_copy);
    if (diff <= 0) {
        if (zest__grow_instance_buffer(layer, layer->instance_struct_size, (layer->memory_refs[layer->draw_routine->fif].instance_count * layer->instance_struct_size) + size_in_bytes_to_copy)) {
            instance_ptr = (zest_byte *)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data;
            instance_ptr += layer->memory_refs[layer->draw_routine->fif].instance_count * layer->instance_struct_size;
            diff = (zest_byte *)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end - instance_ptr;
            zest_UpdateInstanceLayerDescriptorInfo(layer);
    ...
    layer->memory_refs[layer->draw_routine->fif].instance_count += amount;
...
void zest_DrawInstanceInstruction(zest_layer layer, zest_uint amount) {
    ...
    zest_byte* instance_ptr = layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest_InitialiseInstanceLayer(zest_layer layer, zest_size type_size, zest_uint instance_pool_size) {
    ...
    layer->intensity = 1;
    layer->push_constants.model = zest_M4(1);
    ...
    for (ZEST_EACH_FIF_i) {
        ...
        if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            ...
            layer->memory_refs[i].write_to_buffer = layer->memory_refs[i].staging_instance_data;
        }
        else {
            layer->memory_refs[i].instance_ptr = layer->memory_refs[i].device_instance_data->data;
            layer->memory_refs[i].write_to_buffer = layer->memory_refs[i].device_instance_data;
        }
        layer->memory_refs[i].instance_count = 0;
    }
    ...
    zest_ForEachFrameInFlight(i) {
        zest_vec_clear(layer->draw_instructions[i]);
        layer->memory_refs[i].write_to_buffer->memory_in_use = 0;
        layer->current_instruction = zest__layer_instruction();
        layer->memory_refs[i].instance_count = 0;
        layer->memory_refs[i].instance_ptr = layer->memory_refs[i].write_to_buffer->data;
    }
}
...
void zest_DrawSprite(zest_layer layer, zest_image image, float x, float y, float r, float sx, float sy, float hx, float hy, zest_uint alignment, float stretch) {
    ...
    zest_sprite_instance_t* sprite = (zest_sprite_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
float zest_DrawMSDFText(zest_layer layer, const char* text, float x, float y, float handle_x, float handle_y, float size, float letter_spacing) {
    ...
    for (int i = 0; i != length; ++i) {
        zest_sprite_instance_t* font_instance = (zest_sprite_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest_DrawBillboard(zest_layer layer, zest_image image, float position[3], zest_uint alignment, float angles[3], float handle[2], float stretch, zest_uint alignment_type, float sx, float sy) {
    ...
    zest_billboard_instance_t* billboard = (zest_billboard_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest_DrawLine(zest_layer layer, float start_point[2], float end_point[2], float width) {
    ...
    zest_shape_instance_t* line = (zest_shape_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest_Draw3DLine(zest_layer layer, float start_point[3], float end_point[3], float width) {
    ...
    zest_line_instance_t* line = (zest_line_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest__initialise_mesh_layer(zest_layer mesh_layer, zest_size vertex_struct_size, zest_size initial_vertex_capacity) {
    ...
    zest_ForEachFrameInFlight(i) {
        zest_vec_clear(mesh_layer->draw_instructions[i]);
        mesh_layer->memory_refs[i].write_to_buffer->memory_in_use = 0;
        mesh_layer->memory_refs[i].write_to_index_buffer->memory_in_use = 0;
        mesh_layer->current_instruction = zest__layer_instruction();
        mesh_layer->memory_refs[i].index_count = 0;
        mesh_layer->memory_refs[i].vertex_count = 0;
        mesh_layer->memory_refs[i].vertex_ptr = mesh_layer->memory_refs[i].write_to_buffer->data;
        mesh_layer->memory_refs[i].index_ptr = mesh_layer->memory_refs[i].write_to_index_buffer->data;
    }
}

void zest_BindMeshVertexBuffer(VkCommandBuffer command_buffer, zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_buffer_t* buffer = layer->memory_refs[layer->draw_routine->fif].device_vertex_data;
    VkDeviceSize offsets[] = { buffer->memory_offset };
    vkCmdBindVertexBuffers(command_buffer, 0, 1, zest_GetBufferDeviceBuffer(buffer), offsets);
}
...
void zest_BindMeshIndexBuffer(VkCommandBuffer command_buffer, zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_buffer_t* buffer = layer->memory_refs[layer->draw_routine->fif].device_index_data;
    vkCmdBindIndexBuffer(command_buffer, *zest_GetBufferDeviceBuffer(buffer), buffer->memory_offset, VK_INDEX_TYPE_UINT32);
}

zest_buffer zest_GetVertexWriteBuffer(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    return layer->memory_refs[layer->draw_routine->fif].write_to_buffer;
}

zest_buffer zest_GetIndexWriteBuffer(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    return layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer;
}

zest_buffer zest_GetVertexDeviceBuffer(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    return layer->memory_refs[layer->draw_routine->fif].device_vertex_data;
}

zest_buffer zest_GetIndexDeviceBuffer(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    return layer->memory_refs[layer->draw_routine->fif].device_index_data;
}

void zest_GrowMeshVertexBuffers(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_size memory_in_use = zest_GetVertexWriteBuffer(layer)->memory_in_use;
    zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].write_to_buffer, layer->vertex_struct_size, memory_in_use);
    zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_vertex_data, layer->vertex_struct_size, memory_in_use);
}

void zest_GrowMeshIndexBuffers(zest_layer layer) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_size memory_in_use = zest_GetVertexWriteBuffer(layer)->memory_in_use;
    zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer, sizeof(zest_uint), memory_in_use);
    zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_index_data, sizeof(zest_uint), memory_in_use);
}

void zest_UploadMeshBuffersCallback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
    zest_layer layer = (zest_layer)draw_routine->draw_data;
    if (draw_routine->last_fif == layer->draw_routine->fif) return;
    zest__end_mesh_instructions(layer);

    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        zest_buffer staging_vertex = layer->memory_refs[layer->draw_routine->fif].write_to_buffer;
        zest_buffer staging_index = layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer;
        zest_buffer device_vertex = layer->memory_refs[layer->draw_routine->fif].device_vertex_data;
        zest_buffer device_index = layer->memory_refs[layer->draw_routine->fif].device_index_data;
...
void zest_PushVertex(zest_layer layer, float pos_x, float pos_y, float pos_z, float intensity, float uv_x, float uv_y, zest_color color, zest_uint parameters) {
    ...
    zest_textured_vertex_t* vertex_ptr = (zest_textured_vertex_t*)layer->memory_refs[layer->draw_routine->fif].vertex_ptr;
    *vertex_ptr = vertex;
    vertex_ptr = vertex_ptr + 1;
    ZEST_ASSERT(vertex_ptr >= (zest_textured_vertex_t*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data && vertex_ptr <= (zest_textured_vertex_t*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end);
    if (vertex_ptr == layer->memory_refs[layer->draw_routine->fif].write_to_buffer->end) {
        zest_bool grown = 0;
        if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            grown = zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_vertex_data, sizeof(zest_textured_vertex_t), 0);
            layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].device_vertex_data;
        }
        else {
            grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].write_to_buffer, sizeof(zest_textured_vertex_t), 0);
            zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_vertex_data, sizeof(zest_textured_vertex_t), 0);
            layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].staging_vertex_data;
        }
        if (grown) {
            layer->memory_refs[layer->draw_routine->fif].vertex_count++;
            vertex_ptr = (zest_textured_vertex_t*)layer->memory_refs[layer->draw_routine->fif].write_to_buffer->data;
            vertex_ptr += layer->memory_refs[layer->draw_routine->fif].vertex_count;
        }
        else {
            vertex_ptr = vertex_ptr - 1;
        }
    ...
    layer->memory_refs[layer->draw_routine->fif].vertex_ptr = vertex_ptr;
}

void zest_PushIndex(zest_layer layer, zest_uint offset) {
    ZEST_CHECK_HANDLE(layer);	//Not a valid handle!
    zest_uint index = layer->memory_refs[layer->draw_routine->fif].vertex_count + offset;
    zest_uint* index_ptr = (zest_uint*)layer->memory_refs[layer->draw_routine->fif].index_ptr;
    *index_ptr = index;
    index_ptr = index_ptr + 1;
    ZEST_ASSERT(index_ptr >= (zest_uint*)layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->data && index_ptr <= (zest_uint*)layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->end);
    if (index_ptr == layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->end) {
        zest_bool grown = 0;
        if (ZEST__FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            grown = zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_index_data, sizeof(zest_uint), 0);
            layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].device_index_data;
        }
        else {
            grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].write_to_buffer, sizeof(zest_uint), 0);
            zest_GrowBuffer(&layer->memory_refs[layer->draw_routine->fif].device_index_data, sizeof(zest_uint), 0);
            layer->memory_refs[layer->draw_routine->fif].write_to_buffer = layer->memory_refs[layer->draw_routine->fif].staging_index_data;
        }
        if (grown) {
            layer->memory_refs[layer->draw_routine->fif].index_count++;
            layer->current_instruction.total_indexes++;
            index_ptr = (zest_uint*)layer->memory_refs[layer->draw_routine->fif].write_to_index_buffer->data;
            index_ptr += layer->memory_refs[layer->draw_routine->fif].index_count;
        }
        else {
            index_ptr = index_ptr - 1;
        }
    ...
    layer->memory_refs[layer->draw_routine->fif].index_ptr = index_ptr;
}
...
void zest_DrawInstancedMesh(zest_layer layer, float pos[3], float rot[3], float scale[3]) {
    ...
    zest_mesh_instance_t* instance = (zest_mesh_instance_t*)layer->memory_refs[layer->draw_routine->fif].instance_ptr;
...
void zest_SetComputeBindlessLayout(zest_compute_builder_t *builder, zest_descriptor_set_layout bindless_layout) {
    ZEST_CHECK_HANDLE(bindless_layout); //Not a valid descriptor set layout!
    builder->bindless_layout = bindless_layout;
}

void zest_AddComputeSetLayout(zest_compute_builder_t *builder, zest_descriptor_set_layout layout) {

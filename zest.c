...
typedef struct zest_image_t {
	...
	zest_buffer allocation;       // The memory allocated for this image (can be NULL for swapchain images)
...
VkResult zest__os_create_window_surface(zest_window window) {
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateWin32SurfaceKHR(ZestDevice->instance, &surface_create_info, &ZestDevice->allocation_callbacks, &window->surface));
    return VK_SUCCESS;
}
...
void zest__os_create_window_surface(zest_window current_window) {
    ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_surface);
    ZEST_VK_ASSERT_RESULT(glfwCreateWindowSurface(ZestDevice->instance, current_window->window_handle, &ZestDevice->allocation_callbacks, &current_window->surface));
}
...
zest_bool zest_Initialise(zest_create_info_t* info) {
    ...
    ZestDevice->allocation_callbacks.pUserData = &ZestDevice->vulkan_memory_info;
...
VkResult zest__create_instance() {
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateInstance(&create_info, &ZestDevice->allocation_callbacks, &ZestDevice->instance));
...
void zest__setup_validation(void) {
    ...
    ZEST_VK_LOG(zest__create_debug_messenger(ZestDevice->instance, &create_info, &ZestDevice->allocation_callbacks, &ZestDevice->debug_messenger));
...
zest_bool zest__is_device_suitable(VkPhysicalDevice physical_device) {
    ...
    if (extensions_supported) {
        zest_swapchain_support_details_t swapchain_support_details = zest__query_swapchain_support(physical_device);
...
zest_swapchain_support_details_t zest__query_swapchain_support(VkPhysicalDevice physical_device) {
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, ZestRenderer->main_window->surface, &details.capabilities);

    vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, ZestRenderer->main_window->surface, &details.formats_count, ZEST_NULL);

    if (details.formats_count != 0) {
        details.formats = ZEST__ALLOCATE(sizeof(VkSurfaceFormatKHR) * details.formats_count);
        vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, ZestRenderer->main_window->surface, &details.formats_count, details.formats);
    }

    vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, ZestRenderer->main_window->surface, &details.present_modes_count, ZEST_NULL);

    if (details.present_modes_count != 0) {
        details.present_modes = ZEST__ALLOCATE(sizeof(VkPresentModeKHR) * details.present_modes_count);
        vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, ZestRenderer->main_window->surface, &details.present_modes_count, details.present_modes);
    }
...
VkResult zest__create_logical_device() {
    ...
    zest_vec_foreach(i, ZestDevice->queue_families) {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(ZestDevice->physical_device, i, ZestRenderer->main_window->surface, &present_support);
    ...
    ZEST_VK_LOG(vkCreateDevice(ZestDevice->physical_device, &create_info, &ZestDevice->allocation_callbacks, &ZestDevice->logical_device));

    if (ZestDevice->logical_device == VK_NULL_HANDLE) {
        return ZEST_FALSE;
    }
    ...
    zest_ForEachFrameInFlight(fif) {
        zest_vec_foreach(i, ZestDevice->queues) {
            ...
            vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &queue->semaphore[fif]);
    ...
    zest_ForEachFrameInFlight(fif) {
        ZEST_VK_ASSERT_RESULT(vkCreateCommandPool(ZestDevice->logical_device, &cmd_info_pool, &ZestDevice->allocation_callbacks, &ZestDevice->one_time_command_pool[fif]));
    }
...
zest_buffer_type_t zest__get_buffer_memory_type(VkBufferUsageFlags usage_flags, VkMemoryPropertyFlags property_flags, zest_size size) {
    ...
    ZEST_VK_LOG(vkCreateBuffer(ZestDevice->logical_device, &buffer_info, &ZestDevice->allocation_callbacks, &temp_buffer));
...
void zest__do_scheduled_tasks(void) {
    ...
    if (zest_vec_size(ZestRenderer->deferred_resource_freeing_list.images[ZEST_FIF])) {
        zest_vec_foreach(i, ZestRenderer->deferred_resource_freeing_list.images[ZEST_FIF]) {
            zest_image_t *image = &ZestRenderer->deferred_resource_freeing_list.images[ZEST_FIF][i];
            zest_FreeBuffer(image->buffer);
            vkDestroyImage(ZestDevice->logical_device, image->vk_image, &ZestDevice->allocation_callbacks);
            vkDestroyImageView(ZestDevice->logical_device, image->vk_view, &ZestDevice->allocation_callbacks);
			zest_vec_foreach(j, image->vk_mip_views) {
				vkDestroyImageView(ZestDevice->logical_device, image->vk_mip_views[j], &ZestDevice->allocation_callbacks);
			}
        }
...
VkResult zest__main_loop_fence_wait() {
	if (ZestRenderer->fence_count[ZEST_FIF]) {
		...
		while(1) {
			VkResult result = vkWaitForFences(ZestDevice->logical_device, ZestRenderer->fence_count[ZEST_FIF], ZestRenderer->fif_fence[ZEST_FIF], VK_TRUE, ZestApp->fence_wait_timeout_ns);
            if (result == VK_SUCCESS) {
                break;
		...
		vkResetFences(ZestDevice->logical_device, ZestRenderer->fence_count[ZEST_FIF], ZestRenderer->fif_fence[ZEST_FIF]);
...
void* zest__allocate(zest_size size) {
    ...
    if (offset_from_allocator == 8670096 && size == 104) {
        int d = 0;
    }
...
VkResult zest__create_device_memory_pool(VkDeviceSize size, zest_buffer_info_t *buffer_info, zest_device_memory_pool memory_pool, const char* name) {
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_buffer);
    ZEST_VK_ASSERT_RESULT(vkCreateBuffer(ZestDevice->logical_device, &create_buffer_info, &ZestDevice->allocation_callbacks, &temp_buffer));
    ...
    ZEST_VK_ASSERT_RESULT(vkAllocateMemory(ZestDevice->logical_device, &alloc_info, &ZestDevice->allocation_callbacks, &memory_pool->memory));
    ...
    memory_pool->property_flags = buffer_info->property_flags;
    memory_pool->usage_flags = buffer_info->usage_flags;
    memory_pool->buffer_info = create_buffer_info;

    if (ZEST__FLAGGED(create_buffer_info.flags, zest_memory_pool_flag_single_buffer)) {
        vkDestroyBuffer(ZestDevice->logical_device, temp_buffer, &ZestDevice->allocation_callbacks);
    } else {
        memory_pool->vk_buffer = temp_buffer;
        vkBindBufferMemory(ZestDevice->logical_device, memory_pool->vk_buffer, memory_pool->memory, 0);
    }
    return VK_SUCCESS;
}
...
VkResult zest__create_image_memory_pool(VkDeviceSize size_in_bytes, VkImage image, zest_buffer_info_t *buffer_info, zest_device_memory_pool buffer) {
    ...
    buffer->property_flags = buffer_info->property_flags;
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_allocate_memory_pool);
    ZEST_VK_ASSERT_RESULT(vkAllocateMemory(ZestDevice->logical_device, &alloc_info, &ZestDevice->allocation_callbacks, &buffer->memory));
...
void zest__remote_merge_next_callback(void *user_data, zloc_header *block, zloc_header *next_block) {
    ...
    next_remote_block->vk_buffer = VK_NULL_HANDLE;
}
...
void zest__remote_merge_prev_callback(void *user_data, zloc_header *prev_block, zloc_header *block) {
    ...
    remote_block->magic = 0;
    remote_block->vk_buffer = VK_NULL_HANDLE;
    prev_remote_block->vk_buffer = VK_NULL_HANDLE;
}
...
void zest__set_buffer_details(zest_buffer_allocator buffer_allocator, zest_buffer buffer, zest_bool is_host_visible) {
    if (ZEST__NOT_FLAGGED(buffer->memory_pool->flags, zest_memory_pool_flag_single_buffer) && buffer->memory_pool->buffer_info.size > 0) {
		...
		buffer->vk_buffer = buffer->memory_pool->vk_buffer;
...
void zest_FlushUsedBuffers() {
    zest_vec_foreach(i, ZestRenderer->used_buffers_ready_for_freeing) {
        ...
        vkDestroyBuffer(ZestDevice->logical_device, buffer, &ZestDevice->allocation_callbacks);
    }
}
        ...
        zest_vec_foreach(j, allocator->range_pools) {
            ...
            while (!zloc__is_last_block_in_pool(current_block)) {
                ...
                if (ZEST_IS_INTITIALISED(remote_block->magic)) {
                    if (remote_block->vk_buffer != VK_NULL_HANDLE && ZEST__NOT_FLAGGED(remote_block->memory_pool->flags, zest_memory_pool_flag_single_buffer)) {
                        vkDestroyBuffer(ZestDevice->logical_device, remote_block->vk_buffer, &ZestDevice->allocation_callbacks);
                    }
                }
...
zest_bool zest_CopyBufferOneTime(zest_buffer src_buffer, zest_buffer dst_buffer, VkDeviceSize size) {
...
void zest_CopyBuffer(const zest_frame_graph_context context, zest_buffer src_buffer, zest_buffer dst_buffer, VkDeviceSize size) {
    ZEST_ASSERT(size <= src_buffer->size);        //size must be less than or equal to the staging buffer size and the device buffer size
    ZEST_ASSERT(size <= dst_buffer->size);
    ...
    copyInfo.dstOffset = dst_buffer->buffer_offset;
    copyInfo.size = size;
    vkCmdCopyBuffer(context->command_buffer, src_buffer->vk_buffer, dst_buffer->vk_buffer, 1, &copyInfo);
}
...
zest_bool zest_UploadBuffer(zest_buffer_uploader_t* uploader, const zest_frame_graph_context context) {
    if (!zest_vec_size(uploader->buffer_copies)) {
        return ZEST_FALSE;
    }

    vkCmdCopyBuffer(context->command_buffer, *zest_GetBufferDeviceBuffer(uploader->source_buffer), *zest_GetBufferDeviceBuffer(uploader->target_buffer), zest_vec_size(uploader->buffer_copies), uploader->buffer_copies);
...
VkResult zest__initialise_renderer(zest_create_info_t* create_info) {
    ...
    zest_AddLayoutBuilderCombinedImageSamplerBindless(&layout_builder, zest_combined_image_sampler_2d_binding, create_info->bindless_combined_sampler_2d_count);
    zest_AddLayoutBuilderCombinedImageSamplerBindless(&layout_builder, zest_combined_image_sampler_array_binding, create_info->bindless_combined_sampler_array_count);
    zest_AddLayoutBuilderCombinedImageSamplerBindless(&layout_builder, zest_combined_image_sampler_cube_binding, create_info->bindless_combined_sampler_cube_count);
    zest_AddLayoutBuilderCombinedImageSamplerBindless(&layout_builder, zest_combined_image_sampler_3d_binding, create_info->bindless_combined_sampler_3d_count);
	...
	for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
		zest_ForEachFrameInFlight(i) {
            ZEST_VK_ASSERT_RESULT(vkCreateFence(ZestDevice->logical_device, &fence_info, &ZestDevice->allocation_callbacks, &ZestRenderer->fif_fence[i][queue_index]));
			ZestRenderer->fence_count[i] = 0;
        }
		ZEST_VK_ASSERT_RESULT(vkCreateFence(ZestDevice->logical_device, &fence_info, &ZestDevice->allocation_callbacks, &ZestRenderer->intraframe_fence[queue_index]));
    }
    ...
    zest_ForEachFrameInFlight(fif) {
        ...
        ZEST_VK_ASSERT_RESULT(vkAllocateCommandBuffers(ZestDevice->logical_device, &alloc_info, &ZestRenderer->utility_command_buffer[fif]));
    }
...
VkResult zest__initialise_swapchain(zest_swapchain swapchain, zest_window window) {
    ZEST_ASSERT_HANDLE(swapchain);   //Not a valid swapchain handle!
	zest_swapchain_support_details_t swapchain_support_details = zest__query_swapchain_support(ZestDevice->physical_device);
    swapchain->window = window;

    VkSurfaceFormatKHR surfaceFormat = zest__choose_swapchain_format(swapchain_support_details.formats);
    VkPresentModeKHR presentMode = zest_choose_present_mode(swapchain_support_details.present_modes, ZestRenderer->flags & zest_renderer_flag_vsync_enabled); 
    VkExtent2D extent = zest_choose_swap_extent(&swapchain_support_details.capabilities);
    ...
    createInfo.surface = ZestRenderer->main_window->surface;
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateSwapchainKHR(ZestDevice->logical_device, &createInfo, &ZestDevice->allocation_callbacks, &swapchain->vk_swapchain));
    ...
    vkGetSwapchainImagesKHR(ZestDevice->logical_device, swapchain->vk_swapchain, &image_count, images);

    zest_vec_resize(swapchain->images, image_count);
    zest_vec_foreach(i, images) {
        swapchain->images[i] = (zest_image_t){ 0 };
        swapchain->images[i].vk_image = images[i];
        swapchain->images[i].vk_format = surfaceFormat.format;
        swapchain->images[i].vk_extent.width = extent.width;
        swapchain->images[i].vk_extent.width = extent.width;
        swapchain->images[i].vk_extent.height = extent.height;
        swapchain->images[i].vk_current_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    }
...
VkResult zest__initialise_swapchain_semaphores(zest_swapchain swapchain) {
    VkSemaphoreCreateInfo semaphore_info = { 0 };
    semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;

    zest_ForEachFrameInFlight(i) {
		ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_semaphore);
        ZEST_VK_ASSERT_RESULT(vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &swapchain->vk_image_available_semaphore[i]));
    }
    ...
    zest_vec_foreach(i, swapchain->vk_render_finished_semaphore) {
		ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_semaphore);
        ZEST_VK_ASSERT_RESULT(vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &swapchain->vk_render_finished_semaphore[i]));
    }
...
VkResult zest__create_pipeline_cache() {
    ...
    ZEST_VK_ASSERT_RESULT(vkCreatePipelineCache(ZestDevice->logical_device, &pipeline_cache_create_info, &ZestDevice->allocation_callbacks, &ZestRenderer->pipeline_cache));
    return VK_SUCCESS;
}
...
void zest__cleanup_swapchain(zest_swapchain swapchain, zest_bool for_recreation) {
    ...
    zest_vec_foreach(i, swapchain->images) {
        vkDestroyImageView(ZestDevice->logical_device, swapchain->images[i].vk_view, &ZestDevice->allocation_callbacks);
    }
...
void zest__destroy_pipeline(zest_pipeline p) {
...
void zest__cleanup_pipelines() {
    zest_map_foreach(i, ZestRenderer->cached_pipelines) {
        ...
        zest__destroy_pipeline(pipeline);
        ZEST__FREE(pipeline);
    }
}
...
void zest__cleanup_device(void) {
    zest_ForEachFrameInFlight(fif) {
        zest_vec_foreach(i, ZestDevice->queues) {
            zest_queue queue = ZestDevice->queues[i];
            vkDestroySemaphore(ZestDevice->logical_device, queue->semaphore[fif], &ZestDevice->allocation_callbacks);
			vkDestroyCommandPool(ZestDevice->logical_device, ZestDevice->queues[i]->command_pool[fif], &ZestDevice->allocation_callbacks);
			zest_vec_free(queue->command_buffers[fif]);
        }
    }
...
void zest__cleanup_shader_resource_store() {
    ...
    for (int i = 0; i != ZestRenderer->shader_resources.current_size; ++i) {
        if (ZEST_VALID_HANDLE(&shader_resources[i])) {
			...
			zest_ForEachFrameInFlight(fif) {
				zest_vec_free(resource->sets[fif]);
			}
			zest_vec_free(resource->binding_sets);
        }
    }
...
void zest__cleanup_texture_store() {
    ...
    for (int i = 0; i != ZestRenderer->textures.current_size; ++i) {
        if (ZEST_VALID_HANDLE(&textures[i])) {
            ...
            zest__cleanup_texture(resource);
        }
    }
    ...
    zest_ForEachFrameInFlight(fif) {
        if (zest_vec_size(ZestRenderer->deferred_resource_freeing_list.images[fif])) {
            zest_vec_foreach(i, ZestRenderer->deferred_resource_freeing_list.images[fif]) {
                zest_image_t *image = &ZestRenderer->deferred_resource_freeing_list.images[fif][i];
                zest_FreeBuffer(image->buffer);
                vkDestroyImage(ZestDevice->logical_device, image->vk_image, &ZestDevice->allocation_callbacks);
                vkDestroyImageView(ZestDevice->logical_device, image->vk_view, &ZestDevice->allocation_callbacks);
				zest_vec_foreach(j, image->vk_mip_views) {
...
VkResult zest__recreate_swapchain(zest_swapchain swapchain) {
    int fb_width = 0, fb_height = 0;
    int window_width = 0, window_height = 0;
    ...
    swapchain->size = (zest_extent_t){ (float)fb_width, (float)fb_height };
    ...
    if (zest__initialise_swapchain(swapchain, swapchain->window) == VK_SUCCESS) {
        zest__initialise_swapchain_semaphores(swapchain);
        zest__create_swapchain_image_views(swapchain);
    }
...
VkResult zest__create_swapchain_image_views(zest_swapchain swapchain) {
    zest_vec_foreach(i, swapchain->images) {
        ZEST_VK_ASSERT_RESULT(zest__create_image_view(swapchain->images[i].vk_image, swapchain->images[i].vk_format, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, VK_IMAGE_VIEW_TYPE_2D_ARRAY, 1, &swapchain->images[i].vk_view));
    }
    return VK_SUCCESS;
}

zest_frame_graph_semaphores zest__get_frame_graph_semaphores(const char *name) {
...
VkResult zest__create_command_buffer_pools() {
    VkCommandBufferAllocateInfo alloc_info = { 0 };
    alloc_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    alloc_info.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    ...
    zest_ForEachFrameInFlight(fif) {
        zest_vec_foreach(i, ZestDevice->queues) {
			...
			ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_command_buffer);
			ZEST_VK_ASSERT_RESULT(vkAllocateCommandBuffers(ZestDevice->logical_device, &alloc_info, queue->command_buffers[fif]));
        }
    }
    return VK_SUCCESS;
}

VkCommandBuffer zest__get_next_command_buffer(zest_queue queue) {
    if (zest_vec_size(queue->command_buffers[ZEST_FIF]) > queue->next_buffer) {
        VkCommandBuffer command_buffer = queue->command_buffers[ZEST_FIF][queue->next_buffer];
        queue->next_buffer++;
		...
		ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_command_buffer);
        VkResult result = vkAllocateCommandBuffers(ZestDevice->logical_device, &alloc_info, &new_command_buffer);
        if (result != VK_SUCCESS) {
...
void zest__reset_queue_command_pool(zest_queue queue) {
    vkResetCommandPool(ZestDevice->logical_device, queue->command_pool[ZEST_FIF], 0);
    queue->next_buffer = 0;
}
...
zest_set_layout_handle zest_FinishDescriptorSetLayout(zest_set_layout_builder_t *builder, const char *name, ...) {
    ...
    ZEST_CLEANUP_ON_FAIL(vkCreateDescriptorSetLayout(ZestDevice->logical_device, &layoutInfo, &ZestDevice->allocation_callbacks, &layout));

    handle = zest__new_descriptor_set_layout(layout_name.str, layout);
...
zest_set_layout_handle zest_FinishDescriptorSetLayoutForBindless(zest_set_layout_builder_t *builder, zest_uint num_global_sets_this_pool_should_support, VkDescriptorPoolCreateFlags additional_pool_flags, const char *name, ...) {
    ...
    handle = zest__new_descriptor_set_layout(layer_name.str, layout);
    zest_set_layout set_layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, handle.value);
    set_layout->binding_indexes = builder->binding_indexes;
    ...
    zest_CreateDescriptorPoolForLayoutBindless(handle, 1, VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT);
...
ZEST_API zest_descriptor_set zest_CreateBindlessSet(zest_set_layout_handle layout_handle) {
    ...
    *set = (zest_descriptor_set_t){ 0 };
    ...
    VkResult result = vkAllocateDescriptorSets(ZestDevice->logical_device, &alloc_info, &set->vk_descriptor_set);
...
zest_set_layout_handle zest__new_descriptor_set_layout(const char *name, VkDescriptorSetLayout layout) {
...
VkDescriptorSetLayout zest_CreateDescriptorSetLayoutWithBindings(zest_uint count, VkDescriptorSetLayoutBinding* bindings) {
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_descriptor_layout);
    VkResult result = vkCreateDescriptorSetLayout(ZestDevice->logical_device, &layoutInfo, &ZestDevice->allocation_callbacks, &layout);
...
void zest_AddSetBuilderTexture(zest_descriptor_set_builder_t *builder, zest_uint dst_binding, zest_uint dst_array_element, zest_texture_handle handle) {
    ...
    image_info.imageView = texture->image.vk_view;
    image_info.imageLayout = texture->image.vk_current_layout;
    zest_AddSetBuilderWrite(builder, dst_binding, dst_array_element, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &image_info, NULL, NULL);
}
...
zest_descriptor_set zest_FinishDescriptorSet(zest_descriptor_pool pool, zest_descriptor_set_builder_t *builder, zest_descriptor_set new_set_to_populate_or_update ) {
    ...
    if (target_set == VK_NULL_HANDLE) {
        ...
        allocation_info.pSetLayouts = &(associated_layout->vk_layout); // Assuming your layout struct holds the Vk handle

        VkResult result = vkAllocateDescriptorSets(ZestDevice->logical_device, &allocation_info, &target_set);
        if (result != VK_SUCCESS) {
            ZEST_VK_PRINT_RESULT(result);
...
void zest_CreateDescriptorPoolForLayout(zest_set_layout_handle layout_handle, zest_uint max_set_count, VkDescriptorPoolCreateFlags pool_flags) {
    zest_set_layout layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, layout_handle.value);
    ...
    VkResult result = vkCreateDescriptorPool(ZestDevice->logical_device, &pool_create_info, &ZestDevice->allocation_callbacks, &pool->vk_descriptor_pool);

    if (result != VK_SUCCESS) {
        pool->vk_descriptor_pool = VK_NULL_HANDLE;
        ZEST_VK_PRINT_RESULT(result);
    }

    zest_vec_free(vk_pool_sizes);
    zest_map_free(type_counts);
    layout->pool = pool;
}

void zest_CreateDescriptorPoolForLayoutBindless(zest_set_layout_handle layout_handle, zest_uint max_set_count, VkDescriptorPoolCreateFlags pool_flags) {
    zest_set_layout layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, layout_handle.value);
    ZEST_ASSERT(max_set_count > 0); //Must set a set count for the pool
...
void zest_BindPipelineShaderResource(const zest_frame_graph_context context, zest_pipeline pipeline, zest_shader_resources_handle handle) {
    zest_shader_resources shader_resources = (zest_shader_resources)zest__get_store_resource_checked(&ZestRenderer->shader_resources, handle.value);
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    ...
    vkCmdBindPipeline(context->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline);
    vkCmdBindDescriptorSets(context->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline_layout, 0, zest_vec_size(shader_resources->binding_sets), shader_resources->binding_sets, 0, 0);
    zest_vec_clear(shader_resources->binding_sets);
}

void zest_BindPipeline(const zest_frame_graph_context context, zest_pipeline_t* pipeline, VkDescriptorSet *descriptor_set, zest_uint set_count) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    ZEST_ASSERT(set_count && descriptor_set);    //No descriptor sets. Must bind the pipeline with a valid desriptor set
    vkCmdBindPipeline(context->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline);
    vkCmdBindDescriptorSets(context->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline_layout, 0, set_count, descriptor_set, 0, 0);
}

void zest_BindComputePipeline(const zest_frame_graph_context context, zest_compute_handle compute_handle, VkDescriptorSet *descriptor_set, zest_uint set_count) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    zest_compute compute = (zest_compute)zest__get_store_resource_checked(&ZestRenderer->compute_pipelines, compute_handle.value);
    ZEST_ASSERT(set_count && descriptor_set);   //No descriptor sets. Must bind the pipeline with a valid desriptor set
    vkCmdBindPipeline(context->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute->pipeline);
    vkCmdBindDescriptorSets(context->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute->pipeline_layout, 0, set_count, descriptor_set, 0, 0);
}
...
VkResult zest__cache_pipeline(zest_pipeline_template pipeline_template, zest_render_pass render_pass, zest_key cached_pipeline_key, zest_pipeline *out_pipeline) {
    ...
    pipeline->render_pass = render_pass->vk_render_pass;
...
VkResult zest__build_pipeline(zest_pipeline pipeline) {
    ...
    VkResult result = vkCreatePipelineLayout(ZestDevice->logical_device, &pipeline->pipeline_template->pipelineLayoutInfo, &ZestDevice->allocation_callbacks, &pipeline->pipeline_layout);
    if (result != VK_SUCCESS) {
        ZEST_VK_PRINT_RESULT(result);
        return result;
    }
    ...
    result = vkCreateGraphicsPipelines(ZestDevice->logical_device, ZestRenderer->pipeline_cache, 1, &pipeline_info, &ZestDevice->allocation_callbacks, &pipeline->pipeline);
    if (result != VK_SUCCESS) {
        ZEST_VK_PRINT_RESULT(result);
    } else {
...
VkResult zest__create_shader_module(char *code, VkShaderModule *shader_module) {
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateShaderModule(ZestDevice->logical_device, &create_info, &ZestDevice->allocation_callbacks, shader_module));
...
VkResult zest__create_queue_command_pool(int queue_family_index, VkCommandPool *command_pool) {
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_device, zest_vk_command_pool);
    ZEST_VK_ASSERT_RESULT(vkCreateCommandPool(ZestDevice->logical_device, &cmd_info_pool, &ZestDevice->allocation_callbacks, command_pool));
    return VK_SUCCESS;
}
...
zest_pipeline zest_PipelineWithTemplate(zest_pipeline_template pipeline_template, const zest_frame_graph_context context) {
    ...
    pipeline_template->multisampling.rasterizationSamples = (VkSampleCountFlagBits)context->render_pass->sample_count;
    zest_key pipeline_key = (zest_key)pipeline_template;
    zest_cached_pipeline_key_t cached_pipeline = { pipeline_key, context->render_pass->vk_render_pass };
...
zest_extent_t zest_GetSwapChainExtent() { return ZestRenderer->main_swapchain->size; }
zest_extent_t zest_GetWindowExtent(void) { return ZestRenderer->window_extent; }
...
float zest_SwapChainWidthf(void) { return ZestRenderer->main_swapchain->size.width; }
float zest_SwapChainHeightf(void) { return ZestRenderer->main_swapchain->size.height; }
...
zest_window zest_AllocateWindow() { zest_window window; window = ZEST__NEW(zest_window); memset(window, 0, sizeof(zest_window_t)); window->magic = zest_INIT_MAGIC(zest_struct_type_window); return window; }
...
void zest_BindVertexBuffer(const zest_frame_graph_context context, zest_buffer buffer) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    VkDeviceSize offsets[] = { buffer->buffer_offset };
    vkCmdBindVertexBuffers(context->command_buffer, 0, 1, zest_GetBufferDeviceBuffer(buffer), offsets);
}

void zest_BindIndexBuffer(const zest_frame_graph_context context, zest_buffer buffer) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdBindIndexBuffer(context->command_buffer, *zest_GetBufferDeviceBuffer(buffer), buffer->buffer_offset, VK_INDEX_TYPE_UINT32);
}
...
void zest_SendPushConstants(const zest_frame_graph_context context, zest_pipeline pipeline, void *data) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdPushConstants(context->command_buffer, pipeline->pipeline_layout, pipeline->pipeline_template->pushConstantRange.stageFlags, pipeline->pipeline_template->pushConstantRange.offset, pipeline->pipeline_template->pushConstantRange.size, data);
}

void zest_SendCustomComputePushConstants(const zest_frame_graph_context context, zest_compute_handle compute_handle, const void *push_constant) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    zest_compute compute = (zest_compute)zest__get_store_resource_checked(&ZestRenderer->compute_pipelines, compute_handle.value);
    vkCmdPushConstants(context->command_buffer, compute->pipeline_layout, compute->pushConstantRange.stageFlags, 0, compute->pushConstantRange.size, push_constant);
}

void zest_SendStandardPushConstants(const zest_frame_graph_context context, zest_pipeline_t* pipeline, void* data) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdPushConstants(context->command_buffer, pipeline->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(zest_push_constants_t), data);
}

void zest_Draw(const zest_frame_graph_context context, zest_uint vertex_count, zest_uint instance_count, zest_uint first_vertex, zest_uint first_instance) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdDraw(context->command_buffer, vertex_count, instance_count, first_vertex, first_instance);
}

void zest_DrawLayerInstruction(const zest_frame_graph_context context, zest_uint vertex_count, zest_layer_instruction_t *instruction) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdDraw(context->command_buffer, vertex_count, instruction->total_indexes, 0, instruction->start_index);
}

void zest_DrawIndexed(const zest_frame_graph_context context, zest_uint index_count, zest_uint instance_count, zest_uint first_index, int32_t vertex_offset, zest_uint first_instance) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdDrawIndexed(context->command_buffer, index_count, instance_count, first_index, vertex_offset, first_instance);
}

void zest_DispatchCompute(const zest_frame_graph_context context, zest_compute_handle compute_handle, zest_uint group_count_x, zest_uint group_count_y, zest_uint group_count_z) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    vkCmdDispatch(context->command_buffer, group_count_x, group_count_y, group_count_z);
}
...
VkSamplerCreateInfo zest_CreateSamplerInfo() {
    VkSamplerCreateInfo vk_sampler_info = { 0 };
    vk_sampler_info.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
    vk_sampler_info.magFilter = VK_FILTER_LINEAR;
    vk_sampler_info.minFilter = VK_FILTER_LINEAR;
    vk_sampler_info.addressModeU = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    vk_sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    vk_sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    vk_sampler_info.anisotropyEnable = VK_FALSE;
    vk_sampler_info.maxAnisotropy = 1.f;
    vk_sampler_info.unnormalizedCoordinates = VK_FALSE;
    vk_sampler_info.compareEnable = VK_FALSE;
    vk_sampler_info.compareOp = VK_COMPARE_OP_ALWAYS;
    vk_sampler_info.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
    vk_sampler_info.borderColor = VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK;
    vk_sampler_info.mipLodBias = 0.f;
    vk_sampler_info.minLod = 0.0f;
    vk_sampler_info.maxLod = 1.0f;
    vk_sampler_info.pNext = VK_NULL_HANDLE;
    vk_sampler_info.flags = 0;
...
VkSamplerCreateInfo zest_CreateMippedSamplerInfo(zest_uint mip_levels) {
    ...
    vk_sampler_info.mipmapMode = VK_SAMPLER_MIPMAP_MODE_NEAREST;
    vk_sampler_info.maxLod = (float)mip_levels - 1.f;
    return vk_sampler_info;
}
...
void zest__prepare_standard_pipelines() {
    ...
    zest_AddPipelineDescriptorLayout(fonts, ZestRenderer->uniform_buffer->set_layout->vk_layout);
    zest_AddPipelineDescriptorLayout(fonts, ZestRenderer->global_bindless_set_layout->vk_layout);
...
VkResult zest__dummy_submit_for_present_only(void) {
    ...
    image_barrier.image = swapchain->images[swapchain->current_image_frame].vk_image;
...
void zest_SetScreenSizedViewport(const zest_frame_graph_context context, float min_depth, float max_depth) {
...
void zest_cmd_Scissor(const zest_frame_graph_context context, zest_scissor_rect_t *scissor) {
	...
	vkCmdSetScissor(context->command_buffer, 0, 1, &rect);
}
...
void zest_Clip(const zest_frame_graph_context context, float x, float y, float width, float height, float minDepth, float maxDepth) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
	...
	vkCmdSetViewport(context->command_buffer, 0, 1, &view);
	vkCmdSetScissor(context->command_buffer, 0, 1, &scissor);
}

VkResult zest__create_image_view(VkImage image, VkFormat format, VkImageAspectFlags aspect_flags, zest_uint mip_levels_this_view, zest_uint base_mip, VkImageViewType view_type, zest_uint layer_count, VkImageView *image_view) {
    ...
    return vkCreateImageView(ZestDevice->logical_device, &viewInfo, &ZestDevice->allocation_callbacks, image_view);
}
...
VkResult zest__create_transient_image(zest_resource_node resource) {
    VkImageCreateInfo image_info = { 0 };
    image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    image_info.imageType = VK_IMAGE_TYPE_2D;
    image_info.extent.width = resource->image_desc.width;
    image_info.extent.height = resource->image_desc.height;
	...
	ZEST__MAYBE_FLAG(resource->image_desc.usage, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT, 
        ...
        ZEST__NOT_FLAGGED(resource->image_desc.usage, ZEST_TRANSIENT_NON_COMPATIBLE_FLAGS));
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateImage(ZestDevice->logical_device, &image_info, &ZestDevice->allocation_callbacks, &resource->image.vk_image));

    VkMemoryRequirements memory_requirements;
    vkGetImageMemoryRequirements(ZestDevice->logical_device, resource->image.vk_image, &memory_requirements);

    zest_buffer_info_t buffer_info = { 0 };
    buffer_info.image_usage_flags = resource->image_desc.usage;
    buffer_info.property_flags = resource->image_desc.properties;
	buffer_info.memory_type_bits = memory_requirements.memoryTypeBits;
	buffer_info.alignment = memory_requirements.alignment;
    buffer_info.frame_in_flight = ZEST_FIF;
    resource->image.buffer = zest_CreateBuffer(memory_requirements.size, &buffer_info, resource->image.vk_image);
    resource->image.vk_format = image_info.format;
    if (image_info.mipLevels > 1) {
        zest_vec_linear_resize(ZestRenderer->frame_graph_allocator[ZEST_FIF], resource->image.vk_mip_views, image_info.mipLevels);
        memset(resource->image.vk_mip_views, 0, sizeof(VkImageView) * image_info.mipLevels);
    }

    vkBindImageMemory(ZestDevice->logical_device, resource->image.vk_image, zest_GetBufferDeviceMemory(resource->image.buffer), resource->image.buffer->memory_offset);
    return VK_SUCCESS;
}
...
VkResult zest__create_temporary_image(zest_uint width, zest_uint height, zest_uint mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage* image, VkDeviceMemory* memory) {
    ...
    ZEST_VK_ASSERT_RESULT(vkAllocateMemory(ZestDevice->logical_device, &alloc_info, &ZestDevice->allocation_callbacks, memory));
...
VkResult zest__create_image(zest_uint width, zest_uint height, zest_uint mip_levels, VkSampleCountFlagBits num_samples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, zest_image image_handles) {
    VkImageCreateInfo image_info = { 0 };
    image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    image_info.imageType = VK_IMAGE_TYPE_2D;
    image_info.extent.width = width;
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_image);
    ZEST_VK_ASSERT_RESULT(vkCreateImage(ZestDevice->logical_device, &image_info, &ZestDevice->allocation_callbacks, &image_handles->vk_image));

    VkMemoryRequirements memory_requirements;
    vkGetImageMemoryRequirements(ZestDevice->logical_device, image_handles->vk_image, &memory_requirements);
	...
	buffer_info.memory_type_bits = memory_requirements.memoryTypeBits;
	buffer_info.alignment = memory_requirements.alignment;
    image_handles->buffer = zest_CreateBuffer(memory_requirements.size, &buffer_info, image_handles->vk_image);

    if (image_handles->buffer) {
        vkBindImageMemory(ZestDevice->logical_device, image_handles->vk_image, zest_GetBufferDeviceMemory(image_handles->buffer), image_handles->buffer->memory_offset);
    } else {
...
VkResult zest__copy_buffer_to_image(VkBuffer buffer, VkDeviceSize src_offset, VkImage image, zest_uint width, zest_uint height, VkImageLayout vk_image_layout, VkCommandBuffer cb) {
    VkCommandBuffer command_buffer = cb; 
    if (!cb) {
        ZEST_VK_ASSERT_RESULT(zest__begin_single_time_commands(&command_buffer));
    }
    ...
    vkCmdCopyBufferToImage(command_buffer, buffer, image, vk_image_layout, 1, &region);

    if (!cb) {
        ZEST_VK_ASSERT_RESULT(zest__end_single_time_commands(command_buffer));
    }
    return VK_SUCCESS;
}

VkResult zest__copy_buffer_regions_to_image(VkBufferImageCopy* regions, VkBuffer buffer, VkDeviceSize src_offset, VkImage image, VkCommandBuffer cb) {
    VkCommandBuffer command_buffer = cb; 
    if (!cb) {
        ZEST_VK_ASSERT_RESULT(zest__begin_single_time_commands(&command_buffer));
    }
...
VkResult zest__generate_mipmaps(VkImage image, VkFormat vk_image_format, int32_t texture_width, int32_t texture_height, zest_uint mip_levels, zest_uint layer_count, VkImageLayout vk_image_layout, VkCommandBuffer cb) {
    VkFormatProperties format_properties;
    vkGetPhysicalDeviceFormatProperties(ZestDevice->physical_device, vk_image_format, &format_properties);
...
VkImageMemoryBarrier zest__create_image_memory_barrier(VkImage image, VkAccessFlags from_access, VkAccessFlags to_access, VkImageLayout from_layout, VkImageLayout to_layout, VkImageAspectFlags aspect_flags, zest_uint target_mip_level, zest_uint mip_count) {
    VkImageMemoryBarrier barrier = { 0 };
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
...
VkResult zest__transition_image_layout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, zest_uint mip_levels, zest_uint layerCount, VkCommandBuffer cb) {
    VkCommandBuffer command_buffer = cb;
    if (!cb) {
        ZEST_VK_ASSERT_RESULT(zest__begin_single_time_commands(&command_buffer));
    }
...
VkResult zest__begin_single_time_commands(VkCommandBuffer *command_buffer) {
    VkCommandBufferAllocateInfo alloc_info = { 0 };
    alloc_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    alloc_info.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_command_buffer);
    ZEST_VK_ASSERT_RESULT(vkAllocateCommandBuffers(ZestDevice->logical_device, &alloc_info, command_buffer));

    VkCommandBufferBeginInfo begin_info = { 0 };
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    begin_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;

    ZEST_VK_ASSERT_RESULT(vkBeginCommandBuffer(*command_buffer, &begin_info));

    return VK_SUCCESS;
}

VkResult zest__end_single_time_commands(VkCommandBuffer command_buffer) {
    ZEST_VK_ASSERT_RESULT(vkEndCommandBuffer(command_buffer));
...
void zest_UpdateWindowSize(zest_window window, zest_uint width, zest_uint height) {
 ...
 VkSurfaceKHR zest_WindowSurface() {
     return ZestRenderer->main_window->surface;
}

void zest_SetWindowSurface(VkSurfaceKHR surface) {
     ZestRenderer->main_window->surface = surface;
}
...
zest_frame_graph zest__new_frame_graph(const char *name) {
    ...
    *frame_graph = (zest_frame_graph_t){ 0 };
    ...
    /*
    frame_graph->timestamp_count = zest_vec_size(frame_graph->draw_commands) * 2;
    if (!frame_graph->query_pool) {
        frame_graph->query_pool = zest__create_query_pool(frame_graph->timestamp_count);
    }
    zest_vec_resize(frame_graph->timestamps[0], frame_graph->timestamp_count / 2);
    zest_vec_resize(frame_graph->timestamps[1], frame_graph->timestamp_count / 2);
    */
    return frame_graph;
}
...
zest_image zest__swapchain_resource_provider(zest_resource_node resource) {
    return &resource->swapchain->images[resource->swapchain->current_image_frame];
}
...
bool zest_BeginFrameGraph(const char *name, zest_frame_graph_cache_key_t *cache_key) {
    ...
    frame_graph->semaphores = zest__get_frame_graph_semaphores(name);
...
zest_bool zest__create_transient_resource(zest_resource_node resource) {
    if (resource->type & zest_resource_type_is_image) {
        if (zest__create_transient_image(resource) != VK_SUCCESS) {
            return ZEST_FALSE;
        }

        ZEST_RETURN_FALSE_ON_FAIL(zest__create_image_view(
...
void zest__add_image_barriers(zest_frame_graph frame_graph, zloc_linear_allocator_t *allocator, zest_resource_node resource, zest_execution_barriers_t *barriers, 
    ...
    VkImage image = resource->image.vk_image;
    if (!prev_state) {
        ...
        if (needs_acquiring) {
            ...
            zest__add_image_barrier(resource, barriers, true,
                ...
                prev_usage->vk_image_layout, current_usage->vk_image_layout,
                src_queue_family_index, dst_queue_family_index,
    ...
    if (next_state) {
        ...
        if ((needs_releasing || current_usage->vk_image_layout != next_usage->vk_image_layout ||
            ...
            zest__add_image_barrier(resource, barriers, false,
                ...
                current_usage->vk_image_layout, next_usage->vk_image_layout,
                src_queue_family_index, dst_queue_family_index,
        ...
        zest_map_foreach(j, pass_node->outputs) {
            ...
            if (latest_version->id != resource->id) {
                ...
                if (!zest_map_valid_name(pass_node->inputs, resource->name)) {
                    ...
                    switch (pass_node->queue_info.queue_type) {
                    case zest_queue_graphics:
                        input_usage = zest__configure_image_usage(resource, zest_purpose_color_attachment_read, resource->image_desc.format, output_usage->load_op, output_usage->stencil_load_op, output_usage->stage_mask);
                        break;
                    case zest_queue_compute:
                    case zest_queue_transfer:
                        input_usage = zest__configure_image_usage(resource, zest_purpose_storage_image_read, resource->image_desc.format, output_usage->load_op, output_usage->stencil_load_op, output_usage->stage_mask);
                        break;
                    }
            ...
            if (first_batch_to_wait) {
                ...
                for (int queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
                    if (actual_first_wave->batches[queue_index].magic) {
                        ...
                        if (ZestDevice->queue_families[actual_first_wave->batches[queue_index].queue_family_index].queueFlags & VK_QUEUE_TRANSFER_BIT) {
                        ...
                        } else if (ZestDevice->queue_families[actual_first_wave->batches[queue_index].queue_family_index].queueFlags & VK_QUEUE_COMPUTE_BIT) {
    ...
    zest_vec_foreach(submission_index, frame_graph->submissions) {
        ...
        for (int queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            zest_vec_foreach(batch_index, submission->batches[queue_index].pass_indices) {
                ...
                //Create_render_passes
                frame_graph->error_status = zest__create_rg_render_pass(pass, exe_details, current_pass_index);
...
void zest__interpret_hints(zest_resource_node resource, zest_resource_usage_hint usage_hints) {
    if (usage_hints & zest_resource_usage_hint_copy_dst) {
        resource->image_desc.usage |= VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    }
    if (usage_hints & zest_resource_usage_hint_copy_src) {
        resource->image_desc.usage |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
    }
    if (usage_hints & zest_resource_usage_hint_cpu_transfer) {
        resource->image_desc.properties |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    }
}

VkImageLayout zest__determine_final_layout(zest_uint pass_index, zest_resource_node node, zest_resource_usage_t *current_usage) {
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    ZEST_ASSERT_HANDLE(frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_vec_foreach(state_index, node->journey) {
        ...
        if (state->pass_index == pass_index && (zest_uint)state_index < zest_vec_size(node->journey) - 1) {
            return node->journey[state_index + 1].usage.vk_image_layout;
        }
    }
...
zest_bool zest__execute_frame_graph(zest_bool is_intraframe) {
	...
	zest_vec_foreach(resource_index, frame_graph->resources_to_update) {
		...
		if (resource->buffer_provider) {
            ...
            zest_image image = resource->image_provider(resource);
			resource->image.vk_image = image->vk_image;
			resource->image.vk_view = image->vk_view;
		}
	}
    ...
    zest_vec_foreach(submission_index, frame_graph->submissions) {
        ...
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            ...
            case zest_queue_graphics:
                command_buffer = zest__get_next_command_buffer(batch->queue);
                timeline_wait_stage = VK_PIPELINE_STAGE_VERTEX_INPUT_BIT;
                break;
            case zest_queue_compute:
                command_buffer = zest__get_next_command_buffer(batch->queue);
            ...
            if (!command_buffer) {
                frame_graph->error_status |= zest_fgs_unable_to_acquire_command_buffer;
                return ZEST_FALSE;
            }

			VkSemaphore batch_semaphore = frame_graph->semaphores->vk_semaphores[ZEST_FIF][queue_index];
			zest_size *batch_value = &frame_graph->semaphores->values[ZEST_FIF][queue_index];

            VkCommandBufferBeginInfo begin_info = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
            begin_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
            ZEST_RETURN_FALSE_ON_FAIL(vkBeginCommandBuffer(command_buffer, &begin_info));
            frame_graph->context.command_buffer = command_buffer;
            ...
            zest_vec_foreach(i, batch->pass_indices) {
                ...
                if (zest_vec_size(exe_details->barriers.acquire_buffer_barriers) > 0 ||
                    ...
                    zest_vec_foreach(resource_index, exe_details->barriers.acquire_image_barriers) {
                        ...
                        barrier->image = resource->image.vk_image;
                ...
                if (has_render_pass) {
                    ...
                    zest_map_foreach(i, exe_details->attachment_indexes) {
                        ...
                        zest_vec_linear_push(allocator, image_views, node->image.vk_view);
                    }

                    VkFramebufferCreateInfo fb_create_info = { 0 };
                    fb_create_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
                    fb_create_info.renderPass = exe_details->render_pass->vk_render_pass;
                    ...
                    if (zest_vec_size(exe_details->color_attachment_info)) {
                        fb_create_info.width = exe_details->color_attachment_info[0].resource_node->image_desc.width;
                        fb_create_info.height = exe_details->color_attachment_info[0].resource_node->image_desc.height;
                    } else {
                        fb_create_info.width = exe_details->depth_attachment_info.resource_node->image_desc.width;
                        fb_create_info.height = exe_details->depth_attachment_info.resource_node->image_desc.height;
                    }
                    ...
                    render_pass_info.renderPass = exe_details->render_pass->vk_render_pass;
                ...
                if (zest_vec_size(exe_details->barriers.release_buffer_barriers) > 0 ||
                    ...
                    zest_vec_foreach(resource_index, exe_details->barriers.release_buffer_barriers) {
                        VkBufferMemoryBarrier *barrier = &exe_details->barriers.release_buffer_barriers[resource_index];
            ...
            ZEST_RETURN_FALSE_ON_FAIL(vkQueueSubmit(batch->queue->vk_queue, 1, &submit_info, submit_fence));
        ...
        zest_vec_clear(wave_wait_semaphores);
        zest_vec_clear(wave_wait_values);
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            if (wave_submission->batches[queue_index].magic) {
                zest_vec_linear_push(allocator, wave_wait_semaphores, frame_graph->semaphores->vk_semaphores[ZEST_FIF][queue_index]);
                zest_vec_linear_push(allocator, wave_wait_values, frame_graph->semaphores->values[ZEST_FIF][queue_index]);
            }
		}
    }   //Wave 
    ...
    if (is_intraframe && *fence_count > 0) {
		...
		ZEST_RETURN_FALSE_ON_FAIL(vkWaitForFences(ZestDevice->logical_device, *fence_count, fence, VK_TRUE, UINT64_MAX));
    ...
    zest_bucket_array_foreach(resource_index, frame_graph->resources) {
        ...
        if (resource->type == zest_resource_type_buffer) {
            ...
            ZEST_PRINT("Image: %s - VkImage: %p, Size: %u x %u", 
                resource->name, resource->image.vk_image,  
    ...
    zest_vec_foreach(submission_index, frame_graph->submissions) {
        ...
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            ...
            if (zest_map_valid_key(ZestDevice->queue_names, batch->queue_family_index)) {
                ZEST_PRINT("  Target Queue Family: %s - index: %u (VkQueue: %p, command buffer: %p)", *zest_map_at_key(ZestDevice->queue_names, batch->queue_family_index), batch->queue_family_index, (void *)batch->queue->vk_queue, batch->command_buffer);
            } else {
                ZEST_PRINT("  Target Queue Family: %s - index: %u (VkQueue: %p, command buffer: %p)", "Ignored", batch->queue_family_index, (void *)batch->queue->vk_queue, batch->command_buffer);
            }
            ...
            zest_vec_foreach(batch_pass_index, batch->pass_indices) {
                ...
                if (ZEST_VALID_HANDLE(exe_details->render_pass)) {
                    ZEST_PRINT("      VkRenderPass: %p, VkFramebuffer: %p, RenderArea: (%d,%d)-(%ux%u)",
                        (void *)exe_details->render_pass->vk_render_pass, (void *)exe_details->frame_buffer,
...
zest_uint zest__get_image_binding_number(zest_resource_node resource, bool image_view_only) {
    ...
    if (!ZEST_VALID_HANDLE(bindless_layout)) {
        ZEST_PRINT("ERROR: frame graph %s doesn't have a bindless layout assigned, but you're tring to add a resource that uses a binding number.", frame_graph->name);
        return ZEST_INVALID;
    }

    ZEST_ASSERT(resource->type & zest_resource_type_is_image);   //resource must be a image
    zest_uint binding_number = ZEST_INVALID;
	zest_vec_foreach(i, bindless_layout->layout_bindings) {
		VkDescriptorSetLayoutBinding *binding = &bindless_layout->layout_bindings[i];
...
zest_uint zest_AcquireBindlessTextureIndex(zest_texture_handle handle, zest_set_layout_handle layout_handle, zest_descriptor_set set, zest_global_binding_number target_binding_number) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_set_layout layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, layout_handle.value);
    ZEST_ASSERT_HANDLE(texture->sampler);   //The texture must have a valid sampler
    ...
    if (array_index == ZEST_INVALID) {
        ...
        ZEST__REPORT(zest_report_bindless_indexes, "Ran out of space in the descriptor pool when trying to acquire an index for texture [%s], binding number %i.", texture->name.str, binding_number);
        return ZEST_INVALID;
    }
    texture->bindless_set = set;
    texture->bindless_index[binding_number] = array_index;
...
zest_uint zest_AcquireGlobalCombinedSampler2d(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->bindless_index[zest_combined_image_sampler_2d_binding] = zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_2d_binding);
    return texture->bindless_index[zest_combined_image_sampler_2d_binding];
}

zest_uint zest_AcquireGlobalCombinedSampler3d(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->bindless_index[zest_combined_image_sampler_3d_binding] = zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_3d_binding);
    return texture->bindless_index[zest_combined_image_sampler_3d_binding];
}

zest_uint zest_AcquireGlobalCombinedSamplerCube(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->bindless_index[zest_combined_image_sampler_cube_binding] = zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_cube_binding);
    return texture->bindless_index[zest_combined_image_sampler_cube_binding];
}

zest_uint zest_AcquireGlobalCombinedSamplerArray(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->bindless_index[zest_combined_image_sampler_array_binding] = zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_array_binding);
    return texture->bindless_index[zest_combined_image_sampler_array_binding];
}

zest_uint zest_AcquireGlobalSampledImage(zest_texture_handle handle) {
    return zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_sampled_image_binding);
}

zest_uint zest_AcquireGlobalSampler(zest_texture_handle handle) {
    return zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_sampler_binding);
}

zest_uint zest_AcquireGlobalStorageSampler(zest_texture_handle handle) {
    return zest_AcquireBindlessTextureIndex(handle, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_storage_image_binding);
}

zest_uint *zest_AcquireGlobalTextureMipIndexes(zest_texture_handle handle, zest_global_binding_number binding_number) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT(texture->info.mip_levels > 1);   //The resource does not have any mip levels. Make sure to set the number of mip levels when creating the resource in the frame graph

    if (zest_map_valid_key(texture->image.mip_indexes, (zest_key)binding_number)) {
        zest_mip_index_collection *mip_collection = zest_map_at_key(texture->image.mip_indexes, (zest_key)binding_number);
        return mip_collection->mip_indexes;
    }
    VkImageView *mip_views = texture->image.vk_mip_views;
    if (!mip_views) {
        zest_vec_resize(texture->image.vk_mip_views, texture->info.mip_levels);
        mip_views = texture->image.vk_mip_views;
    ...
    for (int mip_index = 0; mip_index != texture->info.mip_levels; ++mip_index) {
        VkImageView mip_view = VK_NULL_HANDLE;
        if (mip_views[mip_index]) {
            mip_view = mip_views[mip_index];
        } else {
            VkResult result = zest__create_image_view(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_ASPECT_COLOR_BIT, 1, mip_index, texture->image.vk_view_type, texture->info.layer_count, &mip_view);
            mip_views[mip_index] = result == VK_SUCCESS ? mip_view : VK_NULL_HANDLE;
        }
        zest_uint bindless_index = zest__acquire_bindless_index(global_layout, binding_number);

        VkDescriptorImageInfo mip_buffer_info;
        mip_buffer_info.imageLayout = texture->image.vk_current_layout;
        mip_buffer_info.imageView = mip_view;
        mip_buffer_info.sampler = texture->sampler->vk_sampler;

        VkWriteDescriptorSet write = zest_CreateImageDescriptorWriteWithType(ZestRenderer->global_set->vk_descriptor_set, &mip_buffer_info, binding_number, global_layout->descriptor_indexes[binding_number].descriptor_type);
        write.dstArrayElement = bindless_index;
        vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);

        zest_vec_push(mip_collection.mip_indexes, bindless_index);
    }
    zest_map_insert_key( texture->image.mip_indexes, (zest_key)binding_number, mip_collection);
    zest_uint size = zest_map_size(texture->image.mip_indexes);
    return mip_collection.mip_indexes;
}
...
void zest_ReleaseGlobalTextureIndex(zest_texture_handle handle, zest_global_binding_number binding_number) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT(binding_number < zest_max_global_binding_number);
    if (texture->bindless_index[binding_number] != ZEST_INVALID) {
        zest__release_bindless_index(ZestRenderer->global_bindless_set_layout, binding_number, texture->bindless_index[binding_number]);
    }
}

void zest_ReleaseAllGlobalTextureIndexes(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest__release_all_global_texture_indexes(texture);
}
...
VkDescriptorSetLayout zest_vk_GetDebugLayout() {
    ...
    return set_layout->vk_layout;
}

VkDescriptorSetLayout zest_vk_GetGlobalBindlessLayout() {
    zest_set_layout global_layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, ZestRenderer->global_bindless_set_layout.value);
    return global_layout->vk_layout;
}
...
zest_resource_node zest_ImportImageResource(const char *name, zest_texture_handle handle, zest_resource_image_provider provider) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
...
zest_resource_node zest__import_swapchain_resource(zest_swapchain swapchain) {
    ...
    node.frame_graph = frame_graph;
    node.magic = zest_INIT_MAGIC(zest_struct_type_resource_node);
    node.image.vk_format = (VkFormat)swapchain->format;
    node.image_desc.width = (zest_uint)swapchain->size.width;
...
zest_resource_versions_t *zest__maybe_add_resource_version(zest_resource_node resource) {
    ...
    if (ZEST__FLAGGED(last_version->flags, zest_resource_node_flag_has_producer)) {
		...
		new_resource.image_desc = resource->image_desc;
...
zest_uint zest_GetTransientImageBindlessIndex(const zest_frame_graph_context context, zest_resource_node resource, zest_global_binding_number binding_number) {
    ...
    VkDescriptorType descriptor_type = bindless_layout->descriptor_indexes[binding_number].descriptor_type;
    ZEST_ASSERT(resource->current_state_index < zest_vec_size(resource->journey));
    ZEST_ASSERT(ZEST_VALID_HANDLE(resource->sampler));
	image_buffer_info.imageLayout = resource->journey[resource->current_state_index].usage.vk_image_layout;
	image_buffer_info.imageView = resource->image.vk_view;
	image_buffer_info.sampler = resource->sampler->vk_sampler;

    VkWriteDescriptorSet write = zest_CreateImageDescriptorWriteWithType(frame_graph->bindless_set->vk_descriptor_set, &image_buffer_info, binding_number, descriptor_type);
    write.dstArrayElement = bindless_index;
    vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);
...
zest_uint *zest_GetTransientMipBindlessIndexes(const zest_frame_graph_context context, zest_resource_node resource, zest_global_binding_number binding_number) {
    ...
    ZEST_ASSERT(resource->type & zest_resource_type_is_image);  //Must be an image resource type
    ZEST_ASSERT(resource->image_desc.mip_levels > 1);   //The resource does not have any mip levels. Make sure to set the number of mip levels when creating the resource in the frame graph
    ZEST_ASSERT(resource->current_state_index < zest_vec_size(resource->journey));
    zest_frame_graph frame_graph = context->frame_graph;
    ...
    VkImageView *mip_views = resource->image.vk_mip_views;
    zest_mip_index_collection mip_collection = { 0 };
    ZEST_ASSERT(ZEST_VALID_HANDLE(resource->sampler));
	for (int mip_index = 0; mip_index != resource->image_desc.mip_levels; ++mip_index) {
        ...
        if (mip_views[mip_index]) {
            ...
            VkResult result = zest__create_image_view(resource->image.vk_image, resource->image_desc.format, VK_IMAGE_ASPECT_COLOR_BIT, 1, mip_index, VK_IMAGE_VIEW_TYPE_2D, 1, &mip_view);
			mip_views[mip_index] = result == VK_SUCCESS ? mip_view : VK_NULL_HANDLE;
        } 
		zest_uint bindless_index = zest__acquire_bindless_index(bindless_layout, binding_number);
		zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], resource->mip_level_bindless_indexes, bindless_index);

		VkDescriptorImageInfo mip_buffer_info;
		mip_buffer_info.imageLayout = resource->journey[resource->current_state_index].usage.vk_image_layout;
		mip_buffer_info.imageView = mip_view;
		mip_buffer_info.sampler = resource->sampler->vk_sampler;

		VkWriteDescriptorSet write = zest_CreateImageDescriptorWriteWithType(frame_graph->bindless_set->vk_descriptor_set, &mip_buffer_info, binding_number, bindless_layout->descriptor_indexes[binding_number].descriptor_type);
		write.dstArrayElement = bindless_index;
		vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);
		...
		zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], mip_collection.mip_indexes, bindless_index );
	}
    zest_map_insert_linear_key(ZestRenderer->frame_graph_allocator[ZEST_FIF], resource->image.mip_indexes, (zest_key)binding_number, mip_collection);
    return mip_collection.mip_indexes;
}
...
zest_uint zest_GetTransientBufferBindlessIndex(const zest_frame_graph_context context, zest_resource_node resource) {
	...
	VkWriteDescriptorSet write = zest_CreateBufferDescriptorWriteWithType(frame_graph->bindless_set->vk_descriptor_set, &buffer_info, zest_storage_buffer_binding, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
	write.dstArrayElement = bindless_index;
	vkUpdateDescriptorSets(ZestDevice->logical_device, 1, &write, 0, 0);
...
void zest_BlitImageMip(const zest_frame_graph_context context, zest_resource_node src, zest_resource_node dst, zest_uint mip_to_blit, zest_supported_pipeline_stages pipeline_stage) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    ...
    ZEST_ASSERT(src->image.vk_image);
    ZEST_ASSERT(dst->image.vk_image);
    ...
    VkImage src_image = src->image.vk_image;
    VkImage dst_image = dst->image.vk_image;
    ...
    VkImageLayout src_current_layout = src->journey[src->current_state_index].usage.vk_image_layout;
    VkImageLayout dst_current_layout = dst->journey[dst->current_state_index].usage.vk_image_layout;
    ...
    VkImageMemoryBarrier blit_src_barrier = zest__create_image_memory_barrier(src_image,
        ...
        mip_to_blit, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, &blit_src_barrier);
    ...
    VkImageMemoryBarrier blit_dst_barrier = zest__create_image_memory_barrier(dst_image,
        ...
        mip_to_blit, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, &blit_dst_barrier);
    ...
    blit_src_barrier = zest__create_image_memory_barrier(src_image,
        ...
        src->image_desc.aspect_flags,
        mip_to_blit, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, &blit_src_barrier);
    ...
    blit_dst_barrier = zest__create_image_memory_barrier(dst_image,
        ...
        dst->image_desc.aspect_flags,
        mip_to_blit, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, &blit_dst_barrier);
}

void zest_CopyImageMip(const zest_frame_graph_context context, zest_resource_node src, zest_resource_node dst, zest_uint mip_to_copy, zest_supported_pipeline_stages pipeline_stage) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    ZEST_ASSERT_HANDLE(src);
    ZEST_ASSERT_HANDLE(dst);
    ZEST_ASSERT(src->type == zest_resource_type_image && dst->type == zest_resource_type_image);
    ...
    blit_src_barrier = zest__create_image_memory_barrier(src_image,
        ...
        src->image_desc.aspect_flags,
        mip_to_copy, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, pipeline_stage, &blit_src_barrier);
    ...
    blit_dst_barrier = zest__create_image_memory_barrier(dst_image,
        ...
        dst->image_desc.aspect_flags,
        mip_to_copy, 1);
    zest__place_image_barrier(context->command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, pipeline_stage, &blit_dst_barrier);
}
...
void zest_InsertComputeImageBarrier(const zest_frame_graph_context context, zest_resource_node resource, zest_uint base_mip) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    ZEST_ASSERT_HANDLE(resource);    //Not a valid resource handle!
...
void zest__add_image_barrier(zest_resource_node resource, zest_execution_barriers_t *barriers, zest_bool acquire, VkAccessFlags src_access, 
    ...
    VkImageMemoryBarrier image_barrier = zest__create_image_memory_barrier(
        ...
        old_layout,
        new_layout,
        resource->image_desc.aspect_flags,
        0, resource->image_desc.mip_levels);
    image_barrier.srcQueueFamilyIndex = src_family;
    image_barrier.dstQueueFamilyIndex = dst_family;
    if (acquire) {
        zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], barriers->acquire_image_barriers, image_barrier);
        ...
        zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], barriers->release_image_barriers, image_barrier);
...
void zest__add_memory_buffer_barrier(zest_resource_node resource, zest_execution_barriers_t *barriers, zest_bool acquire, VkAccessFlags src_access, VkAccessFlags dst_access,
    ...
    if (acquire) {
        zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], barriers->acquire_buffer_barriers, buffer_barrier);
        zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], barriers->acquire_buffer_barrier_nodes, resource);
		barriers->overall_src_stage_mask_for_acquire_barriers |= src_stage;
		barriers->overall_dst_stage_mask_for_acquire_barriers |= dst_stage;
    } else {
        zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], barriers->release_buffer_barriers, buffer_barrier);
...
zest_frame_graph_result zest__create_rg_render_pass(zest_pass_group_t *pass, zest_execution_details_t *exe_details, zest_uint current_pass_index) {
    ...
    if (exe_details->requires_dynamic_render_pass) {
        ...
        zest_vec_foreach(c, exe_details->color_attachment_info) {
            ...
            reference.layout = usage_info->vk_image_layout;
        ...
        if (exe_details->depth_attachment_info.resource_node) {
            ...
            if (node->swapchain) {
				...
				vk_depth_ref.layout = usage_info->vk_image_layout;
            }
        }
        zest_vec_foreach(c, exe_details->resolve_attachment_info) {
            zest_resource_node node = exe_details->resolve_attachment_info[c].resource_node;
...
zest_resource_usage_t zest__configure_image_usage(zest_resource_node resource, zest_resource_purpose purpose, VkFormat format, VkAttachmentLoadOp load_op, VkAttachmentLoadOp stencil_load_op, VkPipelineStageFlags relevant_pipeline_stages) {
    ...
    case zest_purpose_color_attachment_write:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
        ...
        resource->image_desc.usage |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
        break;
    ...
    case zest_purpose_sampled_image:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        usage.access_mask = VK_ACCESS_SHADER_READ_BIT;
        usage.stage_mask = relevant_pipeline_stages;
        resource->image_desc.usage |= VK_IMAGE_USAGE_SAMPLED_BIT;
        usage.is_output = ZEST_FALSE;
        break;
    ...
    case zest_purpose_storage_image_read:
        ...
        usage.access_mask = VK_ACCESS_SHADER_READ_BIT;
        usage.stage_mask = relevant_pipeline_stages;
        resource->image_desc.usage |= VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT;
        usage.is_output = ZEST_FALSE;
        break;

    case zest_purpose_storage_image_write:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_GENERAL;
        usage.access_mask = VK_ACCESS_SHADER_WRITE_BIT;
        usage.stage_mask = relevant_pipeline_stages;
        resource->image_desc.usage |= VK_IMAGE_USAGE_STORAGE_BIT;
        usage.is_output = ZEST_TRUE;
        break;
    ...
    case zest_purpose_depth_stencil_attachment_read:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL;
        ...
        usage.aspect_flags = VK_IMAGE_ASPECT_DEPTH_BIT;
        resource->image_desc.usage |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        usage.is_output = ZEST_FALSE;
        break;

    case zest_purpose_depth_stencil_attachment_write:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    ...
    case zest_purpose_input_attachment: // For subpass reads
        ...
        usage.stage_mask = relevant_pipeline_stages;
        resource->image_desc.usage |= VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
        usage.is_output = ZEST_FALSE;
        break;

    case zest_purpose_transfer_image:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
        usage.access_mask = VK_ACCESS_TRANSFER_READ_BIT;
        usage.stage_mask = VK_PIPELINE_STAGE_TRANSFER_BIT;
        resource->image_desc.usage |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
        usage.is_output = ZEST_FALSE;
        break;

    case zest_purpose_present_src:
        usage.vk_image_layout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
...
void zest__add_pass_image_usage(zest_pass_node pass_node, zest_resource_node image_resource, zest_resource_purpose purpose, VkPipelineStageFlags relevant_pipeline_stages, zest_bool is_output, VkAttachmentLoadOp load_op, VkAttachmentStoreOp store_op, VkAttachmentLoadOp stencil_load_op, VkAttachmentStoreOp stencil_store_op, VkClearValue clear_value) {
    zest_resource_usage_t usage = zest__configure_image_usage(image_resource, purpose, image_resource->image_desc.format, load_op, stencil_load_op, relevant_pipeline_stages);
...
void zest_ConnectInput(zest_resource_node resource, zest_sampler sampler) {
    ...
    if (resource->type & zest_resource_type_is_image) {
        ...
        case zest_pass_type_graphics:
            ...
            if (resource->type & zest_resource_type_is_image && !sampler) {
                VkSamplerCreateInfo info = resource->image_desc.mip_levels == 1 ? zest_CreateSamplerInfo() : zest_CreateMippedSamplerInfo(resource->image_desc.mip_levels);
                resource->sampler = zest_GetSampler(&info);
            }
...
void zest_ConnectSwapChainOutput() {
    ...
    VkClearValue cv = { 0 };
    cv.color = frame_graph->swapchain->vk_clear_color;
...
void zest__reset_query_pool(const zest_frame_graph_context context, VkQueryPool query_pool, zest_uint count) {
    ...
    vkCmdResetQueryPool(context->command_buffer, query_pool, first_query, count);
}
...
zest_texture_handle zest__new_texture() {
    zest_texture_handle handle = { zest__add_store_resource(&ZestRenderer->textures) };
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    *texture = (zest_texture_t){ 0 };
    texture->magic = zest_INIT_MAGIC(zest_struct_type_texture);
    texture->name.str = 0;
    texture->flags = zest_texture_flag_use_filtering;
    texture->storage_type = zest_texture_storage_type_single;
    texture->image_index = 1;
    texture->image.vk_current_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    texture->image.vk_format = VK_FORMAT_R8G8B8A8_UNORM;
    texture->color_channels = 4;
    texture->imgui_blend_type = zest_imgui_blendtype_none;
    texture->image.vk_view_type = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
	texture->vk_sampler_info.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	texture->vk_sampler_info.magFilter = VK_FILTER_LINEAR;
	texture->vk_sampler_info.minFilter = VK_FILTER_LINEAR;
	texture->vk_sampler_info.addressModeU = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
	texture->vk_sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
	texture->vk_sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
	texture->vk_sampler_info.anisotropyEnable = VK_FALSE;
	texture->vk_sampler_info.maxAnisotropy = 1.f;
	texture->vk_sampler_info.unnormalizedCoordinates = VK_FALSE;
	texture->vk_sampler_info.compareEnable = VK_FALSE;
	texture->vk_sampler_info.compareOp = VK_COMPARE_OP_ALWAYS;
	texture->vk_sampler_info.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
	texture->vk_sampler_info.borderColor = VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK;
	texture->vk_sampler_info.mipLodBias = 0.f;
	texture->vk_sampler_info.minLod = 0.0f;
	texture->vk_sampler_info.maxLod = 1.0f;
	texture->vk_sampler_info.pNext = VK_NULL_HANDLE;
	texture->vk_sampler_info.flags = 0;
    texture->info.width = 1024;
    texture->info.height = 1024;
    for (int i = 0; i != zest_max_global_binding_number; ++i) {
...
zest_atlas_region_t zest_NewImage(void) {
    zest_atlas_region_t image = { 0 };
...
zest_atlas_region zest_CreateImage() {
    zest_atlas_region_t blank_image = zest_NewImage();
    zest_atlas_region image = ZEST__NEW_ALIGNED(zest_atlas_region, 16);
    *image = blank_image;
    image->magic = zest_INIT_MAGIC(zest_struct_type_image);
...
zest_bitmap zest__get_bitmap(zest_texture texture, zest_index bitmap_index) {
    zest_vec_test(texture->image_collection->image_bitmaps, (zest_uint)bitmap_index);
    return &texture->image_collection->image_bitmaps[bitmap_index];
}
...
zest_atlas_region zest_AddTextureImageFile(zest_texture_handle handle, const char* filename) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_atlas_region image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->regions, image);
    texture->image_index = zest_vec_last_index(texture->image_collection->regions);
    image->index = texture->image_index;
    if (texture->storage_type == zest_texture_storage_type_single) {
        if (zest_vec_size(texture->image_collection->image_bitmaps)) {
			...
			zest_FreeTextureBitmaps(handle);
        }
    }
    zest_bitmap_t new_bitmap = { 0 };
    new_bitmap.magic = zest_INIT_MAGIC(zest_struct_type_bitmap);
	zest_vec_push(texture->image_collection->image_bitmaps, new_bitmap);
    zest_bitmap_t* bitmap = zest__get_bitmap(texture, zest__texture_image_index(texture));

    zest_LoadBitmapImage(bitmap, filename, 0);
    zest_ConvertBitmapToTextureFormat(bitmap, texture->image.vk_format);
...
zest_atlas_region zest_AddTextureImageBitmap(zest_texture_handle handle, zest_bitmap bitmap_to_load) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_atlas_region image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->regions, image);
    texture->image_index = zest_vec_last_index(texture->image_collection->regions);
    image->index = texture->image_index;
    zest_bitmap_t image_copy = { 0 };
    image_copy.magic = zest_INIT_MAGIC(zest_struct_type_bitmap);
    ...
    zest_vec_push(texture->image_collection->image_bitmaps, image_copy);
    zest_bitmap_t* bitmap = zest__get_bitmap(texture, zest__texture_image_index(texture));
...
zest_atlas_region zest_AddTextureImageMemory(zest_texture_handle handle, const char* name, const unsigned char* buffer, int buffer_size) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_atlas_region image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->regions, image);
...
zest_atlas_region zest_AddTextureAnimationFile(zest_texture_handle handle, const char* filename, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_ConvertBitmapToTextureFormat(&spritesheet, texture->image.vk_format);

    zest_uint animation_area = spritesheet.meta.width * spritesheet.meta.height;
    zest_uint frame_area = width * height;
    zest_index first_index = zest_vec_size(texture->image_collection->regions);
    ...
    zest_SetText(&texture->image_collection->regions[first_index]->name, filename);
    return texture->image_collection->regions[first_index];
}

zest_atlas_region zest_AddTextureAnimationBitmap(zest_texture_handle handle, zest_bitmap spritesheet, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_SetText(&texture->image_collection->regions[first_index]->name, spritesheet->name.str);
    return texture->image_collection->regions[first_index];
}

zest_atlas_region zest_AddTextureAnimationMemory(zest_texture_handle handle, const char* name, unsigned char* buffer, int buffer_size, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_SetText(&texture->image_collection->regions[first_index]->name, name);
    return texture->image_collection->regions[first_index];
}
...
float zest__copy_animation_frames(zest_texture texture, zest_bitmap spritesheet, int width, int height, zest_uint frames, zest_bool row_by_row) {
    ...
    float max_radius = 0;
    zest_atlas_region_t blank_image = zest_NewImage();
    blank_image.magic = zest_INIT_MAGIC(zest_struct_type_image);
    zest_atlas_region frame = zest_CreateAnimation(frames);
    ...
    for (zest_uint r = 0; r != (row_by_row ? rows : cols); ++r) {
        for (zest_uint c = 0; c != (row_by_row ? cols : rows); ++c) {
            ...
            *frame = blank_image;
            zest_vec_push(texture->image_collection->regions, frame);
            texture->image_index = zest_vec_last_index(texture->image_collection->regions);
            frame->index = texture->image_index;
            zest_bitmap_t new_bitmap = { 0 };
            new_bitmap.magic = zest_INIT_MAGIC(zest_struct_type_bitmap);
            zest_vec_push(texture->image_collection->image_bitmaps, new_bitmap);
            zest_bitmap_t* image_bitmap = zest__get_bitmap(texture, texture->image_index);
...
void zest__free_all_texture_images(zest_texture texture) {
    ...
    zest_vec_foreach(i, texture->image_collection->image_bitmaps) {
        zest_FreeBitmapData(&texture->image_collection->image_bitmaps[i]);
    }
    zest__delete_texture_layers(texture);
    zest_uint i = 0;
    while (i < zest_vec_size(texture->image_collection->regions)) {
        zest_atlas_region image = texture->image_collection->regions[i];
        i += image->frames;
        zest_FreeText(&image->name);
        ZEST__FREE(image);
    }
    zest_vec_free(texture->image_collection->regions);
...
void zest__release_all_global_texture_indexes(zest_texture texture) {
    ...
    zest_map_foreach(i, texture->image.mip_indexes) {
        zest_mip_index_collection *mip_collection = &texture->image.mip_indexes.data[i];
...
void zest__cleanup_texture(zest_texture texture) {
    ...
    zest__remove_store_resource(&ZestRenderer->textures, texture->handle.value);
}

void zest__cleanup_texture_vk_handles(zest_texture texture) {
	if(texture->image.vk_view) vkDestroyImageView(ZestDevice->logical_device, texture->image.vk_view, &ZestDevice->allocation_callbacks);
	if(texture->image.vk_image) vkDestroyImage(ZestDevice->logical_device, texture->image.vk_image, &ZestDevice->allocation_callbacks);
	if(texture->image.buffer) zest_FreeBuffer(texture->image.buffer);
    ...
    if (debug_layout) {
        if (ZEST_VALID_HANDLE(texture->debug_set) && texture->debug_set->vk_descriptor_set) vkFreeDescriptorSets(ZestDevice->logical_device, debug_layout->pool->vk_descriptor_pool, 1, &texture->debug_set->vk_descriptor_set);
    }
	texture->image.buffer = 0;
	texture->image.vk_view = VK_NULL_HANDLE;
	texture->image.vk_image = VK_NULL_HANDLE;
    if(ZEST_VALID_HANDLE(texture->debug_set)) ZEST__FREE(texture->debug_set);
	zest_vec_foreach(i, texture->image.vk_mip_views) {
		VkImageView image_view = texture->image.vk_mip_views[i];
		if (image_view) {
			vkDestroyImageView(ZestDevice->logical_device, image_view, &ZestDevice->allocation_callbacks);
		}
	}
	zest_vec_free(texture->image.vk_mip_views);
	zest_uint size = zest_map_size(texture->image.mip_indexes);
	zest_map_foreach(i, texture->image.mip_indexes) {
		zest_vec_free(texture->image.mip_indexes.data[i].mip_indexes);
	}
	zest_map_free(texture->image.mip_indexes);
    texture->flags &= ~zest_texture_flag_ready;
}

void zest__reindex_texture_images(zest_texture texture) {
    zest_index index = 0;
    zest_vec_foreach(i, texture->image_collection->regions) {
        texture->image_collection->regions[i]->index = index++;
    }
...
void zest__create_texture_debug_set(zest_texture texture) {
    ...
    zest_set_layout debug_layout = (zest_set_layout)zest__get_store_resource(&ZestRenderer->set_layouts, ZestRenderer->texture_debug_layout.value);
    texture->debug_set = zest_FinishDescriptorSet(debug_layout->pool, &set_builder, set);
}

void zest__maybe_create_image_collection(zest_texture texture) {
    if (!texture->image_collection) {
        texture->image_collection = ZEST__NEW(zest_image_collection);
...
VkResult zest__process_texture_images(zest_texture texture, VkCommandBuffer command_buffer) {
    if (zest_vec_empty(texture->image_collection->regions) && texture->storage_type != zest_texture_storage_type_storage && texture->storage_type != zest_texture_storage_type_single) {
        return VK_SUCCESS;
    }

    if (texture->storage_type == zest_texture_storage_type_packed) {
        zest__pack_images(texture, texture->info.width);
    }
    else if (texture->storage_type == zest_texture_storage_type_bank) {
        zest__make_image_bank(texture, texture->image_collection->regions[0]->width);
    }
    ...
    if (texture->storage_type < zest_texture_storage_type_sprite_sheet) {
        ...
        ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image_array(texture, mip_levels, command_buffer));
        ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image_view(texture, texture->image.vk_view_type, mip_levels, texture->info.layer_count));

        texture->descriptor_image_info.imageView = texture->image.vk_view;
        ...
        if (texture->flags & zest_texture_flag_use_filtering)
            mip_levels = (zest_uint)floor(log2(ZEST__MAX(texture->image_collection->regions[0]->width, texture->image_collection->regions[0]->height))) + 1;
        else
            mip_levels = 1;

        texture->info.layer_count = 1;

        VkImageUsageFlags flags = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        if (texture->image.vk_current_layout == VK_IMAGE_LAYOUT_GENERAL) {
            flags |= VK_IMAGE_USAGE_STORAGE_BIT;
            ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image(texture, mip_levels, flags, VK_IMAGE_LAYOUT_GENERAL, ZEST_TRUE, command_buffer));
            texture->descriptor_image_info.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        }
        else {
            ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image(texture, mip_levels, flags, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, ZEST_TRUE, command_buffer));
            texture->descriptor_image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        }
        ...
        if (texture->flags & zest_texture_flag_use_filtering) {
            mip_levels = (zest_uint)floor(log2(ZEST__MAX(texture->info.width, texture->info.height))) + 1;
        } else {
            mip_levels = 1;
        }
        if (texture->image.vk_view_type == VK_IMAGE_VIEW_TYPE_CUBE) {
			ZEST_RETURN_RESULT_ON_FAIL(zest__create_image_array(texture->info.width, texture->info.height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->image.vk_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image));
			texture->image.vk_format = texture->image.vk_format;
            texture->info.layer_count = 6;
            zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, texture->image.vk_current_layout, mip_levels, texture->info.layer_count, command_buffer);
        } else {
            ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image(texture, mip_levels, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL, ZEST_FALSE, command_buffer));
            texture->info.layer_count = 1;
        }
        ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image_view(texture, texture->image.vk_view_type, mip_levels, texture->info.layer_count));
    ...
    texture->info.mip_levels = mip_levels;
	texture->vk_sampler_info.maxLod = (float)mip_levels - 1;
	texture->sampler = zest_GetSampler(&texture->vk_sampler_info);
...
void zest_ProcessTextureImages(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT_HANDLE(texture->image_collection);  //Texture hasn't been properly initialised, no image collection found
    zest__process_texture_images(texture, 0);
}

zest_uint zest_GetTextureDescriptorIndex(zest_texture_handle handle, zest_global_binding_number binding_number) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->bindless_index[binding_number];
}

void zest__delete_texture_layers(zest_texture texture) {
    zest_vec_foreach(i, texture->image_collection->layers) {
        zest_FreeBitmapData(&texture->image_collection->layers[i]);
    }
    zest_vec_free(texture->image_collection->layers);
}

VkDescriptorImageInfo *zest_GetTextureDescriptorImageInfo(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return &texture->descriptor_image_info;
}

zest_bitmap zest__get_texture_single_bitmap(zest_texture texture) {
    if (texture->texture_bitmap.data) {
        return &texture->texture_bitmap;
    }

    if (zest_vec_size(texture->image_collection->regions) > 0) {
        return &texture->image_collection->image_bitmaps[0];
    }

    return &texture->texture_bitmap;
}

VkResult zest__create_texture_image(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout vk_image_layout, zest_bool copy_bitmap, VkCommandBuffer command_buffer) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image(bitmap->meta.width, bitmap->meta.height, mip_levels, VK_SAMPLE_COUNT_1_BIT, texture->image.vk_format, VK_IMAGE_TILING_OPTIMAL, usage_flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image));

    texture->image.vk_format = texture->image.vk_format;
    if (copy_bitmap) {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1, command_buffer));

        ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image.vk_image, (zest_uint)bitmap->meta.width, (zest_uint)bitmap->meta.height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, command_buffer));
		if (!command_buffer) {
			zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);
		} else {
			zest_vec_push(ZestRenderer->staging_buffers, staging_buffer);
		}
        ZEST_CLEANUP_ON_FAIL(zest__generate_mipmaps(texture->image.vk_image, texture->image.vk_format, bitmap->meta.width, bitmap->meta.height, mip_levels, 1, vk_image_layout, command_buffer));
    }
    else {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, vk_image_layout, mip_levels, 1, command_buffer));
    }

    texture->image.vk_current_layout = vk_image_layout;
...
VkResult zest__create_texture_image_array(zest_texture texture, zest_uint mip_levels, VkCommandBuffer command_buffer) {
    VkDeviceSize image_size = texture->image_collection->bitmap_array.total_mem_size;
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image_array(texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->info.layer_count, VK_SAMPLE_COUNT_1_BIT, texture->image.vk_format, 0, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image));

    texture->image.vk_format = texture->image.vk_format;
    texture->image.vk_current_layout = VK_IMAGE_LAYOUT_UNDEFINED;

    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, texture->info.layer_count, command_buffer));

    ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_regions_to_image(texture->image_collection->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image.vk_image, command_buffer));
    ...
    zest__generate_mipmaps(texture->image.vk_image, texture->image.vk_format, texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->info.layer_count, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, command_buffer);
...
VkResult zest__create_texture_image_view(zest_texture texture, VkImageViewType view_type, zest_uint mip_levels, zest_uint layer_count) {
     return zest__create_image_view(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, view_type, layer_count, &texture->image.vk_view);
}
...
void zest__make_image_bank(zest_texture texture, zest_uint size) {
    ...
    zest_CreateBitmapArray(&texture->image_collection->bitmap_array, size, size, 4, zest_vec_size(texture->image_collection->regions));
    ...
    texture->info.layer_count = id;

    zest_size offset = 0;

    zest_vec_clear(texture->image_collection->buffer_copy_regions);

    for (zest_uint layer = 0; layer < texture->info.layer_count; layer++) {
...
void zest__make_sprite_sheet(zest_texture texture) {
    ...
    zest_vec_foreach(i, texture->image_collection->regions) {
        zest_atlas_region image = texture->image_collection->regions[i];
		ZEST_ASSERT(image->width <= texture->info.width && image->height <= texture->info.height);   //This image will not pack into the texture because it's bigger then the texture layer size.
																													//Use zest_SetTextureLayerSize to set a bigger layer size after creating the texture.
        stbrp_rect rect;
        rect.w = image->width + texture->image_collection->packed_border_size * 2;
        rect.h = image->height + texture->image_collection->packed_border_size * 2;
        rect.x = 0;
        rect.y = 0;
        rect.was_packed = 0;
        rect.id = image->index;
    ...
    zest_vec_foreach(i, current_rects) {
        ...
        if (rect->was_packed) {
            zest_atlas_region image = texture->image_collection->regions[rect->id];

            float rect_x = (float)rect->x + texture->image_collection->packed_border_size;
            float rect_y = (float)rect->y + texture->image_collection->packed_border_size;
            ...
            zest_CopyBitmap(&texture->image_collection->image_bitmaps[rect->id], 0, 0, image->width, image->height, &texture->texture_bitmap, image->left, image->top);
        }
        else {
            zest_vec_push(rects, *rect);
        }
...
void zest__pack_images(zest_texture texture, zest_uint size) {
    ...
    int max_size = texture->info.width;
    zest_vec_foreach(i, texture->image_collection->regions) {
        zest_atlas_region image = texture->image_collection->regions[i];
        stbrp_rect rect;
		ZEST_ASSERT(image->width <= texture->info.width && image->height <= texture->info.height);   //This image will not pack into the texture because it's bigger then the texture layer size.
    ...
    if (texture->storage_type == zest_texture_storage_type_sprite_sheet) {
        ...
        texture->info.layer_count = zest__calculate_texture_layers(rects, size, node_count);

        zest_FreeBitmapArray(&texture->image_collection->bitmap_array);
        zest_CreateBitmapArray(&texture->image_collection->bitmap_array, size, size, 4, texture->info.layer_count);
    }
    ...
    stbrp_rect* current_rects = 0;
    while (!zest_vec_empty(rects) && current_layer < texture->info.layer_count) {
        stbrp_context context;
        stbrp_init_target(&context, size, size, nodes, node_count);
        stbrp_pack_rects(&context, rects, (int)zest_vec_size(rects));
        ...
        zest_vec_foreach(i, current_rects) {
            ...
            if (rect->was_packed) {
                ...
                zest_CopyBitmap(&texture->image_collection->image_bitmaps[rect->id], 0, 0, image->width, image->height, &tmp_image, image->left, image->top);
...
void zest__update_texture_single_image_meta(zest_texture texture, zest_uint width, zest_uint height) {
    zest_atlas_region_t image = zest_NewImage();
    image.width = width;
    image.height = height;
    zest__update_image_vertices(&image);
    image.uv.x = 0.f;
    ...
    image.texture = texture->handle;
    image.layer = 0;
    if (zest_vec_empty(texture->image_collection->regions)) {
        zest_vec_push(texture->image_collection->regions, zest_CreateImage());
        texture->image_index = 0;
    }
    ZEST_ASSERT(zest_vec_size(texture->image_collection->regions) > 0);
    *texture->image_collection->regions[0] = image;
}

void zest_SetTextureUseFiltering(zest_texture_handle handle, zest_bool value) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
...
void zest_SetTexturePackedBorder(zest_texture_handle handle, zest_uint value) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->image_collection->packed_border_size = value;
}

void zest_SetTextureStorageType(zest_texture_handle handle, zest_texture_storage_type value) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->storage_type = value;
}

zest_texture_handle zest_CreateTexture(const char* name, zest_texture_storage_type storage_type, zest_bool use_filtering, zest_texture_format vk_image_format, zest_uint reserve_images) {
    zest_texture_handle handle = zest__new_texture();
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->handle = handle;
    zest_SetText(&texture->name, name);
    zest_SetTextureImageFormat(handle, vk_image_format);
    if (storage_type < zest_texture_storage_type_render_target) {
        zest_SetTextureStorageType(handle, storage_type);
        zest_SetTextureUseFiltering(handle, use_filtering);
    }
    if (storage_type == zest_texture_storage_type_single || storage_type == zest_texture_storage_type_bank) {
        zest_SetTextureWrappingRepeat(handle);
    }
    for (int i = 0; i != zest_max_global_binding_number; ++i) {
        texture->bindless_index[i] = ZEST_INVALID;
    }
	zest__maybe_create_image_collection(texture);
    if (reserve_images) {
        zest_vec_reserve(texture->image_collection->regions, reserve_images);
    }        
    return handle;
}

zest_texture_handle zest_CreateTexturePacked(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_packed, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture_handle zest_CreateTextureSpritesheet(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_sprite_sheet, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture_handle zest_CreateTextureSingle(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_single, zest_texture_flag_use_filtering, vk_image_format, 1);
}

zest_texture_handle zest_CreateTextureBank(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_bank, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture_handle zest_CreateTextureStorage(const char* name, int width, int height, zest_texture_format format, VkImageViewType view_type, zest_bool with_mip_maps) {
    zest_texture_handle handle = zest_CreateTexture(name, zest_texture_storage_type_storage, with_mip_maps, format, 1);
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    texture->image.vk_current_layout = VK_IMAGE_LAYOUT_GENERAL;
    zest__update_image_vertices(&texture->texture);
    zest_bitmap bitmap = zest__get_texture_single_bitmap(texture);
    bitmap->meta.width = width;
    bitmap->meta.height = height;
    texture->image.vk_view_type = view_type;
    zest__process_texture_images(texture, 0);
    return handle;
}

zest_texture_handle zest_LoadCubemap(const char *name, const char *file_name) {
    zest_image_collection image_collection = zest__load_ktx(file_name);
    if (!image_collection) {
        return (zest_texture_handle){ 0 };
    }
	zest_texture_handle handle = zest__new_texture();
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    texture->image.vk_format = image_collection->vk_format;
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image_array(bitmap_array->meta[0].width, bitmap_array->meta[0].height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->image.vk_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image));
    texture->image.vk_format = texture->image.vk_format;

    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 6, 0));
    ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_regions_to_image(texture->image_collection->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image.vk_image, 0));
    ZEST_CLEANUP_ON_FAIL(zest__create_image_view(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, VK_IMAGE_VIEW_TYPE_CUBE, 6, &texture->image.vk_view));
    texture->descriptor_image_info.imageView = texture->image.vk_view;
    texture->descriptor_image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image.vk_image, texture->image.vk_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mip_levels, 6, 0));
    ...
    texture->descriptor_image_info.sampler = texture->sampler->vk_sampler;
...
void zest_FreeTexture(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_vec_push(ZestRenderer->deferred_resource_freeing_list.resources[ZEST_FIF], texture);
}

void zest_ResetTexture(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest__free_all_texture_images(texture);
    zest__cleanup_texture_vk_handles(texture);
}

void zest_FreeTextureBitmaps(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest__free_all_texture_images(texture);
}

void zest_RemoveTextureImage(zest_texture_handle handle, zest_atlas_region image) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT((zest_uint)image->index < zest_vec_size(texture->image_collection->regions));    //Image not within range of image indexes in the texture
    zest_FreeBitmapData(&texture->image_collection->image_bitmaps[image->index]);
    zest_vec_erase(texture->image_collection->image_bitmaps, &texture->image_collection->image_bitmaps[image->index]);
    zest_vec_erase(texture->image_collection->regions, &texture->image_collection->regions[image->index]);
    ZEST__FREE(image);
    zest__reindex_texture_images(texture);
}

void zest_RemoveTextureAnimation(zest_texture_handle handle, zest_atlas_region first_image) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    for (zest_uint i = 0; i < first_image->frames; ++i) {
        zest_FreeBitmapData(&texture->image_collection->image_bitmaps[first_image->index + i]);
    }
    zest_vec_erase_range(texture->image_collection->image_bitmaps, &texture->image_collection->image_bitmaps[first_image->index], &texture->image_collection->image_bitmaps[first_image->index + first_image->frames]);
    zest_vec_erase_range(texture->image_collection->regions, &texture->image_collection->regions[first_image->index], &texture->image_collection->regions[first_image->index + first_image->frames]);
    ZEST__FREE(first_image);
    zest__reindex_texture_images(texture);
}

void zest_SetTextureImageFormat(zest_texture_handle handle, zest_texture_format format) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    if (texture->image_collection) {
		ZEST_ASSERT(zest_vec_size(texture->image_collection->regions) == 0);    //You cannot change the image format of a texture that already has images
    }
    texture->image.vk_format = (VkFormat)format;
    switch (format) {
...
void zest_SetTextureUserData(zest_texture_handle handle, void *user_data) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
...
zest_atlas_region zest_GetImageFromTexture(zest_texture_handle handle, zest_index index) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->image_collection->regions[index];
}

zest_bool zest_TextureCanTile(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->storage_type == zest_texture_storage_type_bank || texture->storage_type == zest_texture_storage_type_single;
}

void zest_SetTextureWrapping(zest_texture_handle handle, VkSamplerAddressMode u, VkSamplerAddressMode v, VkSamplerAddressMode w) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->vk_sampler_info.addressModeU = u;
    texture->vk_sampler_info.addressModeV = v;
    texture->vk_sampler_info.addressModeW = w;
}

void zest_SetTextureWrappingClamp(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->vk_sampler_info.addressModeU = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    texture->vk_sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
    texture->vk_sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
...
void zest_SetTextureWrappingBorder(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->vk_sampler_info.addressModeU = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
    texture->vk_sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
    texture->vk_sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER;
}

void zest_SetTextureWrappingRepeat(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->vk_sampler_info.addressModeU = VK_SAMPLER_ADDRESS_MODE_REPEAT;
    texture->vk_sampler_info.addressModeV = VK_SAMPLER_ADDRESS_MODE_REPEAT;
    texture->vk_sampler_info.addressModeW = VK_SAMPLER_ADDRESS_MODE_REPEAT;
}

void zest_SetTextureLayerSize(zest_texture_handle handle, zest_uint size) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_uint next_pow2 = (zest_uint)ZEST__NEXTPOW2(size);
    ZEST_ASSERT(ZEST__POW2(next_pow2));
    texture->info.width = next_pow2;
...
void zest_SetTextureMaxRadiusOnLoad(zest_texture_handle handle, zest_bool yesno) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    if (yesno) {
...
void zest_SetTextureImguiBlendType(zest_texture_handle handle, zest_imgui_blendtype blend_type) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    texture->imgui_blend_type = blend_type;
}

void zest_TextureResize(zest_texture_handle handle, zest_uint width, zest_uint height) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
...
zest_bool zest_TextureClear(zest_texture_handle handle, const zest_frame_graph_context context) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    VkCommandBuffer command_buffer = context ? context->command_buffer : 0;
    if (!context) {
        ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&command_buffer));
    }
    ...
    zest__insert_image_memory_barrier(
        ...
        0,
        VK_ACCESS_TRANSFER_WRITE_BIT,
        texture->image.vk_current_layout,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        VK_PIPELINE_STAGE_TRANSFER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT,
        image_sub_resource_range);

    vkCmdClearColorImage(command_buffer, texture->image.vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &ClearColorValue, 1, &image_sub_resource_range);
...
zest_bool zest_CopyTextureToTexture(zest_texture_handle src_handle, zest_texture_handle dst_handle, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ...
    vkCmdBlitImage(
        copy_command,
        src_image->image.vk_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        dst_image->image.vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &image_blit_region,
        VK_FILTER_NEAREST);
...
zest_bool zest_CopyTextureToBitmap(zest_texture_handle src_handle, zest_bitmap image, int src_x, int src_y, int dst_x, int dst_y, int width, int height, zest_bool swap_channel) {
    zest_texture src_image = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, src_handle.value);
    ZEST_ASSERT_HANDLE(src_image);    //Not a valid texture resource
    VkCommandBuffer copy_command = 0;
    ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&copy_command));
    ...
    zest_bool supports_blit = zest__has_blit_support(src_image->image.vk_format);
    ...
    zest__insert_image_memory_barrier(
        ...
        VK_ACCESS_MEMORY_READ_BIT,
        VK_ACCESS_TRANSFER_READ_BIT,
        src_image->image.vk_current_layout,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        VK_PIPELINE_STAGE_TRANSFER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT,
        image_range);
    ...
    if (!supports_blit)
    {
        if (src_image->image.vk_format == VK_FORMAT_B8G8R8A8_SRGB || src_image->image.vk_format == VK_FORMAT_B8G8R8A8_UNORM || src_image->image.vk_format == VK_FORMAT_B8G8R8A8_SNORM) {
            color_swizzle = 1;
        }
    }
...
VkImageLayout zest_GetTextureLayout(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->image.vk_current_layout;
}

zest_texture_info_t zest_GetTextureInfo(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->info;
}

VkDescriptorSet zest_GetTextureDebugDescriptorSet(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->debug_set->vk_descriptor_set;
}

VkDescriptorSet zest_GetImageDebugDescriptorSet(zest_atlas_region image) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, image->texture.value);
    return texture->debug_set->vk_descriptor_set;
}
...
zest_extent_t zest_ImageDimensions(zest_atlas_region image) {
    ZEST_ASSERT_HANDLE(image);  //Not a valid image handle
    return (zest_extent_t) { (float)image->width, (float)image->height };
}

zest_vec4 zest_ImageUV(zest_atlas_region image) {
    ZEST_ASSERT_HANDLE(image);  //Not a valid image handle
    return image->uv;
}

zest_texture_handle zest_ImageTextureHandle(zest_atlas_region image) {
    ZEST_ASSERT_HANDLE(image);  //Not a valid image handle
...
void zest_UploadInstanceLayerData(const zest_frame_graph_context context, void *user_data) {
    ...
    if (ZEST_VALID_HANDLE(layer)) {  //You must pass in the zest_layer in the user data
        ...
        zest_UploadBuffer(&instance_upload, context);
    }
}
...
void zest_DrawInstanceLayer(const zest_frame_graph_context context, void *user_data) {
    ...
    zest_vec_foreach(i, layer->draw_instructions[layer->fif]) {
        ...
        if (pipeline) {
			zest_BindPipelineShaderResource(context, pipeline, current->shader_resources);
        } else {
            continue;
        }
...
void zest_DrawInstanceMeshLayer(const zest_frame_graph_context context, void *user_data) {
    ...
    if (layer->vertex_data && layer->index_data) {
        zest_BindMeshVertexBuffer(context, layer_handle);
        zest_BindMeshIndexBuffer(context, layer_handle);
    } else {
...
void zest_BindMeshVertexBuffer(const zest_frame_graph_context context, zest_layer_handle layer_handle) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    zest_layer layer = (zest_layer)zest__get_store_resource_checked(&ZestRenderer->layers, layer_handle.value);
...
void zest_BindMeshIndexBuffer(const zest_frame_graph_context context, zest_layer_handle layer_handle) {
    ZEST_ASSERT_HANDLE(context);        //Not valid context, this command must be called within a frame graph execution callback
    zest_layer layer = (zest_layer)zest__get_store_resource_checked(&ZestRenderer->layers, layer_handle.value);
...
zest_compute_handle zest_FinishCompute(zest_compute_builder_t *builder, const char *name) {
    ...
    if (bindless_layout) {
        zest_vec_push(set_layouts, bindless_layout->vk_layout);
    }
    zest_vec_foreach(i, builder->non_bindless_layouts) {
        ...
        zest_vec_push(set_layouts, non_bindless_layout->vk_layout);
    }
    ...
    zest_vec_foreach(i, compute->shaders) {
        ...
        ZestRenderer->last_result = vkCreateComputePipelines(ZestDevice->logical_device, VK_NULL_HANDLE, 1, &compute_pipeline_create_info, &ZestDevice->allocation_callbacks, &pipeline);
...
const char *zest__struct_type_to_string(zest_struct_type struct_type) {
    switch (struct_type) {
    case zest_struct_type_texture                 : return "Texture"; break;

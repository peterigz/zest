...
void zest__execute_render_graph() {
    ...
    VkPipelineStageFlags *wave_wait_stages = 0;
    ...
    zest_vec_foreach(submission_index, render_graph->submissions) {
        ...
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            ...
            case zest_queue_graphics:
                ...
                if (!render_graph->queue_semaphores[ZEST_GRAPHICS_QUEUE_INDEX]) {
					render_graph->queue_semaphores[queue_index] = zest__acquire_semaphore();
                    zest_vec_push(ZestRenderer->used_semaphores[ZEST_FIF], render_graph->queue_semaphores[queue_index]);
                }
                break;
            case zest_queue_compute:
                ...
                if (!render_graph->queue_semaphores[ZEST_COMPUTE_QUEUE_INDEX]) {
					render_graph->queue_semaphores[queue_index] = zest__acquire_semaphore();
                    zest_vec_push(ZestRenderer->used_semaphores[ZEST_FIF], render_graph->queue_semaphores[queue_index]);
                }
                break;
            case zest_queue_transfer:
                ...
                if (!render_graph->queue_semaphores[ZEST_TRANSFER_QUEUE_INDEX]) {
					render_graph->queue_semaphores[queue_index] = zest__acquire_semaphore();
                    zest_vec_push(ZestRenderer->used_semaphores[ZEST_FIF], render_graph->queue_semaphores[queue_index]);
                }
                break;
            ...
            //between each wave
            render_graph->queue_semaphore_values[queue_index]++;
			zest_vec_linear_push(allocator, signal_semaphores, render_graph->queue_semaphores[queue_index]);
			zest_vec_linear_push(allocator, signal_values, render_graph->queue_semaphore_values[queue_index]);
        ...
        zest_vec_clear(wave_wait_stages);
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
...
zest_render_target_composite_t zest_CreateRenderTargetComposite(zest_resource_node resource, void *push_constant, zest_uint push_constant_size, zest_pipeline_template pipeline, zest_sampler sampler) {
    zest_render_target_composite_t composite = { 0 };
    composite.resource = resource;
    composite.push_constant = push_constant;
    composite.push_constant_size = push_constant_size;
    composite.pipeline = pipeline;
    composite.sampler = sampler;
    return composite;
}
...
zest_resource_node zest_ImportSwapChainResource(const char *name) {
    ...
    render_graph->swapchain_resource_handle = zest__add_render_graph_resource(&node);
...
zest_resource_node zest__add_render_graph_resource(zest_resource_node resource) {
    ...
    ZEST_CHECK_HANDLE(render_graph);        //Not a valid render graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zloc_linear_allocator_t *allocator = ZestRenderer->render_graph_allocator[ZEST_FIF];
    zest_vec_linear_push(allocator, render_graph->resources, *resource);
    zest_resource_versions_t resource_version = { 0 };
    zest_resource_node node = &zest_vec_back(render_graph->resources);
    node->version = 0;
...
zest_resource_versions_t *zest__maybe_add_resource_version(zest_resource_node resource) {
    ...
    if (ZEST__FLAGGED(last_version->flags, zest_resource_node_flag_has_producer)) {
		...
		zloc_linear_allocator_t *allocator = ZestRenderer->render_graph_allocator[ZEST_FIF];
        zest_vec_linear_push(allocator, render_graph->resources, new_resource);
        zest_vec_linear_push(allocator, versions->resources, &zest_vec_back(render_graph->resources));
    }
...
zest_pass_node zest_AddGraphicBlankScreen(const char *name) {
    ...
    pass->type = zest_pass_type_graphics;
    return pass;
}
...
void zest_BlitImageMip(VkCommandBuffer command_buffer, zest_resource_node src, zest_resource_node dst, zest_uint mip_to_blit) {
...
void zest__add_pass_image_usage(zest_pass_node pass_node, zest_resource_node image_resource, zest_resource_purpose purpose, VkPipelineStageFlags relevant_pipeline_stages, zest_bool is_output, VkAttachmentLoadOp load_op, VkAttachmentStoreOp store_op, VkAttachmentLoadOp stencil_load_op, VkAttachmentStoreOp stencil_store_op, VkClearValue clear_value) {
    ...
    if (usage.is_output) {
		zest_resource_versions_t *versions = zest__maybe_add_resource_version(image_resource);
        ...
        if (latest_version->id == image_resource->id) {
            ...
            zest_map_linear_insert(ZestRenderer->render_graph_allocator[ZEST_FIF], pass_node->outputs, latest_version->name, usage);
            pass_node->output_key += image_resource->id + zest_Hash(&usage, sizeof(zest_resource_usage_t), 0);
            //Check if the user already added this as input:
            if (!zest_map_valid_name(pass_node->inputs, image_resource->name)) {
                ...
                case zest_pass_type_graphics: 
                    input_usage = zest__get_image_usage(zest_purpose_color_attachment_read, image_resource->image_desc.format, load_op, stencil_load_op, relevant_pipeline_stages); 
                    break;
                case zest_pass_type_compute: 
                case zest_pass_type_transfer: 
                    input_usage = zest__get_image_usage(zest_purpose_storage_image_read, image_resource->image_desc.format, load_op, stencil_load_op, relevant_pipeline_stages); 
                    break;
                }
                usage.store_op = store_op;
                usage.stencil_store_op = stencil_store_op;
				...
				zest_map_linear_insert(ZestRenderer->render_graph_allocator[ZEST_FIF], pass_node->inputs, image_resource->name, input_usage);
            }
        }
    } else {
		zest_render_graph render_graph = ZestRenderer->current_render_graph;
		ZEST_CHECK_HANDLE(render_graph);        //Not a valid render graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
		zest_resource_versions_t *versions = zest_map_at_key(render_graph->resource_versions, image_resource->original_id);
		zest_resource_node latest_version = zest_vec_back(versions->resources);
        if (!zest_map_valid_name(pass_node->inputs, latest_version->name)) {
			latest_version->reference_count++;
        }
        usage.resource_node = latest_version;
		zest_map_linear_insert(ZestRenderer->render_graph_allocator[ZEST_FIF], pass_node->inputs, latest_version->name, usage);
    }
}
...
void zest_ConnectStorageImageInput(zest_pass_node pass, zest_resource_node texture, zest_bool read_only) {
    ...
    switch (pass->type) {
    case zest_pass_type_graphics: stages = zest_pipeline_fragment_stage; break;
    case zest_pass_type_compute: stages = zest_pipeline_compute_stage; break;
    case zest_pass_type_transfer: stages = zest_pipeline_transfer_stage; break;
    }
...
//Render Targets
void zest__initialise_render_target(zest_texture render_target, zest_render_target_create_info_t* info) {
    ...
    render_target->width = info->viewport.extent.width;
    render_target->height = info->viewport.extent.height;
    render_target->mip_levels = info->mip_levels;

    render_target->image_format = info->render_format;

    zest_image_t image = zest_NewImage();
    image.width = render_target->width;
    image.height = render_target->width;
    zest__update_image_vertices(&image);
    image.uv.x = 0.f;
    image.uv.y = 0.f;
    image.uv.z = 1.f;
    ...
    image.texture = 0;
    image.layer = 0;
	render_target->sampler_info.addressModeU = info->sampler_address_mode_u;
	render_target->sampler_info.addressModeV = info->sampler_address_mode_v;
	render_target->sampler_info.addressModeW = info->sampler_address_mode_w;
	...
	//render_target->sampler = zest__create_sampler(render_target->sampler_info);
...
void zest__update_instance_layer_buffers_callback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
    ...
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        if (!zest_vec_empty(layer->draw_instructions[layer->fif])) {
            //zest_AddCopyCommand(&layer->vertex_upload, layer->memory_refs[layer->fif].staging_instance_data, layer->memory_refs[layer->fif].device_instance_data, layer->memory_refs[layer->fif].device_instance_data->buffer_offset);
        }
...
void zest_UploadMeshBuffersCallback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
    ...
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
        ...
        if (staging_vertex->memory_in_use && staging_index->memory_in_use) {
            //zest_AddCopyCommand(&layer->vertex_upload, staging_vertex, device_vertex, device_vertex->buffer_offset);
            //zest_AddCopyCommand(&layer->index_upload, staging_index, device_index, device_index->buffer_offset);
        }

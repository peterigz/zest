...
VkResult zest__create_swapchain_image_views(zest_swapchain swapchain) {
    ...
    zest_vec_foreach(i, swapchain->vk_images) {
        ...
        ZEST_VK_ASSERT_RESULT(zest__create_image_view(swapchain->vk_images[i], swapchain->vk_format, VK_IMAGE_ASPECT_COLOR_BIT, 1, 0, VK_IMAGE_VIEW_TYPE_2D_ARRAY, 1, &image_view));
        zest_vec_set(swapchain->vk_image_views, i, image_view);
    }
    return VK_SUCCESS;
}
...
void zest_AddSetBuilderTexture(zest_descriptor_set_builder_t *builder, zest_uint dst_binding, zest_uint dst_array_element, zest_texture_handle handle) {
    ...
    image_info.imageView = texture->image_buffer.image_handles.view;
    image_info.imageLayout = texture->vk_image_layout;
    zest_AddSetBuilderWrite(builder, dst_binding, dst_array_element, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &image_info, NULL, NULL);
}
...
void zest_BindPipelineShaderResource(VkCommandBuffer command_buffer, zest_pipeline pipeline, zest_shader_resources_handle handle) {
    zest_shader_resources shader_resources = (zest_shader_resources)zest__get_store_resource_checked(&ZestRenderer->shader_resources, handle.value);
...
zest_pipeline zest_PipelineWithTemplate(zest_pipeline_template pipeline_template, zest_render_pass render_pass) {
    ...
    pipeline_template->multisampling.rasterizationSamples = (VkSampleCountFlagBits)render_pass->sample_count;
    ...
    ZestRenderer->last_result = zest__cache_pipeline(pipeline_template, render_pass, cached_pipeline_key, &pipeline);
	return pipeline;
}
...
double zest_MouseX() { return ZestApp->mouse_x; }
double zest_MouseY() { return ZestApp->mouse_y; }
double zest_MouseDeltaX() { return ZestApp->mouse_delta_x; }
double zest_MouseDeltaY() { return ZestApp->mouse_delta_y; }
void zest_SetMouseLocation(double x, double y) { ZestApp->mouse_x = x; ZestApp->mouse_y = y; }
void zest_SetMouseDelta(double x, double y) { ZestApp->mouse_delta_x = x; ZestApp->mouse_delta_y = y; }
...
void zest_SetScreenSizedViewport(const zest_frame_graph_context_t *context, float min_depth, float max_depth) {
...
VkResult zest__create_transient_image(zest_resource_node resource) {
    ...
    ZEST_VK_ASSERT_RESULT(vkCreateImage(ZestDevice->logical_device, &image_info, &ZestDevice->allocation_callbacks, &resource->image_buffer.image_handles.image));
    ...
    if (image_info.mipLevels > 1) {
        zest_vec_linear_resize(ZestRenderer->frame_graph_allocator[ZEST_FIF], resource->image_buffer.mip_views, image_info.mipLevels);
...
VkResult zest__create_image(zest_uint width, zest_uint height, zest_uint mip_levels, VkSampleCountFlagBits num_samples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, zest_image_handles_t *image_handles) {
    VkImageCreateInfo image_info = { 0 };
    image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    image_info.imageType = VK_IMAGE_TYPE_2D;
    image_info.extent.width = width;
	...
	ZEST_SET_MEMORY_CONTEXT(zest_vk_renderer, zest_vk_image);
    ZEST_VK_ASSERT_RESULT(vkCreateImage(ZestDevice->logical_device, &image_info, &ZestDevice->allocation_callbacks, &image_handles->image));

    VkMemoryRequirements memory_requirements;
    vkGetImageMemoryRequirements(ZestDevice->logical_device, image_handles->image, &memory_requirements);
	...
	buffer_info.memory_type_bits = memory_requirements.memoryTypeBits;
	buffer_info.alignment = memory_requirements.alignment;
    image_handles->buffer = zest_CreateBuffer(memory_requirements.size, &buffer_info, image_handles->image);

    if (image_handles->buffer) {
        vkBindImageMemory(ZestDevice->logical_device, image_handles->image, zest_GetBufferDeviceMemory(image_handles->buffer), image_handles->buffer->memory_offset);
    } else {
...
VkResult zest__create_image_array(zest_uint width, zest_uint height, zest_uint mipLevels, zest_uint layers, VkSampleCountFlagBits numSamples, VkFormat format, VkImageCreateFlags flags, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, zest_image_handles_t *image_handles) {
...
zest_bool zest__create_transient_resource(zest_resource_node resource) {
    if (resource->type & zest_resource_type_is_image) {
        ...
        ZEST_RETURN_FALSE_ON_FAIL(zest__create_image_view(
            ...
            1,
            &resource->image_buffer.image_handles.view
        ));
        ...
        for (zest_uint queue_index = 0; queue_index != ZEST_QUEUE_COUNT; ++queue_index) {
            ...
            zest_vec_foreach(i, batch->pass_indices) {
                ...
                zest_vec_foreach(pass_index, grouped_pass->passes) {
                    ...
                    pass->execution_callback.callback(command_buffer, &frame_graph->context, pass->execution_callback.user_data);
                }
...
void zest_EmptyRenderPass(VkCommandBuffer command_buffer, const zest_frame_graph_context_t *context, void *user_data) {
...
zest_uint *zest_AcquireGlobalTextureMipIndexes(zest_texture_handle handle, zest_global_binding_number binding_number) {
    ...
    if (zest_map_valid_key(texture->image_buffer.mip_indexes, (zest_key)binding_number)) {
        zest_mip_index_collection *mip_collection = zest_map_at_key(texture->image_buffer.mip_indexes, (zest_key)binding_number);
        return mip_collection->mip_indexes;
    }
    VkImageView *mip_views = texture->image_buffer.mip_views;
    if (!mip_views) {
        zest_vec_resize(texture->image_buffer.mip_views, texture->info.mip_levels);
        mip_views = texture->image_buffer.mip_views;
    ...
    for (int mip_index = 0; mip_index != texture->info.mip_levels; ++mip_index) {
        ...
        if (mip_views[mip_index]) {
            ...
            VkResult result = zest__create_image_view(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, 1, mip_index, texture->vk_image_view_type, texture->info.layer_count, &mip_view);
        ...
        mip_buffer_info.imageLayout = texture->vk_image_layout;
    ...
    zest_map_insert_key( texture->image_buffer.mip_indexes, (zest_key)binding_number, mip_collection);
    zest_uint size = zest_map_size(texture->image_buffer.mip_indexes);
    return mip_collection.mip_indexes;
}
...
zest_resource_id zest__add_transient_image_resource(const char *name, const zest_image_description_t *description, zest_resource_usage_hint usage_hints, zest_bool assign_bindless, zest_bool image_view_binding_only) {
    ...
    zest__interpret_hints(&node, usage_hints);
	ZEST__FLAG(node.flags, zest_resource_node_flag_transient);
    return zest__add_frame_graph_resource(&node);
}

void zest_FlagResourceAsEssential(zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&ZestRenderer->current_frame_graph->resources, zest_resource_node_t, resource_id);
...
void zest_AddSwapchainToRenderTargetGroup(zest_output_group group) {
    ...
    zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], group->resources, frame_graph->swapchain_resource->resource_id);
}

void zest_AddImageToRenderTargetGroup(zest_output_group group, zest_resource_id resource_id) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    ZEST_ASSERT_HANDLE(group);  //Not a valid render target group
    zest_resource_node image = zest_bucket_array_get(&ZestRenderer->current_frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(image);  //Not a valid resource id passed into the function!
    ZEST_ASSERT(image->type & zest_resource_type_is_image_or_depth);  //Must be a depth buffer resource type
    ZEST__FLAG(image->flags, zest_resource_node_flag_essential_output);
    zest_vec_linear_push(ZestRenderer->frame_graph_allocator[ZEST_FIF], group->resources, resource_id);
}
...
zest_resource_id zest_AddTransientImageResource(const char *name, zest_image_resource_info_t *info) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
	...
	return zest__add_transient_image_resource(name, &description, info->usage_hints, 0, ZEST_TRUE);
}

zest_resource_id zest_AddTransientBufferResource(const char *name, const zest_buffer_resource_info_t *info) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    zest_resource_node_t node = { 0 };
    node.name = name;
...
zest_resource_id zest_AddTransientLayerResource(const char *name, const zest_layer_handle layer_handle, zest_bool prev_fif) {
    zest_layer layer = (zest_layer)zest__get_store_resource_checked(&ZestRenderer->layers, layer_handle.value);
    zest_size layer_size = layer->memory_refs[layer->fif].instance_count * layer->instance_struct_size;
    if (layer_size == 0) {
        return ZEST_INVALID;
    ...
    zest_resource_id resource_id = ZEST_INVALID;
    if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_manual_fif)) {
        zest_buffer_resource_info_t buffer_desc = { 0 };
        buffer_desc.size = layer_size;
        buffer_desc.usage_hints = zest_resource_usage_hint_vertex_buffer;
        resource_id = zest_AddTransientBufferResource(name, &buffer_desc);
    } else {
		...
		resource_id = zest__add_frame_graph_resource(&node);
        frame_graph->last_node_added->bindless_index[0] = layer->memory_refs[fif].device_vertex_data->array_index;
    }
    frame_graph->last_node_added->user_data = layer;
    frame_graph->last_node_added->buffer_provider = zest__instance_layer_resource_provider;
    layer->vertex_buffer_node = frame_graph->last_node_added;
    return resource_id;
}
...
zest_resource_node_t zest__create_import_image_resource_node(const char *name, zest_texture texture) {
    ...
    node.image_desc.aspect_flags = zest__determine_aspect_flag(texture->vk_image_format);
    node.image_desc.numSamples = VK_SAMPLE_COUNT_1_BIT;
    node.image_desc.mip_levels = texture->info.mip_levels;
    node.current_queue_family_index = VK_QUEUE_FAMILY_IGNORED;
    node.linked_layout = &texture->vk_image_layout;
    node.producer_pass_idx = -1;
	ZEST__FLAG(node.flags, zest_resource_node_flag_imported);
	ZEST__FLAG(node.flags, zest_resource_node_flag_essential_output);
    return node;
...
zest_resource_id zest_ImportImageResource(const char *name, zest_texture_handle handle, zest_resource_image_provider provider) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    zest_resource_node_t node = zest__create_import_image_resource_node(name, texture);
    node.final_layout = texture->vk_image_layout;
    node.current_layout = texture->vk_image_layout;
	ZEST__FLAG(node.flags, zest_resource_node_flag_imported);
    node.image_provider = provider;
    return zest__add_frame_graph_resource(&node);
}

zest_resource_id zest_ImportBufferResource(const char *name, zest_buffer buffer, zest_resource_buffer_provider provider) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
...
zest_resource_id zest__import_swapchain_resource(zest_swapchain swapchain) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    ...
    node.image_desc.numSamples = VK_SAMPLE_COUNT_1_BIT;
    node.image_desc.aspect_flags = zest__determine_aspect_flag(swapchain->vk_format);
    node.current_layout = VK_IMAGE_LAYOUT_UNDEFINED;
	...
	ZEST__FLAG(node.flags, zest_resource_node_flag_imported);
	ZEST__FLAG(node.flags, zest_resource_node_flag_essential_output);
    zest_resource_id resource_id = zest__add_frame_graph_resource(&node);
    frame_graph->swapchain_resource = frame_graph->last_node_added;
    return resource_id;
}
...
zest_resource_id zest__add_frame_graph_resource(zest_resource_node resource) {
    zest_frame_graph frame_graph = ZestRenderer->current_frame_graph;
    ZEST_ASSERT_HANDLE(frame_graph);        //Not a valid frame graph! Make sure you called BeginRenderGraph or BeginRenderToScreen
    zloc_linear_allocator_t *allocator = ZestRenderer->frame_graph_allocator[ZEST_FIF];
    ...
    *node = *resource;
    ...
    node->resource_id = zest_bucket_array_size(&frame_graph->resources) - 1;
    ...
    frame_graph->last_node_added = node;
...
zest_buffer zest_GetPassInputBuffer(const zest_frame_graph_context_t *context, const char *name) {
...
zest_buffer zest_GetPassOutputBuffer(const zest_frame_graph_context_t *context, const char *name) {
...
zest_uint zest_GetTransientImageBindlessIndex(const zest_frame_graph_context_t *context, zest_resource_id resource_id, zest_bool base_mip_only, zest_global_binding_number binding_number) {
    zest_frame_graph frame_graph = context->frame_graph;
    zest_resource_node resource = zest_bucket_array_get(&frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);                               // Not a valid resource handle
    ...
    if (resource->image_desc.mip_levels == 1 || base_mip_only) {
        ...
        if (!resource->image_buffer.multi_mip_view) {
             VkResult result = zest__create_image_view(
                resource->image_buffer.image_handles.image,
                resource->image_desc.format,
                zest__determine_aspect_flag(resource->image_desc.format),
                ...
                VK_IMAGE_VIEW_TYPE_2D,
                1,
				&resource->image_buffer.multi_mip_view
            );
            if (result != VK_SUCCESS) {
                resource->image_buffer.multi_mip_view = VK_NULL_HANDLE;
            }
        }
...
zest_uint *zest_GetTransientMipBindlessIndexes(const zest_frame_graph_context_t *context, zest_resource_id resource_id, zest_global_binding_number binding_number) {
    ...
    if (zest_map_valid_key(resource->image_buffer.mip_indexes, (zest_key)binding_number)) {
        zest_mip_index_collection *mip_collection = zest_map_at_key(resource->image_buffer.mip_indexes, (zest_key)binding_number);
        return mip_collection->mip_indexes;
    }
    VkImageView *mip_views = resource->image_buffer.mip_views;
    zest_mip_index_collection mip_collection = { 0 };
    ZEST_ASSERT(ZEST_VALID_HANDLE(resource->sampler));
	for (int mip_index = 0; mip_index != resource->image_desc.mip_levels; ++mip_index) {
        ...
        if (mip_views[mip_index]) {
            ...
            VkResult result = zest__create_image_view(resource->image_buffer.image_handles.image, resource->image_desc.format, VK_IMAGE_ASPECT_COLOR_BIT, 1, mip_index, VK_IMAGE_VIEW_TYPE_2D, 1, &mip_view);
			mip_views[mip_index] = result == VK_SUCCESS ? mip_view : VK_NULL_HANDLE;
        } 
		zest_uint bindless_index = zest__acquire_bindless_index(bindless_layout, binding_number);
    ...
    zest_map_insert_linear_key(ZestRenderer->frame_graph_allocator[ZEST_FIF], resource->image_buffer.mip_indexes, (zest_key)binding_number, mip_collection);
    return mip_collection.mip_indexes;
}

zest_uint zest_GetTransientBufferBindlessIndex(const zest_frame_graph_context_t *context, zest_resource_id resource_id) {
    zest_set_layout bindless_layout = (zest_set_layout)zest__get_store_resource_checked(&ZestRenderer->set_layouts, context->frame_graph->bindless_layout.value);
    zest_frame_graph frame_graph = context->frame_graph;
    zest_resource_node resource = zest_bucket_array_get(&frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);   //Not a valid resource handle
...
zest_uint zest_GetResourceMipLevels(const zest_frame_graph_context_t *context, zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);	//Not a valid resource handle!
    return resource->image_desc.mip_levels;
}

zest_uint zest_GetResourceWidth(const zest_frame_graph_context_t *context, zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);	//Not a valid resource handle!
    return resource->image_desc.width;
}

zest_uint zest_GetResourceHeight(const zest_frame_graph_context_t *context, zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);	//Not a valid resource handle!
    return resource->image_desc.height;
}

VkImage zest_GetResourceImage(const zest_frame_graph_context_t *context, zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, resource_id);
	ZEST_ASSERT_HANDLE(resource);   //Not a valid resource handle!
	if (resource->type & zest_resource_type_is_image_or_depth) {
        return resource->image_buffer.image_handles.image;
	}
	return VK_NULL_HANDLE;
}

void zest_BlitImageMip(const zest_frame_graph_context_t *context, zest_resource_id src_id, zest_resource_id dst_id, zest_uint mip_to_blit, zest_supported_pipeline_stages pipeline_stage) {
    zest_resource_node src = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, src_id);
    zest_resource_node dst = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, dst_id);
...
void zest_CopyImageMip(const zest_frame_graph_context_t *context, zest_resource_id src_id, zest_resource_id dst_id, zest_uint mip_to_copy, zest_supported_pipeline_stages pipeline_stage) {
    zest_resource_node src = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, src_id);
    zest_resource_node dst = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, dst_id);
    ZEST_ASSERT_HANDLE(src);
    ZEST_ASSERT_HANDLE(dst);
...
void zest_SetResourceClearColor(zest_resource_id resource_id, float red, float green, float blue, float alpha) {
    zest_resource_node resource = zest_bucket_array_get(&ZestRenderer->current_frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);   //Not a valid resource handle! Check the resource id is valid.
    resource->clear_color = (zest_vec4){red, green, blue, alpha};
}

void zest_InsertComputeImageBarrier(const zest_frame_graph_context_t *context, zest_resource_id resource_id, zest_uint base_mip) {
    zest_resource_node resource = zest_bucket_array_get(&context->frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);    //Not a valid resource handle!
...
void zest_ReleaseBufferAfterUse(zest_resource_id resource_id) {
    zest_resource_node resource = zest_bucket_array_get(&ZestRenderer->current_frame_graph->resources, zest_resource_node_t, resource_id);
    ZEST_ASSERT_HANDLE(resource);  
    ZEST_ASSERT(ZEST__FLAGGED(resource->flags, zest_resource_node_flag_imported));  //The resource must be imported, transient buffers are simply discarded after use.
    ZEST__FLAG(resource->flags, zest_resource_node_flag_release_after_use);
}

// --- Image Helpers ---
void zest_ConnectInput(zest_resource_id resource_id, zest_sampler sampler) {
...
void zest_ConnectOutput(zest_resource_id resource_id) {
    ZEST_ASSERT_HANDLE(ZestRenderer->current_frame_graph);  //This function must be called withing a Being/EndRenderGraph block
    ZEST_ASSERT_HANDLE(ZestRenderer->current_pass); //No current pass found. Make sure you call zest_BeginPass
    zest_pass_node pass = ZestRenderer->current_pass;
    zest_resource_node resource = zest_bucket_array_get(&ZestRenderer->current_frame_graph->resources, zest_resource_node_t, resource_id);
...
void zest_ConnectGroupedOutput(zest_output_group group) {
    ...
    zest_vec_foreach(i, group->resources) {
        zest_resource_id resource_id = group->resources[i];
	    zest_ConnectOutput(resource_id); 
    }
}
...
zest_texture_handle zest__new_texture() {
    ...
    *texture = (zest_texture_t){ 0 };
    ...
    texture->vk_image_format = VK_FORMAT_R8G8B8A8_UNORM;
    texture->color_channels = 4;
    texture->imgui_blend_type = zest_imgui_blendtype_none;
    texture->vk_image_view_type = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
...
zest_image zest_AddTextureImageFile(zest_texture_handle handle, const char* filename) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_image image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->images, image);
    texture->image_index = zest_vec_last_index(texture->image_collection->images);
    ...
    zest_ConvertBitmapToTextureFormat(bitmap, texture->vk_image_format);
...
zest_image zest_AddTextureImageBitmap(zest_texture_handle handle, zest_bitmap bitmap_to_load) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_image image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->images, image);
...
zest_image zest_AddTextureImageMemory(zest_texture_handle handle, const char* name, const unsigned char* buffer, int buffer_size) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    zest_image image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->images, image);
...
zest_image zest_AddTextureAnimationFile(zest_texture_handle handle, const char* filename, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_ConvertBitmapToTextureFormat(&spritesheet, texture->vk_image_format);
    ...
    zest_SetText(&texture->image_collection->images[first_index]->name, filename);
    return texture->image_collection->images[first_index];
}

zest_image zest_AddTextureAnimationBitmap(zest_texture_handle handle, zest_bitmap spritesheet, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_SetText(&texture->image_collection->images[first_index]->name, spritesheet->name.str);
    return texture->image_collection->images[first_index];
}

zest_image zest_AddTextureAnimationMemory(zest_texture_handle handle, const char* name, unsigned char* buffer, int buffer_size, int width, int height, zest_uint frames, float* _max_radius, zest_bool row_by_row) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_SetText(&texture->image_collection->images[first_index]->name, name);
    return texture->image_collection->images[first_index];
}
...
float zest__copy_animation_frames(zest_texture texture, zest_bitmap spritesheet, int width, int height, zest_uint frames, zest_bool row_by_row) {
    ...
    for (zest_uint r = 0; r != (row_by_row ? rows : cols); ++r) {
        for (zest_uint c = 0; c != (row_by_row ? cols : rows); ++c) {
            ...
            *frame = blank_image;
            zest_vec_push(texture->image_collection->images, frame);
            texture->image_index = zest_vec_last_index(texture->image_collection->images);
...
void zest__free_all_texture_images(zest_texture texture) {
    ...
    zest_uint i = 0;
    while (i < zest_vec_size(texture->image_collection->images)) {
...
void zest__cleanup_texture_vk_handles(zest_texture texture) {
	...
	zest_vec_foreach(i, texture->image_buffer.mip_views) {
		VkImageView image_view = texture->image_buffer.mip_views[i];
		if (image_view) {
			vkDestroyImageView(ZestDevice->logical_device, image_view, &ZestDevice->allocation_callbacks);
		}
	}
	zest_vec_free(texture->image_buffer.mip_views);
	zest_uint size = zest_map_size(texture->image_buffer.mip_indexes);
	zest_map_foreach(i, texture->image_buffer.mip_indexes) {
		zest_vec_free(texture->image_buffer.mip_indexes.data[i].mip_indexes);
	}
	zest_map_free(texture->image_buffer.mip_indexes);
...
VkResult zest__process_texture_images(zest_texture texture, VkCommandBuffer command_buffer) {
    ...
    if (texture->storage_type == zest_texture_storage_type_packed) {
        ...
        zest__make_image_bank(texture, texture->image_collection->images[0]->width);
    }
    ...
    if (texture->storage_type < zest_texture_storage_type_sprite_sheet) {
        ...
        ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image_view(texture, texture->vk_image_view_type, mip_levels, texture->info.layer_count));

        texture->descriptor_image_info.imageView = texture->image_buffer.image_handles.view;
        ...
        if (texture->flags & zest_texture_flag_use_filtering)
            mip_levels = (zest_uint)floor(log2(ZEST__MAX(texture->image_collection->images[0]->width, texture->image_collection->images[0]->height))) + 1;
        else
            mip_levels = 1;
        ...
        if (texture->vk_image_layout == VK_IMAGE_LAYOUT_GENERAL) {
        ...
        if (texture->vk_image_view_type == VK_IMAGE_VIEW_TYPE_CUBE) {
			ZEST_RETURN_RESULT_ON_FAIL(zest__create_image_array(texture->info.width, texture->info.height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer.image_handles));
			texture->image_buffer.format = texture->vk_image_format;
            texture->info.layer_count = 6;
            zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, texture->vk_image_layout, mip_levels, texture->info.layer_count, command_buffer);
        } else {
...
zest_bitmap zest__get_texture_single_bitmap(zest_texture texture) {
    ...
    if (zest_vec_size(texture->image_collection->images) > 0) {
...
VkResult zest__create_texture_image(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout vk_image_layout, zest_bool copy_bitmap, VkCommandBuffer command_buffer) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image(bitmap->meta.width, bitmap->meta.height, mip_levels, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_TILING_OPTIMAL, usage_flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer.image_handles));

    texture->image_buffer.format = texture->vk_image_format;
    if (copy_bitmap) {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1, command_buffer));

        ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image_buffer.image_handles.image, (zest_uint)bitmap->meta.width, (zest_uint)bitmap->meta.height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, command_buffer));
		if (!command_buffer) {
			zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);
		} else {
			zest_vec_push(ZestRenderer->staging_buffers, staging_buffer);
		}
        ZEST_CLEANUP_ON_FAIL(zest__generate_mipmaps(texture->image_buffer.image_handles.image, texture->vk_image_format, bitmap->meta.width, bitmap->meta.height, mip_levels, 1, vk_image_layout, command_buffer));
    }
    else {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, vk_image_layout, mip_levels, 1, command_buffer));
    }

    texture->vk_image_layout = vk_image_layout;
...
VkResult zest__create_texture_image_array(zest_texture texture, zest_uint mip_levels, VkCommandBuffer command_buffer) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image_array(texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->info.layer_count, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, 0, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer.image_handles));

    texture->image_buffer.format = texture->vk_image_format;
    texture->vk_image_layout = VK_IMAGE_LAYOUT_UNDEFINED;

    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, texture->info.layer_count, command_buffer));

    ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_regions_to_image(texture->image_collection->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image_buffer.image_handles.image, command_buffer));
    ...
    zest__generate_mipmaps(texture->image_buffer.image_handles.image, texture->vk_image_format, texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->info.layer_count, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, command_buffer);
...
VkResult zest__create_texture_image_view(zest_texture texture, VkImageViewType view_type, zest_uint mip_levels, zest_uint layer_count) {
     return zest__create_image_view(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, view_type, layer_count, &texture->image_buffer.image_handles.view);
}
...
void zest__make_image_bank(zest_texture texture, zest_uint size) {
    ...
    zest_CreateBitmapArray(&texture->image_collection->bitmap_array, size, size, 4, zest_vec_size(texture->image_collection->images));
...
void zest__make_sprite_sheet(zest_texture texture) {
    ...
    zest_vec_foreach(i, current_rects) {
        ...
        if (rect->was_packed) {
            zest_image image = texture->image_collection->images[rect->id];
...
void zest__update_texture_single_image_meta(zest_texture texture, zest_uint width, zest_uint height) {
    ...
    image.texture = texture->handle;
    image.layer = 0;
    if (zest_vec_empty(texture->image_collection->images)) {
        zest_vec_push(texture->image_collection->images, zest_CreateImage());
        texture->image_index = 0;
    }
    ZEST_ASSERT(zest_vec_size(texture->image_collection->images) > 0);
    *texture->image_collection->images[0] = image;
}
...
zest_texture_handle zest_CreateTextureStorage(const char* name, int width, int height, zest_texture_format format, VkImageViewType view_type, zest_bool with_mip_maps) {
    ...
    texture->vk_image_layout = VK_IMAGE_LAYOUT_GENERAL;
    zest__update_image_vertices(&texture->texture);
    zest_bitmap bitmap = zest__get_texture_single_bitmap(texture);
    bitmap->meta.width = width;
    bitmap->meta.height = height;
    texture->vk_image_view_type = view_type;
    zest__process_texture_images(texture, 0);
    return handle;
}
...
zest_texture_handle zest_LoadCubemap(const char *name, const char *file_name) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image_array(bitmap_array->meta[0].width, bitmap_array->meta[0].height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer.image_handles));
    texture->image_buffer.format = texture->vk_image_format;

    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 6, 0));
    ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_regions_to_image(texture->image_collection->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image_buffer.image_handles.image, 0));
    ZEST_CLEANUP_ON_FAIL(zest__create_image_view(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, VK_IMAGE_VIEW_TYPE_CUBE, 6, &texture->image_buffer.image_handles.view));
    texture->descriptor_image_info.imageView = texture->image_buffer.image_handles.view;
    texture->descriptor_image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer.image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mip_levels, 6, 0));
...
void zest_RemoveTextureImage(zest_texture_handle handle, zest_image image) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ZEST_ASSERT((zest_uint)image->index < zest_vec_size(texture->image_collection->images));    //Image not within range of image indexes in the texture
    ...
    zest_vec_erase(texture->image_collection->images, &texture->image_collection->images[image->index]);
    ZEST__FREE(image);
    zest__reindex_texture_images(texture);
}

void zest_RemoveTextureAnimation(zest_texture_handle handle, zest_image first_image) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    zest_vec_erase_range(texture->image_collection->images, &texture->image_collection->images[first_image->index], &texture->image_collection->images[first_image->index + first_image->frames]);
...
zest_image zest_GetImageFromTexture(zest_texture_handle handle, zest_index index) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
...
zest_bool zest_TextureClear(zest_texture_handle handle, VkCommandBuffer cb) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    VkCommandBuffer command_buffer = cb;
    if (!cb) {
        ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&command_buffer));
    ...
    zest__insert_image_memory_barrier(
        command_buffer,
        texture->image_buffer.image_handles.image,
        0,
        VK_ACCESS_TRANSFER_WRITE_BIT,
    ...
    vkCmdClearColorImage(command_buffer, texture->image_buffer.image_handles.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &ClearColorValue, 1, &image_sub_resource_range);
...
zest_bool zest_CopyFramebufferToTexture(zest_frame_buffer_t* src_image, zest_texture_handle handle, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    ...
    vkCmdBlitImage(
        ...
        texture->image_buffer.image_handles.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &image_blit,
        VK_FILTER_LINEAR);
...
zest_bool zest_CopyTextureToTexture(zest_texture_handle src_handle, zest_texture_handle dst_handle, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ...
    zest__insert_image_memory_barrier(
        copy_command,
        dst_image->image_buffer.image_handles.image,
        0,
        VK_ACCESS_TRANSFER_WRITE_BIT,
    ...
    zest__insert_image_memory_barrier(
        copy_command,
        src_image->image_buffer.image_handles.image,
        VK_ACCESS_MEMORY_READ_BIT,
        VK_ACCESS_TRANSFER_READ_BIT,
    ...
    vkCmdBlitImage(
        copy_command,
        src_image->image_buffer.image_handles.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        dst_image->image_buffer.image_handles.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &image_blit_region,
        VK_FILTER_NEAREST);
...
zest_bool zest_CopyTextureToBitmap(zest_texture_handle src_handle, zest_bitmap image, int src_x, int src_y, int dst_x, int dst_y, int width, int height, zest_bool swap_channel) {
    ...
    if (!supports_blit)
    {
        if (src_image->vk_image_format == VK_FORMAT_B8G8R8A8_SRGB || src_image->vk_image_format == VK_FORMAT_B8G8R8A8_UNORM || src_image->vk_image_format == VK_FORMAT_B8G8R8A8_SNORM) {
            color_swizzle = 1;
        }
    }
...
VkImageLayout zest_GetTextureLayout(zest_texture_handle handle) {
    zest_texture texture = (zest_texture)zest__get_store_resource_checked(&ZestRenderer->textures, handle.value);
    return texture->vk_image_layout;
}
...
void zest__record_mesh_layer(zest_layer layer, zest_uint fif) {
    ...
    zest_vec_foreach(i, layer->draw_instructions[layer->fif]) {
		...
		ZEST_ASSERT(current->shader_resources.value);     //Not a valid descriptor set in the shader resource. Make sure all frame in flights are set
...
void zest_UploadLayerStagingData(zest_layer_handle layer_handle, const zest_frame_graph_context_t *context) {
    ...
    if (ZEST_VALID_HANDLE(layer)) {  //You must pass in the zest_layer in the user data
        ...
        zest_UploadBuffer(&instance_upload, context->command_buffer);
    }
}
...
zest_imgui zest_GetImGuiInfo() {
    return &ZestRenderer->imgui_info;
}

float zest_GetDPIScale() {
    return ZestRenderer->dpi_scale;
}

VkInstance const zest_vk_Instance() {
    return ZestDevice->instance;
}

VkAllocationCallbacks *const zest_vk_AllocationCallbacks() {
    return &ZestDevice->allocation_callbacks;
}

void zest_vk_SetMemoryContext(zest_vulkan_memory_context context, zest_vulkan_command command) {
    ZEST_SET_MEMORY_CONTEXT(context, command);
}

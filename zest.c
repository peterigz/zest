...
void zest__make_pipeline_descriptor_writes(zest_pipeline pipeline) {
	zest_uint binding = 0;
	for (ZEST_EACH_FIF_i) {
...
void zest__prepare_standard_pipelines() {
	...
	zest_vec_push(line_instance_vertex_input_attributes, zest_CreateVertexInputDescription(0, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(zest_line_instance_t, start)));	// Location 0: Start Position
	zest_vec_push(line_instance_vertex_input_attributes, zest_CreateVertexInputDescription(0, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(zest_line_instance_t, end)));		// Location 1: End Position
	zest_vec_push(line_instance_vertex_input_attributes, zest_CreateVertexInputDescription(0, 2, VK_FORMAT_R8G8B8A8_UNORM, offsetof(zest_line_instance_t, start_color)));	// Location 2: Start Color
	zest_vec_push(line_instance_vertex_input_attributes, zest_CreateVertexInputDescription(0, 3, VK_FORMAT_R8G8B8A8_UNORM, offsetof(zest_line_instance_t, end_color)));		// Location 3: End Color
	...
	zest_SetText(&instance_create_info.vertShaderFile, "spv/line_instance.spv");
	zest_SetText(&instance_create_info.fragShaderFile, "spv/line_instance.spv");
	...
	zest__make_pipeline_descriptor_writes(line_instance_pipeline);
...
zest_layer zest_CreateBuiltinLineLayer(const char *name) {
	...
	zest__initialise_line_layer(layer, 1000);
	zest_map_insert(ZestRenderer->layers, name, layer);
	return layer;
}
...
zest_draw_routine zest__create_draw_routine_with_builtin_layer(const char *name, zest_builtin_layer_type builtin_layer) {
	...
	*draw_routine = blank_draw_routine;
	...
	if (builtin_layer == zest_builtin_layer_sprites) {
		...
		draw_routine->draw_callback = zest__draw_line_layer_callback;
		draw_routine->draw_data = layer;
		draw_routine->update_buffers_callback = zest__update_instance_layer_buffers_callback;
	}
...
void zest__reset_mesh_layer_drawing(zest_layer layer) {
	...
	layer->memory_refs[ZEST_FIF].staging_vertex_data->memory_in_use = 0;
	layer->memory_refs[ZEST_FIF].staging_index_data->memory_in_use = 0;
	...
	layer->memory_refs[ZEST_FIF].vertex_ptr = layer->memory_refs[ZEST_FIF].staging_vertex_data->data;
	layer->memory_refs[ZEST_FIF].index_ptr = layer->memory_refs[ZEST_FIF].staging_index_data->data;
}
...
void zest__end_instance_instructions(zest_layer instance_layer) {
	if (instance_layer->current_instruction.total_instances) {
		...
		instance_layer->memory_refs[ZEST_FIF].staging_instance_data->memory_in_use += instance_layer->current_instruction.total_instances * instance_layer->instance_struct_size;
...
void zest__end_mesh_instructions(zest_layer layer) {
	if (layer->current_instruction.total_indexes) {
		...
		layer->memory_refs[ZEST_FIF].staging_index_data->memory_in_use += layer->current_instruction.total_indexes * sizeof(zest_uint);
		layer->memory_refs[ZEST_FIF].staging_vertex_data->memory_in_use += layer->current_instruction.total_indexes * layer->vertex_struct_size;
...
void zest__update_instance_layer_buffers_callback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
	...
	zest__end_instance_instructions(instance_layer);
...
//Start internal line instance layer functionality -----
void zest__draw_line_layer_callback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
	zest_layer line_layer = (zest_layer)draw_routine->draw_data;
	zest__draw_instance_layer(line_layer, command_buffer);
	zest__reset_instance_layer_drawing(line_layer);
}

void zest__next_line_instance(zest_layer layer) {
	zest_line_instance_t *instance_ptr = (zest_line_instance_t*)layer->memory_refs[ZEST_FIF].instance_ptr;
	instance_ptr = instance_ptr + 1;
	//Make sure we're not trying to write outside of the buffer range
	ZEST_ASSERT(instance_ptr >= (zest_line_instance_t*)layer->memory_refs[ZEST_FIF].staging_instance_data->data && instance_ptr <= (zest_line_instance_t*)layer->memory_refs[ZEST_FIF].staging_instance_data->end);
	if (instance_ptr == layer->memory_refs[ZEST_FIF].staging_instance_data->end) {
		zest_bool grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].staging_instance_data, sizeof(zest_line_instance_t), 0);
		zest_GrowBuffer(&layer->memory_refs[ZEST_FIF].device_instance_data, sizeof(zest_line_instance_t), 0);
		if (grown) {
			layer->memory_refs[ZEST_FIF].instance_count++;
			instance_ptr = (zest_line_instance_t*)layer->memory_refs[ZEST_FIF].staging_instance_data->data;
			instance_ptr += layer->memory_refs[ZEST_FIF].instance_count;
		}
		else {
			instance_ptr = instance_ptr - 1;
...
void zest_SetSpriteDrawing(zest_layer sprite_layer, zest_texture texture, zest_descriptor_set descriptor_set, zest_pipeline pipeline) {
	zest__end_instance_instructions(sprite_layer);
...
void zest_SetBillboardDrawing(zest_layer billboard_layer, zest_texture texture, zest_descriptor_set descriptor_set, zest_pipeline pipeline) {
	zest__end_instance_instructions(billboard_layer);
...
void zest__initialise_line_layer(zest_layer line_layer, zest_uint instance_pool_size) {
...
void zest_SetLineDrawing(zest_layer line_layer, zest_descriptor_set descriptor_set, zest_pipeline pipeline) {
	zest__end_instance_instructions(line_layer);
	zest__start_instance_instructions(line_layer);
...
void zest_DrawLine(zest_layer layer, float start_point[2], float end_point[2], float width) {
	...
	zest__next_line_instance(layer);
}
...
zest_buffer zest_GetVertexStagingBuffer(zest_layer layer) {
	return layer->memory_refs[ZEST_FIF].staging_vertex_data;
}

zest_buffer zest_GetIndexStagingBuffer(zest_layer layer) {
...
void zest_UploadMeshBuffersCallback(zest_draw_routine draw_routine, VkCommandBuffer command_buffer) {
	...
	zest_buffer staging_vertex = mesh_layer->memory_refs[ZEST_FIF].staging_vertex_data;
	zest_buffer staging_index = mesh_layer->memory_refs[ZEST_FIF].staging_index_data;
...
void zest_PushVertex(zest_layer layer, float pos_x, float pos_y, float pos_z, float intensity, float uv_x, float uv_y, zest_color color, zest_uint parameters) {
	...
	*vertex_ptr = vertex;
	...
	ZEST_ASSERT(vertex_ptr >= (zest_vertex_t*)layer->memory_refs[ZEST_FIF].staging_vertex_data->data && vertex_ptr <= (zest_vertex_t*)layer->memory_refs[ZEST_FIF].staging_vertex_data->end);
	if (vertex_ptr == layer->memory_refs[ZEST_FIF].staging_vertex_data->end) {
		zest_bool grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].staging_vertex_data, sizeof(zest_vertex_t), 0);
		...
		if (grown) {
			...
			vertex_ptr = (zest_vertex_t*)layer->memory_refs[ZEST_FIF].staging_vertex_data->data;
...
void zest_PushIndex(zest_layer layer, zest_uint offset) {
	...
	*index_ptr = index;
	...
	ZEST_ASSERT(index_ptr >= (zest_uint*)layer->memory_refs[ZEST_FIF].staging_index_data->data && index_ptr <= (zest_uint*)layer->memory_refs[ZEST_FIF].staging_index_data->end);
	if (index_ptr == layer->memory_refs[ZEST_FIF].staging_index_data->end) {
		zest_bool grown = zest_GrowBuffer(&layer->memory_refs[ZestDevice->current_fif].staging_index_data, sizeof(zest_uint), 0);
		...
		if (grown) {
			...
			index_ptr = (zest_uint*)layer->memory_refs[ZEST_FIF].staging_index_data->data;

...
void zest__initialise_renderer(zest_create_info_t* create_info) {
    ...
    ZestRenderer->sampler_descriptor_pool = zest_CreateDescriptorPoolForLayout(zest_GetDescriptorSetLayout("Sampler"), 25, 0);
    ZestRenderer->uniform_descriptor_pool = zest_CreateDescriptorPoolForLayout(zest_GetDescriptorSetLayout("uniform buffer"), 25, 0);
...
void zest__cleanup_renderer() {
    ...
    zest_map_foreach(i, ZestRenderer->render_graphs) {
        ...
        zest_ForEachFrameInFlight(fif) {
            zest_vec_foreach(i, render_graph->old_frame_buffers[fif]) {
                VkFramebuffer frame_buffer = render_graph->old_frame_buffers[fif][i];
        ...
        for (zest_foreach_k(command_queue->draw_commands)) {
            ...
            if (draw_commands->composite_pipeline) {
				...
				zest_ForEachFrameInFlight(fif) {
					...
					draw_commands->composite_descriptor_set[fif].descriptor_set = zest_FinishDescriptorSet(draw_commands->descriptor_pool, &set_builder, draw_commands->composite_descriptor_set[fif].descriptor_set);
                }
                zest_ForEachFrameInFlight(fif) {
...
zest_descriptor_set_layout zest_FinishDescriptorSetLayout(zest_descriptor_set_layout_builder_t *builder, const char *name, ...) {
    ...
    //zloc_SafeCopy(set_layout->layout_bindings, builder->bindings, zest_vec_size_in_bytes(builder->bindings));
...
void zest_AddSetBuilderStorageBuffer( zest_descriptor_set_builder_t *builder, zest_uint dst_binding, zest_uint dst_array_element, zest_descriptor_buffer storage_buffer, zest_uint fif) {
    ZEST_CHECK_HANDLE(storage_buffer);  //Not a valid storage buffer
    VkDescriptorBufferInfo buffer_info = storage_buffer->descriptor_info[fif];
...
zest_descriptor_pool zest_CreateDescriptorPoolForLayout(zest_descriptor_set_layout layout, zest_uint max_set_count, VkDescriptorPoolCreateFlags pool_flags) {
    if (max_set_count == 0) {
    ...
    ZEST_VK_CHECK_RESULT(vkCreateDescriptorPool(ZestDevice->logical_device, &pool_create_info, &ZestDevice->allocation_callbacks, &pool->descriptor_pool));
...
void zest_FinalisePipelineTemplate(zest_pipeline_template pipeline_template) {
    ...
    ZEST_ASSERT(zest_vec_size(pipeline_template->descriptorSetLayouts));  //you must define at least one descriptor set layout for the pipeline
    pipeline_template->multisampling.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;
}
...
zest_pipeline_template zest_CreatePipelineTemplate(const char* name) {
    ...
    *pipeline_template = blank;
    ...
    zest_AddPipelineTemplateDescriptorLayout(pipeline_template, *zest_GetDescriptorSetLayoutVK("uniform buffer"));
...
void zest__prepare_standard_pipelines() {
    ...
    zest_AddPipelineTemplateDescriptorLayout(sprite_instance_pipeline, *zest_GetDescriptorSetLayoutVK("Sampler"));
    ...
    zest_AddPipelineTemplateDescriptorLayout(line_instance_pipeline, *zest_GetDescriptorSetLayoutVK("uniform buffer"));
    ...
    imgui_pipeline->attributeDescriptions = imgui_vertex_input_attributes;
    ...
    zest_AddPipelineTemplateDescriptorLayout(imgui_pipeline, *zest_GetDescriptorSetLayoutVK("Sampler"));
    ...
    zest_AddPipelineTemplateDescriptorLayout(mesh_pipeline, *zest_GetDescriptorSetLayoutVK("Sampler"));
    ...
    zest_AddPipelineTemplateDescriptorLayout(imesh_pipeline, *zest_GetDescriptorSetLayoutVK("uniform buffer"));
    ...
    zest_AddPipelineTemplateDescriptorLayout(final_render, *zest_GetDescriptorSetLayoutVK("Sampler"));
...
void zest__create_transient_image(zest_rg_resource_node node) {
    ...
    image_info.initialLayout = node->initial_layout;
...
zest_render_graph zest_NewRenderGraph(const char *name) {
    ...
    *render_graph = blank;
    ...
    render_graph->recorder = zest_CreatePrimaryRecorder();
    VkSemaphoreCreateInfo semaphore_info = { 0 };
    semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
    zest_map_insert(ZestRenderer->render_graphs, name, render_graph);
    /*
    zest_ForEachFrameInFlight(fif) {
        ZEST_VK_CHECK_RESULT(vkCreateSemaphore(ZestDevice->logical_device, &semaphore_info, &ZestDevice->allocation_callbacks, &render_graph->signal_present[fif]));
    }
...
bool zest_BeginRenderGraph(zest_render_graph render_graph) {
    ...
    zest_vec_clear(render_graph->passes);
    zest_vec_clear(render_graph->resources);
    zest_vec_clear(render_graph->compiled_execution_order);
    zest_vec_foreach(i, render_graph->pass_exec_details_list) {
        zest_execution_details_t *exe_details = &render_graph->pass_exec_details_list[i];
        zest_vec_free(exe_details->pre_pass_buffer_barriers);
        zest_vec_free(exe_details->pre_pass_image_barriers);
        zest_vec_free(exe_details->clear_values);
        exe_details->frame_buffer = VK_NULL_HANDLE;
        exe_details->render_pass = VK_NULL_HANDLE;
    }
    zest_vec_foreach(i, render_graph->old_frame_buffers[ZEST_FIF]) {
        VkFramebuffer frame_buffer = render_graph->old_frame_buffers[ZEST_FIF][i];
		vkDestroyFramebuffer(ZestDevice->logical_device, frame_buffer, &ZestDevice->allocation_callbacks);
    }
    zest_vec_clear(render_graph->pass_exec_details_list);
...
void zest_EndRenderGraph(zest_render_graph render_graph) {
    ...
    zest_vec_resize(dependency_count, zest_vec_size(render_graph->passes));
    zest_vec_resize(adjacency_list, zest_vec_size(render_graph->passes));

    // Set producers and consumers:
    zest_vec_foreach(i, render_graph->passes) { // i is pass_original_idx
        ...
        zest_vec_foreach(j, pass_node->outputs) {
            zest_resource_usage_t *output_usage = &pass_node->outputs[j];
        ...
        zest_vec_foreach(j, pass_node->inputs) {
            zest_resource_usage_t *input_usage = &pass_node->inputs[j];
            zest_rg_resource_node resource = input_usage->resource_node;
            zest_vec_push(resource->consumer_pass_indices, i); // pass 'i' consumes this
        }
    }
    ...
    zest_vec_foreach(consumer_idx, render_graph->passes) {
        ...
        zest_vec_foreach(j, consumer_pass->inputs) {
            zest_resource_usage_t *input_usage = &consumer_pass->inputs[j];
            zest_rg_resource_node resource = input_usage->resource_node;
            ...
            if (producer_idx != -1 && producer_idx != consumer_idx) {
                ...
                if (!already_linked) {
                    zest_vec_push(adjacency_list[producer_idx].pass_indices, consumer_idx);
    ...
    zest_vec_reserve(dependency_queue, 64);
    zest_vec_foreach(i, dependency_count) {
        if (dependency_count[i] == 0) {
            zest_vec_push(dependency_queue, i);
        }
    }
    ...
    while (index != zest_vec_size(dependency_queue)) {
        ...
        zest_vec_push(render_graph->compiled_execution_order, pass_index);
        ...
        zest_vec_foreach(i, adjacency_list[pass_index].pass_indices) {
            ...
            if (dependency_count[adj_index] == 0) {
                zest_vec_push(dependency_queue, adj_index);
            }
        }
    }
    ...
    //Calculate lifetime of resources
    zest_vec_foreach(k, render_graph->compiled_execution_order) {
        int current_pass_index = render_graph->compiled_execution_order[k];
        zest_rg_pass_node pass = &render_graph->passes[current_pass_index];
        //We just basically loop over all passes in execution order then for each pass loop over the inputs
        //and set their resource first usage pass to the execution index if it's lower then the current value
        zest_vec_foreach(i, pass->inputs) {
            zest_rg_resource_node input_resource = pass->inputs[i].resource_node;
            input_resource->first_usage_pass_idx = ZEST__MIN(input_resource->first_usage_pass_idx, (zest_uint)k);
        }
        //And simalarly do the same with outputs this time setting the last usage index if it's higher then
        //the current value
        zest_vec_foreach(o, pass->outputs) {
            zest_rg_resource_node output_resource = pass->outputs[o].resource_node;
            output_resource->last_usage_pass_idx = ZEST__MAX(output_resource->last_usage_pass_idx, (zest_uint)k);
        }
    }

    //--------------------------------------------------------
    //Potentially cull unused resources and render passes here
    //--------------------------------------------------------

    //Alocate transient images
    zest_vec_foreach(k, render_graph->compiled_execution_order) {
        int current_pass_index = render_graph->compiled_execution_order[k];
        zest_rg_pass_node pass = &render_graph->passes[current_pass_index];
        zest_vec_foreach(o, pass->outputs) {
            ...
            if (output_resource->type == zest_resource_type_image) {
                if (ZEST__FLAGGED(output_resource->flags, zest_resource_node_flag_transient) && !output_resource->image_buffer.image && output_resource->first_usage_pass_idx == (zest_uint)k) {
                    zest__create_transient_image(output_resource);
					zest__create_image_view(output_resource->image_buffer.image, output_resource->image_desc.format, 
                        pass->outputs[o].aspect_flags, output_resource->image_desc.mip_levels, 0, VK_IMAGE_VIEW_TYPE_2D, 1);
                }
            } else {
                if (ZEST__FLAGGED(output_resource->flags, zest_resource_node_flag_transient) && !output_resource->image_buffer.image) {
                    zest__create_transient_buffer(output_resource);
                }
            }
        }
        zest_vec_foreach(i, pass->inputs) {
            ...
            if (input_resource->type == zest_resource_type_image) {
                if (ZEST__FLAGGED(input_resource->flags, zest_resource_node_flag_transient) && !input_resource->image_buffer.image && input_resource->first_usage_pass_idx == (zest_uint)k) {
                    zest__create_transient_image(input_resource);
					zest__create_image_view(input_resource->image_buffer.image, input_resource->image_desc.format, 
                        pass->inputs[i].aspect_flags, input_resource->image_desc.mip_levels, 0, VK_IMAGE_VIEW_TYPE_2D, 1);
                }
            } else {
                if (ZEST__FLAGGED(input_resource->flags, zest_resource_node_flag_transient) && !input_resource->image_buffer.image) {
                    zest__create_transient_buffer(input_resource);
                }
            }
        }
    }

    zest_vec_resize(render_graph->pass_exec_details_list, zest_vec_size(render_graph->compiled_execution_order));
    ...
    zest_vec_foreach(k, render_graph->compiled_execution_order) {
        int current_pass_index = render_graph->compiled_execution_order[k];
        zest_rg_pass_node pass = &render_graph->passes[current_pass_index];
        zest_execution_details_t *exe_details = &render_graph->pass_exec_details_list[k];
        ...
        //First loop over inputs and create memory barriers for transitioning for the initial layout that this pass expects
        zest_vec_foreach(i, pass->inputs) {
            zest_resource_usage_t *input_usage = &pass->inputs[i];
            zest_rg_resource_node input_resource = pass->inputs[i].resource_node;
            if (input_resource->type == zest_resource_type_image) {
                ...
                if (needs_barrier) {
                    ...
                    zest_vec_push(exe_details->pre_pass_image_barriers, image_barrier);
					exe_details->overall_src_stage_mask_for_pre_pass_barriers |= input_resource->last_stage_mask;
					exe_details->overall_dst_stage_mask_for_pre_pass_barriers |= input_usage->stage_mask;
                }
        ...
        //First loop over inputs and create memory barriers for transitioning for whatever the final layout should be (for this pass)
        zest_vec_foreach(o, pass->outputs) {
            zest_resource_usage_t *output_usage = &pass->outputs[o];
            zest_rg_resource_node output_resource = pass->outputs[o].resource_node;
            if (output_resource->type == zest_resource_type_image || output_resource->type == zest_resource_type_swap_chain_image) {
                ...
                if (needs_barrier) {
					...
					exe_details->overall_dst_stage_mask_for_pre_pass_barriers |= output_usage->stage_mask;
                }
        ...
        if (pass->execution_callback) {
            ...
            zest_vec_foreach(o, pass->outputs) {
                ...
                if (output_resource->type == zest_resource_type_image || output_resource->type == zest_resource_type_swap_chain_image) {
                    if (ZEST__FLAGGED(output_usage->access_mask, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT)) {
                        ...
                        zest_vec_push(color_attachment_info, color);
            ...
            zest_vec_foreach(c, color_attachment_info) {
                ...
                if (!zest_map_valid_key(attachment_indexes, (zest_key)node)) {
					...
					attachment.finalLayout = zest__determine_final_layout(render_graph, k + 1, node, usage_info); 
					...
					zest_vec_push(attachments, attachment);
                    zest_map_insert_key(attachment_indexes, (zest_key)node, (zest_vec_size(attachments) - 1));
                    zest_vec_push(attachment_resource_nodes, node);
                    zest_vec_push(exe_details->clear_values, usage_info->clear_value);
            ...
            zest_vec_foreach(c, color_attachment_info) {
				...
				zest_vec_push(vk_color_refs, reference);
            }
            if(depth_attachment_info.resource_node) {
                zest_rg_resource_node node = depth_attachment_info.resource_node;
                zest_resource_usage_t *usage_info = depth_attachment_info.usage_info;
            ...
            zest_map_foreach(i, attachment_indexes) {
                ...
                zest_vec_push(image_views, node->image_buffer.base_view);
            }
            VkFramebufferCreateInfo fb_create_info = { 0 };
            fb_create_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
            fb_create_info.renderPass = render_graph->pass_exec_details_list[k].render_pass;
            ...
            zest_vec_push(render_graph->old_frame_buffers[ZEST_FIF], exe_details->frame_buffer);

            //Cleanup the temp lists
            zest_vec_free(vk_color_refs);
            zest_vec_free(vk_input_refs);
            zest_vec_free(vk_resolve_refs);
            zest_vec_free(attachments);
            zest_map_free(attachment_indexes);
            zest_vec_free(color_attachment_info);
            zest_vec_free(attachment_resource_nodes);
        }
    }
    ...
    zest_vec_free(adjacency_list);
    zest_vec_free(dependency_count);
    zest_vec_free(dependency_queue);
}
...
VkImageLayout zest__determine_final_layout(zest_render_graph render_graph, int start_from_idx, zest_rg_resource_node node, zest_resource_usage_t *current_usage) {
    for (zest_uint idx = start_from_idx; idx < zest_vec_size(render_graph->compiled_execution_order); ++idx) {
        ...
        zest_vec_foreach(i, pass->inputs) {
            if (node == pass->inputs[i].resource_node) {
                return pass->inputs[i].image_layout;
            }
        }
        zest_vec_foreach(o, pass->outputs) {
            if (node == pass->outputs[o].resource_node) {
                return pass->outputs[o].image_layout;
            }
        }
    }
    if (node->type == zest_resource_type_swap_chain_image) {
        ...
        node->final_layout; //? Need the final layout after graph finished
    } 
...
void zest_ExecuteRenderGraph(zest_render_graph render_graph) {
	...
	zest_StartRecordingRenderGraph(render_graph, ZEST_FIF);
    VkCommandBuffer command_buffer = render_graph->recorder->command_buffer[ZEST_FIF];
    ...
    zest_SubmitRenderGraph(render_graph, ZestRenderer->fif_fence[ZEST_FIF]);
}

void zest_StartRecordingRenderGraph(zest_render_graph render_graph, zest_index fif) {
    /*
    if (render_graph->query_state[ZestDevice->previous_fif] == zest_query_state_ready) {
        VkResult result = vkGetQueryPoolResults(ZestDevice->logical_device,
            render_graph->query_pool,
            ZestDevice->previous_fif * render_graph->timestamp_count,
            render_graph->timestamp_count,
            sizeof(zest_u64) * render_graph->timestamp_count,
            render_graph->timestamps[ZEST_FIF],
            sizeof(zest_u64),
    ...
    */
    ...
    VkCommandBufferBeginInfo begin_info = { 0 };
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    ZEST_VK_CHECK_RESULT(vkBeginCommandBuffer(render_graph->recorder->command_buffer[fif], &begin_info));
    ZestRenderer->current_command_buffer = render_graph->recorder->command_buffer[fif];
    /*
    if (render_graph->timestamp_count) {
        zest__reset_query_pool(render_graph->recorder->command_buffer[ZEST_FIF], render_graph->query_pool, render_graph->timestamp_count);
    }
    */
}

void zest_EndRecordingRenderGraph(zest_render_graph render_graph, zest_index fif) {
    if (render_graph->timestamp_count) {
        render_graph->query_state[fif] = zest_query_state_ready;
    }
    ZEST_VK_CHECK_RESULT(vkEndCommandBuffer(render_graph->recorder->command_buffer[fif]));
}

void zest_SubmitRenderGraph(zest_render_graph render_graph, VkFence fence) {
    VkPipelineStageFlags dst_stage_mask_array[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT }; 
    VkSemaphore image_available_semaphore_handle = ZestRenderer->frame_sync[ZEST_FIF].image_available_semaphore;
    VkSemaphore render_finshed_semaphore_handle = ZestRenderer->frame_sync[ZEST_FIF].render_finished_semaphore;
    VkCommandBuffer current_cmd_buffer = render_graph->recorder->command_buffer[ZEST_FIF];
    ...
    submit_info.signalSemaphoreCount = 1;
    submit_info.pSignalSemaphores = &render_finshed_semaphore_handle; 

    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = &current_cmd_buffer;          
...
zest_rg_resource_node zest_AddTransientImageResource(zest_render_graph render_graph, const char *name, const zest_image_description_t *description) {
    ...
    node.initial_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    ...
    zest_vec_push(render_graph->resources, node);
    return &zest_vec_back(render_graph->resources);
}

zest_rg_resource_node zest_AddTransientBufferResource(zest_render_graph render_graph, const char *name, const zest_buffer_description_t *description) {
    zest_rg_resource_node_t node = { 0 };
    node.id = render_graph->id_counter++;
    node.first_usage_pass_idx = UINT_MAX;
...
zest_rg_resource_node zest_ImportImageResource(zest_render_graph render_graph, const char *name, zest_texture texture, VkImageLayout initial_layout_at_graph_start, VkImageLayout desired_layout_after_graph_use) {
    ...
    node.producer_pass_idx = -1;
    node.desired_layout_after_graph_use = desired_layout_after_graph_use;
    node.initial_layout = initial_layout_at_graph_start;
...
zest_rg_resource_node zest_ImportSwapChainResource(zest_render_graph render_graph, const char *name) {
    ...
    node.current_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    node.producer_pass_idx = -1;
    node.desired_layout_after_graph_use = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
	ZEST__FLAG(node.flags, zest_resource_node_flag_imported);
    zest_vec_push(render_graph->resources, node);
    return &zest_vec_back(render_graph->resources);
}

zest_rg_pass_node zest_AddPassNode(zest_render_graph render_graph, const char *name, zest_rg_execution_callback callback) {
    ZEST_CHECK_HANDLE(render_graph);        //Not a valid render graph!
    zest_rg_pass_node_t node = { 0 };
    node.execution_callback = callback;
    node.id = render_graph->id_counter++;
    node.name = name;
    node.render_graph = render_graph;
    node.magic = zest_INIT_MAGIC;
    zest_vec_push(render_graph->passes, node);
...
void zest_AddPassInputDetailed( zest_rg_pass_node pass_node, const zest_rg_pass_resource_usage_desc_t *usage_desc) {
    ...
    zest_vec_push(pass_node->inputs, internal_usage);
}
...
void zest_AddPassOutputDetailed( zest_rg_pass_node pass_node, const zest_rg_pass_resource_usage_desc_t *usage_desc) {
    ...
    zest_vec_push(pass_node->outputs, internal_usage);
}
...
void zest_AddPassStorageBufferUsage(zest_rg_pass_node pass_node, zest_rg_resource_node buffer_resource, VkAccessFlags access_flags, VkPipelineStageFlags shader_stages) {
    zest_rg_pass_resource_usage_desc_t desc = { 0 };
    ...
    // Similar to storage images, read-write buffers might need special handling
    // or be considered an output if any write occurs.
}
    ...
    zest_vec_foreach(i, command_queue->draw_commands) {
        ...
        if (ZEST_VALID_HANDLE(draw_command->composite_pipeline) && !zest_vec_empty(draw_command->render_targets)) {
			...
			zest_ForEachFrameInFlight(fif) {
				...
				draw_command->composite_descriptor_set[fif].descriptor_set = zest_FinishDescriptorSet(draw_command->descriptor_pool, &set_builder, draw_command->composite_descriptor_set[fif].descriptor_set);
...
zest_command_queue_draw_commands zest_NewDrawCommandSetupUpSampler(const char *name, zest_render_target render_target, zest_render_target downsampler, zest_pipeline_template pipeline) { ZEST_CHECK_HANDLE(render_target);  //render_target must be a valid handle to a render target
    ...
    draw_commands->descriptor_pool = zest_CreateDescriptorPoolForLayout(zest_GetDescriptorSetLayout("2 sampler"), render_target->mip_levels * 2 * ZEST_MAX_FIF + 4, 0);
    zest_ForEachFrameInFlight(fif) {
    ...
    for (int mip_level = 0; mip_level != render_target->mip_levels; ++mip_level) {
        zest_ForEachFrameInFlight(fif) {
			...
			render_target->mip_level_descriptor_sets[render_target->current_index][fif][mip_level].descriptor_set = zest_FinishDescriptorSet(draw_commands->descriptor_pool,&builder, render_target->mip_level_descriptor_sets[render_target->current_index][fif][mip_level].descriptor_set);
        }
    }
    if (zest_Vec2Length2(render_target->create_info.ratio_of_screen_size)) {
        draw_commands->viewport_scale = render_target->create_info.ratio_of_screen_size;
...
void zest__initialise_render_target(zest_render_target render_target, zest_render_target_create_info_t* info) {
    ...
    render_target->descriptor_pool[0] = zest_CreateDescriptorPoolForLayout(zest_GetDescriptorSetLayout("2 sampler"), render_target->mip_levels * 2 * ZEST_MAX_FIF + 4, 0);
...
void zest__create_render_target_sampler(zest_render_target render_target) {
    ...
    zest_ForEachFrameInFlight(fif) {
        ...
        set->descriptor_set = zest_FinishDescriptorSet(render_target->descriptor_pool[index], &builder, set->descriptor_set);
    }
}
    ...
    for (zest_index mip_level = 0; mip_level != render_target->mip_levels; ++mip_level) {
        ...
        if (ZEST__FLAGGED(render_target->flags, zest_render_target_flag_upsampler)) {
            zest_ForEachFrameInFlight(fif) {
				...
				render_target->mip_level_descriptor_sets[index][fif][mip_level].descriptor_set = zest_FinishDescriptorSet(render_target->descriptor_pool[index], &builder, render_target->mip_level_descriptor_sets[index][fif][mip_level].descriptor_set);
            }
        }
    }
}
...
void zest_MakeCompute(zest_compute_builder_t* builder, zest_compute compute) {
    ...
    //compute->descriptor_pool = zest_CreateDescriptorPoolForLayout(compute->descriptor_set_layout, 2, 0);

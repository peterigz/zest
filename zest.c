...
void zest__os_poll_events() {
    ...
    ScreenToClient(ZestRenderer->current_window->window_handle, &cursor_position);
...
void zest__get_window_size_callback(void* user_data, int* fb_width, int* fb_height, int* window_width, int* window_height) {
    RECT window_rect;
    GetClientRect(ZestRenderer->current_window->window_handle, &window_rect);
...
void zest__os_set_window_title(const char* title) {
    SetWindowText(ZestRenderer->current_window->window_handle, title);
}
...
zest_swapchain_support_details_t zest__query_swapchain_support(VkPhysicalDevice physical_device) {
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, ZestRenderer->current_window->surface, &details.capabilities);

    vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, ZestRenderer->current_window->surface, &details.formats_count, ZEST_NULL);

    if (details.formats_count != 0) {
        details.formats = ZEST__ALLOCATE(sizeof(VkSurfaceFormatKHR) * details.formats_count);
        vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device, ZestRenderer->current_window->surface, &details.formats_count, details.formats);
    }

    vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, ZestRenderer->current_window->surface, &details.present_modes_count, ZEST_NULL);

    if (details.present_modes_count != 0) {
        details.present_modes = ZEST__ALLOCATE(sizeof(VkPresentModeKHR) * details.present_modes_count);
        vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, ZestRenderer->current_window->surface, &details.present_modes_count, details.present_modes);
    }
...
VkResult zest__create_logical_device() {
    ...
    zest_vec_foreach(i, ZestDevice->queue_families) {
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(ZestDevice->physical_device, i, ZestRenderer->current_window->surface, &present_support);
...
void zest__initialise_window(zest_create_info_t *create_info) {
    ...
    ZestRenderer->current_window = ZestRenderer->create_window_callback(create_info->screen_x, create_info->screen_y, create_info->screen_width, create_info->screen_height, ZEST__FLAGGED(create_info->flags, zest_init_flag_maximised), "Zest");
    zest_map_insert(ZestRenderer->windows, create_info->title, ZestRenderer->current_window);
}
...
void zest__do_scheduled_tasks(void) {
    ...
    if (zest_vec_size(ZestRenderer->deferred_resource_freeing_list.textures[ZEST_FIF])) {
        zest_vec_foreach(i, ZestRenderer->deferred_resource_freeing_list.textures[ZEST_FIF]) {
            ...
            if (ZEST__NOT_FLAGGED(texture->flags, zest_texture_flag_was_replaced)) {
...
VkResult zest__initialise_swapchain(zest_swapchain swapchain, zest_window window) {
    ...
    ZestRenderer->dpi_scale = (float)extent.width / (float)ZestRenderer->current_window->window_width;
    ...
    ZestRenderer->window_extent.width = ZestRenderer->current_window->window_width;
    ZestRenderer->window_extent.height = ZestRenderer->current_window->window_height;
    ...
    createInfo.surface = ZestRenderer->current_window->surface;
...
void zest__cleanup_renderer() {
    ...
    zest_map_foreach(i, ZestRenderer->frame_graph_semaphores) {
        zest_frame_graph_semaphores semaphores = ZestRenderer->frame_graph_semaphores.data[i];
        zest_ForEachFrameInFlight(fif) {
    ...
    zest_map_foreach(i, ZestRenderer->windows) {
        zest_window window = ZestRenderer->windows.data[i];
    ...
    zest_map_free(ZestRenderer->texture_reprocess_queue);
    zest_vec_free(ZestRenderer->texture_cleanup_queue);
...
VkResult zest__recreate_swapchain(zest_swapchain swapchain) {
    ...
    zest__update_window_size(ZestRenderer->current_window, window_width, window_height);
    swapchain->screen_resolution_push.x = 1.f / fb_width;
    swapchain->screen_resolution_push.y = 1.f / fb_height;

    zest_WaitForIdleDevice();

    zest__cleanup_swapchain(swapchain, 1);
    zest__cleanup_pipelines();
    zest_map_free(ZestRenderer->cached_pipelines);
...
zest_frame_graph_semaphores zest__get_frame_graph_semaphores(const char *name) {
    if (!zest_map_valid_name(ZestRenderer->frame_graph_semaphores, name)) {
        ...
        *semaphores = (zest_frame_graph_semaphores_t){ 0 };
        ...
        zest_map_insert(ZestRenderer->frame_graph_semaphores, name, semaphores);
        return semaphores;
    }
	return *zest_map_at(ZestRenderer->frame_graph_semaphores, name);
}
...
bool zest_ValidateShaderResource(zest_shader_resources shader_resources) {
    ZEST_ASSERT_HANDLE(shader_resources);
    zest_ForEachFrameInFlight(fif) {
...
void zest_DeletePipeline(zest_pipeline_template pipeline_template) {
    ZEST_ASSERT_HANDLE(pipeline_template);   //Not a valid pipeline template handle
	zest_vec_foreach(i, pipeline_template->cached_pipeline_keys) {
		...
		if (zest_map_valid_key(ZestRenderer->cached_pipelines, key)) {
			...
			zest__destroy_pipeline(pipeline);
			zest_map_remove_key(ZestRenderer->cached_pipelines, key);
            ZEST__FREE(pipeline);
		}
	}
...
zest_uint zest_ScreenWidth() { return ZestRenderer->current_window->window_width; }
zest_uint zest_ScreenHeight() { return ZestRenderer->current_window->window_height; }
float zest_ScreenWidthf() { return (float)ZestRenderer->current_window->window_width; }
float zest_ScreenHeightf() { return (float)ZestRenderer->current_window->window_height; }
...
void* zest_Window() {
    return ZestRenderer->current_window->window_handle;
}
...
void zest__prepare_standard_pipelines() {
    ...
    */
    ...
    zest_SetPipelinePushConstantRange(imesh_pipeline, sizeof(zest_push_constants_t), VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);
...
VkResult zest__copy_buffer_regions_to_image(VkBufferImageCopy* regions, VkBuffer buffer, VkDeviceSize src_offset, VkImage image, VkCommandBuffer cb) {
    VkCommandBuffer command_buffer = cb ? cb : 0; 
    if (!cb) {
        ZEST_VK_ASSERT_RESULT(zest__begin_single_time_commands(&command_buffer));
    }
...
VkResult zest__end_single_time_commands(VkCommandBuffer command_buffer) {
    ...
    ZEST_VK_ASSERT_RESULT(vkQueueSubmit(ZestDevice->graphics_queue.vk_queue, 1, &submit_info, VK_NULL_HANDLE));
    ZEST_VK_ASSERT_RESULT(vkQueueWaitIdle(ZestDevice->graphics_queue.vk_queue));
...
zest_create_info_t zest_CreateInfoWithValidationLayers(zest_validation_flags flags) {
 ...
 zest_window zest_GetCurrentWindow() {
     return ZestRenderer->current_window;
}

 void zest_CloseWindow(const char *name) {
     if (zest_map_valid_name(ZestRenderer->windows, name)) {
         zest_window window = *zest_map_at(ZestRenderer->windows, name);
         ZestRenderer->destroy_window_callback(window, ZestApp->user_data);
         zest_map_remove(ZestRenderer->windows, name);
         ZEST__FREE(window);
     }
}
...
zest_uint zest_AcquireBindlessTextureIndex(zest_texture texture, zest_set_layout layout, zest_descriptor_set set, zest_global_binding_number target_binding_number) {
    ZEST_ASSERT_HANDLE(layout);  //Must be a valid handle to a descriptor set layout
    ZEST_ASSERT_HANDLE(texture); //Must be a valid handle to a texture
    ...
    VkWriteDescriptorSet write = { 0 };
	VkDescriptorImageInfo image_info = texture->descriptor_image_info[texture->fif];
...
zest_uint zest_AcquireGlobalCombinedSampler2d(zest_texture texture) {
    texture->bindless_index[zest_combined_image_sampler_2d_binding] = zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_2d_binding);
    return texture->bindless_index[zest_combined_image_sampler_2d_binding];
}

zest_uint zest_AcquireGlobalCombinedSampler3d(zest_texture texture) {
    texture->bindless_index[zest_combined_image_sampler_3d_binding] = zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_3d_binding);
    return texture->bindless_index[zest_combined_image_sampler_3d_binding];
}

zest_uint zest_AcquireGlobalCombinedSamplerCube(zest_texture texture) {
    texture->bindless_index[zest_combined_image_sampler_cube_binding] = zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_cube_binding);
    return texture->bindless_index[zest_combined_image_sampler_cube_binding];
}

zest_uint zest_AcquireGlobalCombinedSamplerArray(zest_texture texture) {
    texture->bindless_index[zest_combined_image_sampler_array_binding] = zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_combined_image_sampler_array_binding);
...
zest_uint zest_AcquireGlobalSampledImage(zest_texture texture) {
    return zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_sampled_image_binding);
}

zest_uint zest_AcquireGlobalSampler(zest_texture texture) {
    return zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_sampler_binding);
}

zest_uint zest_AcquireGlobalStorageSampler(zest_texture texture) {
    return zest_AcquireBindlessTextureIndex(texture, ZestRenderer->global_bindless_set_layout, ZestRenderer->global_set, zest_storage_image_binding);
}

zest_uint *zest_AcquireGlobalTextureMipIndexes(zest_texture texture, zest_global_binding_number binding_number) {
    ZEST_ASSERT_HANDLE(texture);  //No bindless layout has been set in the frame graph
    ZEST_ASSERT(texture->mip_levels > 1);   //The resource does not have any mip levels. Make sure to set the number of mip levels when creating the resource in the frame graph

    if (zest_map_valid_key(texture->image_buffer[texture->fif].mip_indexes, (zest_key)binding_number)) {
        zest_mip_index_collection *mip_collection = zest_map_at_key(texture->image_buffer[texture->fif].mip_indexes, (zest_key)binding_number);
        return mip_collection->mip_indexes;
    }
    VkImageView *mip_views = texture->image_buffer[texture->fif].mip_views;
    if (!mip_views) {
        zest_vec_resize(texture->image_buffer[texture->fif].mip_views, texture->mip_levels);
        mip_views = texture->image_buffer[texture->fif].mip_views;
    ...
    for (int mip_index = 0; mip_index != texture->mip_levels; ++mip_index) {
        ...
        if (mip_views[mip_index]) {
            ...
            VkResult result = zest__create_image_view(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, 1, mip_index, texture->vk_image_view_type, texture->layer_count, &mip_view);
    ...
    zest_map_insert_key( texture->image_buffer[texture->fif].mip_indexes, (zest_key)binding_number, mip_collection);
    zest_uint size = zest_map_size(texture->image_buffer[texture->fif].mip_indexes);
    return mip_collection.mip_indexes;
}
...
void zest_ReleaseGlobalTextureIndex(zest_texture texture, zest_global_binding_number binding_number) {
...
zest_resource_node_t zest__create_import_image_resource_node(const char *name, zest_texture texture) {
    ...
    node.frame_graph = frame_graph;
    node.magic = zest_INIT_MAGIC(zest_struct_type_resource_node);
    node.image_buffer = texture->image_buffer[texture->fif];
    node.image_desc.width = texture->width;
    node.image_desc.height = texture->height;
...
zest_image zest_AddTextureImageBitmap(zest_texture texture, zest_bitmap bitmap_to_load) {
    zest_image image = zest_CreateImage();
    zest__maybe_create_image_collection(texture);
    zest_vec_push(texture->image_collection->images, image);
    texture->image_index = zest_vec_last_index(texture->image_collection->images);
...
void zest__cleanup_texture(zest_texture texture) {
    ...
    zest_ForEachFrameInFlight(fif) {
        zest_vec_foreach(i, texture->image_buffer[fif].mip_views) {
            VkImageView image_view = texture->image_buffer[fif].mip_views[i];
            if (image_view) {
	            vkDestroyImageView(ZestDevice->logical_device, image_view, &ZestDevice->allocation_callbacks);
            }
        }
        zest_vec_free(texture->image_buffer[fif].mip_views);
		zest_uint size = zest_map_size(texture->image_buffer[texture->fif].mip_indexes);
        zest_map_foreach(i, texture->image_buffer[fif].mip_indexes) {
            zest_vec_free(texture->image_buffer[fif].mip_indexes.data[i].mip_indexes);
        }
		zest_map_free(texture->image_buffer[fif].mip_indexes);
    }
...
void zest__create_texture_debug_set(zest_texture texture) {
	...
	zest_AddSetBuilderCombinedImageSampler(&set_builder, 0, 0, texture->sampler->vk_sampler, texture->descriptor_image_info[texture->fif].imageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
VkResult zest__process_texture_images(zest_texture texture, VkCommandBuffer command_buffer) {
    ...
    if (texture->storage_type == zest_texture_storage_type_packed) {
        zest__pack_images(texture, texture->width);
    }
    ...
    if (texture->storage_type < zest_texture_storage_type_sprite_sheet) {
        ...
        ZEST_RETURN_RESULT_ON_FAIL(zest__create_texture_image_view(texture, texture->vk_image_view_type, mip_levels, texture->layer_count));
        ...
        if (texture->flags & zest_texture_flag_use_filtering) {
            mip_levels = (zest_uint)floor(log2(ZEST__MAX(texture->width, texture->height))) + 1;
        } else {
            mip_levels = 1;
        }
        if (texture->vk_image_view_type == VK_IMAGE_VIEW_TYPE_CUBE) {
			ZEST_RETURN_RESULT_ON_FAIL(zest__create_image_array(texture->width, texture->height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer[texture->fif].image_handles));
			texture->image_buffer[texture->fif].format = texture->vk_image_format;
            texture->layer_count = 6;
            zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, texture->vk_image_layout, mip_levels, texture->layer_count, command_buffer);
        } else {
...
zest_uint zest_GetTextureDescriptorIndex(zest_texture texture, zest_global_binding_number binding_number) {
...
VkDescriptorImageInfo *zest_GetTextureDescriptorImageInfo(zest_texture texture) {
    return &texture->descriptor_image_info[texture->fif];
}
...
VkResult zest__create_texture_image(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout vk_image_layout, zest_bool copy_bitmap, VkCommandBuffer command_buffer) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image(zest_GetTextureSingleBitmap(texture)->meta.width, zest_GetTextureSingleBitmap(texture)->meta.height, mip_levels, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_TILING_OPTIMAL, usage_flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer[texture->fif].image_handles));

    texture->image_buffer[texture->fif].format = texture->vk_image_format;
    if (copy_bitmap) {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1, command_buffer));
        ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image_buffer[texture->fif].image_handles.image, (zest_uint)zest_GetTextureSingleBitmap(texture)->meta.width, (zest_uint)zest_GetTextureSingleBitmap(texture)->meta.height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, command_buffer));
		if (!command_buffer) {
			zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);
		} else {
			zest_vec_push(ZestRenderer->staging_buffers, staging_buffer);
		}
        ZEST_CLEANUP_ON_FAIL(zest__generate_mipmaps(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, zest_GetTextureSingleBitmap(texture)->meta.width, zest_GetTextureSingleBitmap(texture)->meta.height, mip_levels, 1, vk_image_layout, command_buffer));
    }
    else {
        ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, vk_image_layout, mip_levels, 1, command_buffer));
    }
...
VkResult zest__create_texture_image_array(zest_texture texture, zest_uint mip_levels, VkCommandBuffer command_buffer) {
    ...
    VkResult result = zest__create_image_array(texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->layer_count, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, 0, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer[texture->fif].image_handles);
    if (result != VK_SUCCESS) {
        goto cleanup;
    }
    ...
    zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, texture->layer_count, command_buffer);
    ...
    zest__generate_mipmaps(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, texture->image_collection->bitmap_array.meta[0].width, texture->image_collection->bitmap_array.meta[0].height, mip_levels, texture->layer_count, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, command_buffer);
...
VkResult zest__create_texture_image_view(zest_texture texture, VkImageViewType view_type, zest_uint mip_levels, zest_uint layer_count) {
     return zest__create_image_view(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, view_type, layer_count, &texture->image_buffer[texture->fif].image_handles.view);
}
...
void zest__pack_images(zest_texture texture, zest_uint size) {
    ...
    if (texture->storage_type == zest_texture_storage_type_sprite_sheet) {
        ...
        zest_FreeBitmapArray(&texture->image_collection->bitmap_array);
        zest_CreateBitmapArray(&texture->image_collection->bitmap_array, size, size, 4, texture->layer_count);
    }
...
zest_texture zest_ReplaceTexture(zest_texture texture, zest_texture_storage_type storage_type, zest_bool use_filtering, zest_texture_format vk_image_format, zest_uint reserve_images) {
    ZEST_ASSERT_HANDLE(texture); //Not a valid texture handle
    ...
    zest_SetTextureImageFormat(new_texture, vk_image_format);
    if (storage_type < zest_texture_storage_type_render_target) {
    ...
    if (reserve_images) {
        zest__maybe_create_image_collection(new_texture);
        zest_vec_reserve(new_texture->image_collection->images, reserve_images);
    }        
    zest_map_insert(ZestRenderer->textures, texture->name.str, new_texture);
    ZEST__FLAG(texture->flags, zest_texture_flag_was_replaced);
    zest_vec_push(ZestRenderer->deferred_resource_freeing_list.textures[ZEST_FIF], texture);
    return new_texture;
}

zest_texture zest_CreateTexturePacked(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_packed, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture zest_CreateTextureSpritesheet(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_sprite_sheet, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture zest_CreateTextureSingle(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_single, zest_texture_flag_use_filtering, vk_image_format, 1);
}

zest_texture zest_CreateTextureBank(const char* name, zest_texture_format vk_image_format) {
    return zest_CreateTexture(name, zest_texture_storage_type_bank, zest_texture_flag_use_filtering, vk_image_format, 10);
}

zest_texture zest_CreateTextureStorage(const char* name, int width, int height, zest_texture_format format, VkImageViewType view_type, zest_bool with_mip_maps) {
    zest_texture texture = zest_CreateTexture(name, zest_texture_storage_type_storage, with_mip_maps, format, 1);
...
zest_texture zest_LoadCubemap(const char *name, const char *file_name) {
    ...
    ZEST_CLEANUP_ON_FAIL(zest__create_image_array(bitmap_array->meta[0].width, bitmap_array->meta[0].height, mip_levels, 6, VK_SAMPLE_COUNT_1_BIT, texture->vk_image_format, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->image_buffer[texture->fif].image_handles));
    texture->image_buffer[texture->fif].format = texture->vk_image_format;

    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 6, 0));
    ZEST_CLEANUP_ON_FAIL(zest__copy_buffer_regions_to_image(texture->image_collection->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->buffer_offset, texture->image_buffer[texture->fif].image_handles.image, 0));
    ZEST_CLEANUP_ON_FAIL(zest__create_image_view(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, 0, VK_IMAGE_VIEW_TYPE_CUBE, 6, &texture->image_buffer[texture->fif].image_handles.view));
    texture->descriptor_image_info[texture->fif].imageView = texture->image_buffer[texture->fif].image_handles.view;
    texture->descriptor_image_info[texture->fif].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ZEST_CLEANUP_ON_FAIL(zest__transition_image_layout(texture->image_buffer[texture->fif].image_handles.image, texture->vk_image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mip_levels, 6, 0));
    ...
    texture->descriptor_image_info[texture->fif].sampler = texture->sampler->vk_sampler;
...
zest_bool zest_TextureClear(zest_texture texture) {
    ZEST_ASSERT_HANDLE(texture);	//Not a valid handle!
    VkCommandBuffer command_buffer = 0;
    ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&command_buffer));
...
zest_bool zest_CopyFramebufferToTexture(zest_frame_buffer_t* src_image, zest_texture texture, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ZEST_ASSERT_HANDLE(texture);	//Not a valid handle!
    VkCommandBuffer copy_command = 0;
    ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&copy_command));

    VkImageLayout target_layout = texture->descriptor_image_info[texture->fif].imageLayout;
...
zest_bool zest_CopyTextureToTexture(zest_texture src_image, zest_texture target, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ZEST_ASSERT_HANDLE(src_image);	//Not a valid handle!
    ZEST_ASSERT_HANDLE(target);	    //Not a valid handle!
    VkCommandBuffer copy_command = 0; 
    ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&copy_command));

    VkImageLayout target_layout = target->descriptor_image_info[target->fif].imageLayout;
...
zest_bool zest_CopyTextureToBitmap(zest_texture src_image, zest_bitmap image, int src_x, int src_y, int dst_x, int dst_y, int width, int height, zest_bool swap_channel) {
    ZEST_ASSERT_HANDLE(src_image);	//Not a valid handle!
    VkCommandBuffer copy_command = 0;
    ZEST_RETURN_FALSE_ON_FAIL(zest__begin_single_time_commands(&copy_command));
    VkImage temp_image;
    ...
    zest_bool supports_blit = zest__has_blit_support(src_image->image_buffer[src_image->fif].format);
...
VkImageLayout zest_GetTextureLayout(zest_texture texture) {
    ZEST_ASSERT_HANDLE(texture);    //Not a valid texture handle
...
void zest_DrawInstanceLayer(VkCommandBuffer command_buffer, const zest_frame_graph_context_t *context, void *user_data) {
    ...
    zest_vec_foreach(i, layer->draw_instructions[layer->fif]) {
        ...
        ZEST_ASSERT_HANDLE(current->shader_resources); //Shader resources must be set in the instruction
...
void zest__record_mesh_layer(zest_layer layer, zest_uint fif) {
    ...
    zest_vec_foreach(i, layer->draw_instructions[layer->fif]) {
		...
		ZEST_ASSERT_HANDLE(current->shader_resources);     //Not a valid descriptor set in the shader resource. Make sure all frame in flights are set
...
void zest_SetMSDFFontDrawing(zest_layer layer, zest_font font) {
    ...
    push.aa_factor = 5.f;
    push.thickness = 5.5f;
    push.font_texture_index = font->texture->bindless_index[zest_combined_image_sampler_2d_binding];
...
void zest_SetInstanceDrawing(zest_layer layer, zest_shader_resources shader_resources, zest_pipeline_template pipeline) {
	...
	ZEST_ASSERT_HANDLE(shader_resources);   //You must specifiy the shader resources to draw with
...
void zest_Set3DLineDrawing(zest_layer layer, zest_shader_resources shader_resources, zest_pipeline_template pipeline) {
    ZEST_ASSERT_HANDLE(layer);	//Not a valid handle!
    ZEST_ASSERT_HANDLE(shader_resources);	//Not a valid handle!
    zest_EndInstanceInstructions(layer);
    zest_StartInstanceInstructions(layer);
    layer->current_instruction.pipeline_template = pipeline;
	layer->current_instruction.shader_resources = shader_resources;

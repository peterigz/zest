...
zest_uint zest_FloatToHalf(float f) {
    ...
    uint32_t sign = (u.u & 0x80000000) >> 16;
    uint32_t rest = u.u & 0x7FFFFFFF;
    ...
    uint32_t exponent = rest & 0x7F800000;
    uint32_t mantissa = rest & 0x007FFFFF;
...
void zest__initialise_renderer(zest_create_info_t* create_info) {
    ...
    if (ZEST__FLAGGED(create_info->flags, zest_init_flag_use_depth_buffer)) {
        ZestRenderer->final_render_pass = zest__get_render_pass(ZestRenderer->swapchain_image_format, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_ATTACHMENT_LOAD_OP_CLEAR, ZEST_TRUE);
        ...
        ZestRenderer->final_render_pass = zest__get_render_pass(ZestRenderer->swapchain_image_format, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_ATTACHMENT_LOAD_OP_CLEAR, ZEST_FALSE);
    }
...
VkRenderPass zest__get_render_pass_with_info(zest_render_pass_info_t render_pass_info) {
    return zest__get_render_pass(render_pass_info.render_format, render_pass_info.final_layout, render_pass_info.load_op, render_pass_info.depth_buffer);
}

VkRenderPass zest__get_render_pass(VkFormat render_format, VkImageLayout final_layout, VkAttachmentLoadOp load_op, zest_bool depth_buffer) {
    ...
    zest_key hash = zest_Hash(ZestHasher, &render_pass_info, sizeof(zest_render_pass_info_t), ZEST_HASH_SEED);
    if (zest_map_valid_key(ZestRenderer->render_passes, hash)) {
		return *zest_map_at_key(ZestRenderer->render_passes, hash); 
    }
    VkRenderPass render_pass = zest__create_render_pass(render_format, final_layout, load_op, depth_buffer);
...
zest_descriptor_set_layout zest__add_descriptor_set_layout(const char *name, VkDescriptorSetLayout layout) {
    zest_descriptor_set_layout descriptor_layout = ZEST__NEW(zest_descriptor_set_layout);
    memset(descriptor_layout, 0, sizeof(zest_descriptor_set_layout));
    descriptor_layout->name.str = 0;
    descriptor_layout->magic = zest_INIT_MAGIC;
    zest_SetText(&descriptor_layout->name, name);
...
void zest_AddDescriptorSetToResources(zest_shader_resources resources, zest_descriptor_set descriptor_set) {
	zest_vec_push(resources->sets, descriptor_set);
}
...
zest_shader_resources zest_CombineUniformAndTextureSampler(zest_descriptor_set descriptor_set, zest_texture texture) {
    ...
    zest_AddDescriptorSetToResources(shader_resources, descriptor_set);
    zest_AddDescriptorSetToResources(shader_resources, &texture->descriptor_sets[texture->current_index]);
...
zest_pipeline_template_create_info_t zest_CreatePipelineTemplateCreateInfo(void) {
    ...
    zest_AddPipelineTemplateDescriptorLayout(&create_info, *zest_GetDescriptorSetLayout("uniform buffer"));
...
void zest_AddPipelineTemplateDescriptorLayout(zest_pipeline_template_create_info_t *create_info, VkDescriptorSetLayout layout) {
    zest_vec_push(create_info->descriptorSetLayouts, layout);
}

void zest_ClearPipelineTemplateDescriptorLayouts(zest_pipeline_template_create_info_t *create_info) {
    zest_vec_clear(create_info->descriptorSetLayouts);
}
...
void zest__set_pipeline_template(zest_pipeline_template pipeline_template, zest_pipeline_template_create_info_t* create_info) {
    ...
    if (zest_TextSize(&create_info->shader_path_prefix)) {
    ...
    pipeline_template->pipelineLayoutInfo.setLayoutCount = zest_vec_size(create_info->descriptorSetLayouts);
    pipeline_template->pipelineLayoutInfo.pSetLayouts = create_info->descriptorSetLayouts;
...
zest_pipeline zest__cache_pipeline(zest_pipeline_template pipeline_template, VkRenderPass render_pass) {
    ...
    zest_MakePipelineDescriptorWrites(pipeline);
    zest_key pipeline_key = zest_Hash(ZestHasher, pipeline_template->name.str, zest_TextLength(&pipeline_template->name), ZEST_HASH_SEED);
    struct {
    ...
    zest_key cached_pipeline_key = zest_Hash(ZestHasher, &cached_pipeline, sizeof(cached_pipeline), ZEST_HASH_SEED);
...
void zest_BuildPipeline(zest_pipeline pipeline) {
    ...
    pipeline->pipeline_template->vertShaderStageInfo.pName = pipeline->pipeline_template->create_info.vertShaderFunctionName.str;
    ...
    pipeline->pipeline_template->fragShaderStageInfo.pName = pipeline->pipeline_template->create_info.fragShaderFunctionName.str;
...
void zest_MakePipelineTemplate(zest_pipeline_template pipeline_template, zest_pipeline_template_create_info_t* create_info) {
    if (create_info != &pipeline_template->create_info) {
        pipeline_template->create_info = zest__copy_pipeline_create_info(create_info);
        zest__free_pipeline_create_info(create_info);
    }
    pipeline_template->create_info.viewport.extent = zest_GetSwapChainExtent();

    zest__set_pipeline_template(pipeline_template, &pipeline_template->create_info);
    ZEST_ASSERT(zest_vec_size(create_info->descriptorSetLayouts));  //you must define at least one descriptor set layout for the pipeline
    pipeline_template->multisampling.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;
}
...
zest_pipeline_template_create_info_t zest__copy_pipeline_create_info(zest_pipeline_template_create_info_t* create_info) {
    ...
    zest_vec_foreach(i, create_info->descriptorSetLayouts) {
        zest_vec_push(copy.descriptorSetLayouts, create_info->descriptorSetLayouts[i]);
    }
...
void zest__refresh_pipeline_template(zest_pipeline_template pipeline_template) {
    ...
    pipeline_template->create_info.viewport.extent = zest_GetSwapChainExtent();
    zest__update_pipeline_template(pipeline_template, &pipeline_template->create_info);
}

void zest__rebuild_pipeline(zest_pipeline pipeline) {
    zest_BuildPipeline(pipeline);
    for (ZEST_EACH_FIF_i) {
        zest_vec_clear(pipeline->descriptor_sets);
    }
}
...
zest_pipeline_template zest_AddPipeline(const char* name) {
    zest_pipeline_template_t blank = { 0 };
    zest_pipeline_template pipeline = ZEST__NEW(zest_pipeline_template);
    *pipeline = blank;
    pipeline->magic = zest_INIT_MAGIC;
...
VkDescriptorSetLayout *zest_GetDescriptorSetLayout(const char* name) { 
...
zest_pipeline_template zest_PipelineTemplate(const char *name) {
    zest_key pipeline_key = zest_Hash(ZestHasher, name, strlen(name), ZEST_HASH_SEED);
...
zest_pipeline zest_PipelineWithTemplate(zest_pipeline_template template, VkRenderPass render_pass) {
    zest_key pipeline_key = zest_Hash(ZestHasher, template->name.str, zest_TextLength(&template->name), ZEST_HASH_SEED);
    struct {
        zest_key pipeline_key;
        VkRenderPass render_pass;
    } cached_pipeline;
...
zest_pipeline_template_create_info_t* zest_PipelineCreateInfo(const char* name) { 
    ZEST_ASSERT(zest_map_valid_name(ZestRenderer->pipelines, name)); 
    zest_pipeline_template pipeline_template = *zest_map_at(ZestRenderer->pipelines, name); 
    return &pipeline_template->create_info; 
}
...
void zest_SendPushConstants(VkCommandBuffer command_buffer, zest_pipeline pipeline, VkShaderStageFlags shader_flags, zest_uint size, void* data) {
...
void zest__prepare_standard_pipelines() {
    ...
    //2d sprite rendering
    zest_pipeline_template sprite_instance_pipeline = zest_AddPipeline("pipeline_2d_sprites");
    ...
    zest_MakePipelineTemplate(sprite_instance_pipeline, &instance_create_info);
    ...
    zest_pipeline_template sprite_instance_pipeline_alpha = zest_AddPipeline("pipeline_2d_sprites_alpha");
    zest_SetText(&instance_create_info.vertShaderFile, "sprite_vert.spv");
    zest_SetText(&instance_create_info.fragShaderFile, "sprite_alpha_frag.spv");
    zest_MakePipelineTemplate(sprite_instance_pipeline_alpha, &instance_create_info);
    ...
    zest_pipeline_template line_instance_pipeline = zest_AddPipeline("pipeline_line_instance");
    ...
    zest_ClearPipelineTemplateDescriptorLayouts(&instance_create_info);
    zest_AddPipelineTemplateDescriptorLayout(&instance_create_info, *zest_GetDescriptorSetLayout("uniform buffer"));
    zest_MakePipelineTemplate(line_instance_pipeline, &instance_create_info);
    ...
    zest_pipeline_template line3d_instance_pipeline = zest_AddPipeline("pipeline_line3d_instance");
    ...
    zest_MakePipelineTemplate(line3d_instance_pipeline, &instance_create_info);
    ...
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 1, VK_FORMAT_R8G8B8_SNORM, offsetof(zest_billboard_instance_t, alignment)));		         	// Location 9: Alignment X, Y and Z
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 2, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(zest_billboard_instance_t, rotations_stretch)));	// Location 2: Rotations + stretch
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 3, VK_FORMAT_R16G16B16A16_SNORM, offsetof(zest_billboard_instance_t, uv)));		    		// Location 1: uv_packed
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 4, VK_FORMAT_R16G16B16A16_SSCALED, offsetof(zest_billboard_instance_t, scale_handle)));		// Location 4: Scale + Handle
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 5, VK_FORMAT_R32_UINT, offsetof(zest_billboard_instance_t, intensity_texture_array)));		// Location 6: texture array index * intensity
    zest_vec_push(billboard_vertex_input_attributes, zest_CreateVertexInputDescription(0, 6, VK_FORMAT_R8G8B8A8_UNORM, offsetof(zest_billboard_instance_t, color)));			        // Location 7: Instance Color

    zest_pipeline_template billboard_instance_pipeline = zest_AddPipeline("pipeline_billboard");
    ...
    zest_SetText(&instance_create_info.fragShaderFile, "image_frag.spv");
    zest_MakePipelineTemplate(billboard_instance_pipeline, &instance_create_info);
    billboard_instance_pipeline->depthStencil.depthWriteEnable = VK_FALSE;
    billboard_instance_pipeline->depthStencil.depthTestEnable = VK_TRUE;
    ...
    zest_pipeline_template billboard_pipeline_alpha = zest_AddPipeline("pipeline_billboard_alpha");
    zest_SetText(&instance_create_info.vertShaderFile, "billboard_vert.spv");
    zest_SetText(&instance_create_info.fragShaderFile, "sprite_alpha_frag.spv");
    zest_MakePipelineTemplate(billboard_pipeline_alpha, &instance_create_info);
    billboard_pipeline_alpha->depthStencil.depthWriteEnable = VK_FALSE;
    billboard_pipeline_alpha->depthStencil.depthTestEnable = VK_TRUE;
    ...
    zest_vec_push(instance_create_info.pushConstantRange, image_pushconstant_range);
    zest_pipeline_template font_pipeline = zest_AddPipeline("pipeline_fonts");
    zest_SetText(&instance_create_info.vertShaderFile, "sprite_vert.spv");
    zest_SetText(&instance_create_info.fragShaderFile, "font_frag.spv");
    zest_MakePipelineTemplate(font_pipeline, &instance_create_info);
    ...
    zest_pipeline_template imgui_pipeline = zest_AddPipeline("pipeline_imgui");
    ...
    zest_ClearPipelineTemplateDescriptorLayouts(&imgui_pipeline_template);
    zest_AddPipelineTemplateDescriptorLayout(&imgui_pipeline_template, *zest_GetDescriptorSetLayout("1 sampler"));
    zest_MakePipelineTemplate(imgui_pipeline, &imgui_pipeline_template);
    ...
    zest_pipeline_template mesh_pipeline = zest_AddPipeline("pipeline_mesh");
    ...
    zest_AddPipelineTemplateDescriptorLayout(&mesh_pipeline_template, *zest_GetDescriptorSetLayout("1 sampler"));
    zest_MakePipelineTemplate(mesh_pipeline, &mesh_pipeline_template);
    ...
    zest_pipeline_template imesh_pipeline = zest_AddPipeline("pipeline_mesh_instance");
    ...
    zest_ClearPipelineTemplateDescriptorLayouts(&imesh_pipeline_template);
    zest_AddPipelineTemplateDescriptorLayout(&imesh_pipeline_template, *zest_GetDescriptorSetLayout("uniform buffer"));
    zest_MakePipelineTemplate(imesh_pipeline, &imesh_pipeline_template);
    ...
    zest_pipeline_template final_render = zest_AddPipeline("pipeline_swap_chain");
    ...
    zest_ClearPipelineTemplateDescriptorLayouts(&final_render->create_info);
    zest_AddPipelineTemplateDescriptorLayout(&final_render->create_info, *zest_GetDescriptorSetLayout("1 sampler"));
    zest_MakePipelineTemplate(final_render, &final_render->create_info);
...
void zest__process_texture_images(zest_texture texture, VkCommandBuffer command_buffer) {
    ...
    if (texture->storage_type < zest_texture_storage_type_sprite_sheet) {
        ...
        texture->descriptor_image_info[texture->current_index].imageView = texture->frame_buffer[texture->current_index].view;
...
zest_descriptor_set zest_GetTextureDescriptorSet(zest_texture texture) {
...
void zest__create_texture_image(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout image_layout, zest_bool copy_bitmap, VkCommandBuffer command_buffer) {
    ...
    texture->frame_buffer[texture->current_index].buffer = zest__create_image(zest_GetTextureSingleBitmap(texture)->width, zest_GetTextureSingleBitmap(texture)->height, mip_levels, VK_SAMPLE_COUNT_1_BIT, texture->image_format, VK_IMAGE_TILING_OPTIMAL, usage_flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->frame_buffer[texture->current_index].image);

    texture->frame_buffer[texture->current_index].format = texture->image_format;
    if (copy_bitmap) {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1, command_buffer);
        zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->memory_offset, texture->frame_buffer[texture->current_index].image, (zest_uint)zest_GetTextureSingleBitmap(texture)->width, (zest_uint)zest_GetTextureSingleBitmap(texture)->height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, command_buffer);
		if (!command_buffer) {
			zloc_FreeRemote(staging_buffer->buffer_allocator->allocator, staging_buffer);
		} else {
			zest_vec_push(ZestRenderer->staging_buffers, staging_buffer);
		}
        zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, zest_GetTextureSingleBitmap(texture)->width, zest_GetTextureSingleBitmap(texture)->height, mip_levels, 1, image_layout, command_buffer);
    }
    else {
...
void zest__create_texture_image_array(zest_texture texture, zest_uint mip_levels, VkCommandBuffer command_buffer) {
    ...
    texture->frame_buffer[texture->current_index].buffer = zest__create_image_array(texture->bitmap_array.width, texture->bitmap_array.height, mip_levels, texture->layer_count, VK_SAMPLE_COUNT_1_BIT, texture->image_format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &texture->frame_buffer[texture->current_index].image);
    texture->frame_buffer[texture->current_index].format = texture->image_format;

    zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, texture->layer_count, command_buffer);
    zest__copy_buffer_regions_to_image(texture->buffer_copy_regions, *zest_GetBufferDeviceBuffer(staging_buffer), staging_buffer->memory_offset, texture->frame_buffer[texture->current_index].image, command_buffer);
    ...
    zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, texture->bitmap_array.width, texture->bitmap_array.height, mip_levels, texture->layer_count, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, command_buffer);
}
...
void zest__create_texture_stream(zest_texture texture, zest_uint mip_levels, VkImageUsageFlags usage_flags, VkImageLayout image_layout, zest_bool copy_bitmap, VkCommandBuffer command_buffer) {
    ...
    if (copy_bitmap) {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mip_levels, 1, command_buffer);
        zest__copy_buffer_to_image(*zest_GetBufferDeviceBuffer(texture->stream_staging_buffer), texture->stream_staging_buffer->memory_offset, texture->frame_buffer[texture->current_index].image, (zest_uint)zest_GetTextureSingleBitmap(texture)->width, (zest_uint)zest_GetTextureSingleBitmap(texture)->height, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, command_buffer);
        zest__generate_mipmaps(texture->frame_buffer[texture->current_index].image, texture->image_format, zest_GetTextureSingleBitmap(texture)->width, zest_GetTextureSingleBitmap(texture)->width, mip_levels, 1, image_layout, command_buffer);
    }
    else {
        zest__transition_image_layout(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_LAYOUT_UNDEFINED, image_layout, mip_levels, 1, command_buffer);
    }
...
void zest__create_texture_image_view(zest_texture texture, VkImageViewType view_type, zest_uint mip_levels, zest_uint layer_count) {
    texture->frame_buffer[texture->current_index].view = zest__create_image_view(texture->frame_buffer[texture->current_index].image, texture->image_format, VK_IMAGE_ASPECT_COLOR_BIT, mip_levels, view_type, layer_count);
}
...
zest_frame_buffer_t zest_CreateFrameBuffer(VkRenderPass render_pass, uint32_t width, uint32_t height, VkFormat render_format, zest_bool use_depth_buffer, zest_bool is_src) {
    ...
    zest__transition_image_layout(frame_buffer.color_buffer.image, color_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, 1, 0);
...
void zest_TextureClear(zest_texture texture) {
    ...
    zest__insert_image_memory_barrier(
        command_buffer,
        texture->frame_buffer[texture->current_index].image,
        0,
    ...
    vkCmdClearColorImage(command_buffer, texture->frame_buffer[texture->current_index].image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &ClearColorValue, 1, &image_sub_resource_range);
...
void zest_CopyFramebufferToTexture(zest_frame_buffer_t* src_image, zest_texture texture, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ...
    vkCmdBlitImage(
        ...
        texture->frame_buffer[texture->current_index].image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &image_blit,
        VK_FILTER_LINEAR);
...
void zest_CopyTextureToTexture(zest_texture src_image, zest_texture target, int src_x, int src_y, int dst_x, int dst_y, int width, int height) {
    ...
    zest__insert_image_memory_barrier(
        copy_command,
        target->frame_buffer[target->current_index].image,
        0,
        VK_ACCESS_TRANSFER_WRITE_BIT,
    ...
    zest__insert_image_memory_barrier(
        copy_command,
        src_image->frame_buffer[src_image->current_index].image,
        VK_ACCESS_MEMORY_READ_BIT,
        VK_ACCESS_TRANSFER_READ_BIT,
    ...
    vkCmdBlitImage(
        copy_command,
        src_image->frame_buffer[src_image->current_index].image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        target->frame_buffer[target->current_index].image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1,
        &image_blit_region,
        VK_FILTER_NEAREST);
...
void zest_CopyTextureToBitmap(zest_texture src_image, zest_bitmap_t* image, int src_x, int src_y, int dst_x, int dst_y, int width, int height, zest_bool swap_channel) {
    ...
    zest_bool supports_blit = zest__has_blit_support(src_image->frame_buffer[src_image->current_index].format);
...
void zest__initialise_font_layer(zest_layer layer, zest_uint instance_pool_size) {
    ...
    layer->instance_struct_size = sizeof(zest_sprite_instance_t);
    ...
    for (ZEST_EACH_FIF_i) {
        layer->memory_refs[i].device_instance_data = zest_CreateBuffer(sizeof(zest_sprite_instance_t) * instance_pool_size, &device_buffer_info, ZEST_NULL);
        if (ZEST__NOT_FLAGGED(layer->flags, zest_layer_flag_device_local_direct)) {
            layer->memory_refs[i].staging_instance_data = zest_CreateBuffer(sizeof(zest_sprite_instance_t) * instance_pool_size, &staging_buffer_info, ZEST_NULL);
...
void zest__initialise_render_target(zest_render_target render_target, zest_render_target_create_info_t* info) {
    ...
    for (zest_index i = 0; i != render_target->frames_in_flight; ++i) {
        render_target->framebuffers[i] = zest_CreateFrameBuffer(render_target->render_pass,
	...
	render_target->sampler_texture = VK_NULL_HANDLE;

	zest_text_t texture_name = { 0 };
	zest_SetTextf(&texture_name, "%s_texture", render_target->name);
	render_target->sampler_texture = zest_CreateTexture(texture_name.str, zest_texture_storage_type_render_target, 0, render_target->render_format, 0);
	zest_FreeText(&texture_name);

	zest_texture texture = render_target->sampler_texture;
	...
	for (ZEST_EACH_FIF_i) {
		zest__record_render_target_commands(render_target, i);
	}
...
void zest__create_render_target_sampler_image(zest_render_target render_target) {
    ...
    for (zest_index fif = 0; fif != render_target->frames_in_flight; ++fif) {
        ...
        render_target->image_info[fif].imageView = render_target->framebuffers[fif].color_buffer.view;
        render_target->image_info[fif].sampler = texture->sampler[texture->current_index];
...
zest_texture zest_GetRenderTargetTexture(zest_render_target render_target) {
    ZEST_CHECK_HANDLE(render_target);	//Not a valid handle!
    return render_target->sampler_texture;
}
...
void zest_RecreateRenderTargetResources(zest_render_target render_target, zest_index fif) {
    ...
    render_target->pipeline_template->create_info.viewport.extent = render_target->viewport.extent;
    render_target->pipeline_template->create_info.viewport.offset = render_target->viewport.offset;
...
void zest__refresh_render_target_sampler(zest_render_target render_target) {
    ZEST_CHECK_HANDLE(render_target);	//Not a valid handle!
    render_target->sampler_texture->current_index ^= 1;
	zest__cleanup_unused_texture_buffers(render_target->sampler_texture, render_target->sampler_texture->current_index);
	zest_texture texture = render_target->sampler_texture;
...
void zest_PreserveRenderTargetFrames(zest_render_target render_target, zest_bool yesno) {
    ZEST_CHECK_HANDLE(render_target);	//Not a valid handle!
    if (ZEST__FLAGGED(render_target->flags, zest_render_target_flag_use_depth_buffer)) {
        render_target->render_pass = (yesno != 0 ? zest__get_render_pass(render_target->render_format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, 1) 
            : zest__get_render_pass(render_target->render_format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, 1));
    }
    else {
        render_target->render_pass = (yesno != 0 ? zest__get_render_pass(render_target->render_format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, 0) 
            : zest__get_render_pass(render_target->render_format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, 0));
    }
}
